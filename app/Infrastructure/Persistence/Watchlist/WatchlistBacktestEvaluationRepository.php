...
{
    private const KEY_FIELDS = ['policy_code', 'param_id', 'eval_model', 'paramset_hash', 'from_date', 'to_date'];
    private const PAYLOAD_FIELDS = [
        'policy_code', 'param_id', 'eval_model', 'paramset_hash', 'from_date', 'to_date', 'days_covered', 'picks_count',
    ...
    public function persist(array $row): array
    {
        ...
        if ($existing !== null) {
            ...
            if ($existingPayload !== $payload) {
                throw new \RuntimeException('WS_BT_OOS_PROOF_MISSING: Duplicate persistence conflict for watchlist_bt_eval; existing payload differs.');
            }
    ...
    private function canonicalRow(array $row): array
    {
        ...
        foreach (self::PAYLOAD_FIELDS as $field) {
            ...
            } elseif (! in_array($field, ['policy_code', 'eval_model', 'paramset_hash', 'from_date', 'to_date'], true)) {
