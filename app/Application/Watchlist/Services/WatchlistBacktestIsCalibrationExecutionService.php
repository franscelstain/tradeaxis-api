...
class WatchlistBacktestIsCalibrationExecutionService
{
    ...
    public function execute(
        ...
        try {
            ...
            $c07Before = [];
            if (! empty($definition['requires_c07_catalog'])) {
                $c07Before = $this->paramGrid->catalogSnapshot(WatchlistBacktestC07ParamGridCatalog::CATALOG_CODE);
            }
            $c14Before = [];
            ...
            $c16Before = [];
            if (! empty($definition['requires_c16_catalog'])) {
                $c16Before = $this->paramGrid->catalogSnapshot(WatchlistBacktestC16ParamGridCatalog::CATALOG_CODE);
            }
        ...
        if (! empty($definition['requires_c07_catalog'])
            && ((int) ($c07Before['catalog_count'] ?? 0) !== WatchlistBacktestC07ParamGridCatalog::CATALOG_COUNT
                || (string) ($c07Before['catalog_hash'] ?? '') !== WatchlistBacktestC07ParamGridCatalog::hash())) {
            return $this->blocked('WS_BT_R2_CATALOG_PERSISTED_SET_MISMATCH', 'Immutable C07 catalog count/hash is not intact before C14 calibration.');
        }
        if (! empty($definition['requires_c14_catalog'])
            && ((int) ($c14Before['catalog_count'] ?? 0) !== WatchlistBacktestC14ParamGridCatalog::CATALOG_COUNT
        ...
        if (! empty($definition['requires_c16_catalog'])
            && ((int) ($c16Before['catalog_count'] ?? 0) !== WatchlistBacktestC16ParamGridCatalog::CATALOG_COUNT
                || (string) ($c16Before['catalog_hash'] ?? '') !== WatchlistBacktestC16ParamGridCatalog::hash())) {
            return $this->blocked('WS_BT_R2_CATALOG_PERSISTED_SET_MISMATCH', 'Immutable C16 catalog count/hash is not intact before C17 calibration.');
        }
        ...
        try {
            ...
            $c07After = [];
            if (! empty($definition['requires_c07_catalog'])) {
                $c07After = $this->paramGrid->catalogSnapshot(WatchlistBacktestC07ParamGridCatalog::CATALOG_CODE);
            }
            $c14After = [];
            if (! empty($definition['requires_c14_catalog'])) {
            ...
            $c16After = [];
            if (! empty($definition['requires_c16_catalog'])) {
                $c16After = $this->paramGrid->catalogSnapshot(WatchlistBacktestC16ParamGridCatalog::CATALOG_CODE);
            }
        ...
        if (! empty($definition['requires_c07_catalog']) && $c07Before !== $c07After) {
            return $this->blocked('WS_BT_R2_CATALOG_IDENTITY_CONFLICT', 'C07 snapshot changed during C14 IS-only calibration.');
        }
        ...
        if (! empty($definition['requires_c16_catalog']) && $c16Before !== $c16After) {
            return $this->blocked('WS_BT_R2_CATALOG_IDENTITY_CONFLICT', 'C16 snapshot changed during C17 IS-only calibration.');
        }
    ...
    private function buildArtifact(
        ...
        if (! empty($definition['requires_c07_catalog'])) {
            $artifact['c07_immutability_proof'] = [
                'required' => true,
                'before' => $c07Before,
                'after' => $c07After,
                'equal' => $c07Before === $c07After,
            ];
            $artifact['validation']['c07_immutable'] = $c07Before === $c07After;
        }
        if (! empty($definition['requires_c14_catalog'])) {
            $artifact['c14_immutability_proof'] = [
                'required' => true,
                'before' => $c14Before,
                'after' => $c14After,
                'equal' => $c14Before === $c14After,
            ];
            $artifact['validation']['c14_immutable'] = $c14Before === $c14After;
        }
        if (! empty($definition['requires_c16_catalog'])) {
            $artifact['c16_immutability_proof'] = [
                'required' => true,
                'before' => $c16Before,
                'after' => $c16After,
                'equal' => $c16Before === $c16After,
            ];
            $artifact['validation']['c16_immutable'] = $c16Before === $c16After;
        }
    ...
    private function executionAxisArtifact(array $definition): array
    {
        $policy = is_array($definition['execution_axis_policy'] ?? null)
            ? (string) ($definition['execution_axis_policy']['policy'] ?? '')
            : WatchlistBacktestExitAxisSupport::POLICY_FIXED_EXECUTION;

        if ($policy === WatchlistBacktestExitAxisSupport::POLICY_VARIABLE_RISK_EXIT_AXIS) {
            $axisPolicy = $definition['execution_axis_policy'];

            return [
                'execution_axis_policy' => WatchlistBacktestExitAxisSupport::POLICY_VARIABLE_RISK_EXIT_AXIS,
                'variable_execution_axes' => [
                    'risk.stop_atr_mult' => is_array($axisPolicy['bounds'] ?? null) ? [
                        'min' => $axisPolicy['bounds']['stop_atr_mult_min'] ?? null,
                        'max' => $axisPolicy['bounds']['stop_atr_mult_max'] ?? null,
                    ] : [],
                    'risk.min_rr' => is_array($axisPolicy['bounds'] ?? null) ? [
                        'min' => $axisPolicy['bounds']['min_rr_min'] ?? null,
                        'max' => $axisPolicy['bounds']['min_rr_max'] ?? null,
                    ] : [],
                ],
                'fixed_grouping_axes' => [
                ...
                'blocked_first_phase_axes' => $axisPolicy['blocked_first_phase_axes'] ?? [],
                'contract_source' => $axisPolicy['contract_source'] ?? WatchlistBacktestExitAxisSupport::CONTRACT_SOURCE,
    ...
    private function catalogDefinition(string $catalogCode): array
    {
        $definitions = [
            ...
            WatchlistBacktestC14ParamGridCatalog::CATALOG_CODE => [
                'catalog_code' => WatchlistBacktestC14ParamGridCatalog::CATALOG_CODE,
                'catalog_version' => WatchlistBacktestC14ParamGridCatalog::CATALOG_VERSION,
                'catalog_count' => WatchlistBacktestC14ParamGridCatalog::CATALOG_COUNT,
                'catalog_hash' => WatchlistBacktestC14ParamGridCatalog::hash(),
                'parameter_axes' => WatchlistBacktestC14ParamGridCatalog::parameterAxes(),
                'axis_rationale' => WatchlistBacktestC14ParamGridCatalog::axisRationale(),
                'provenance' => WatchlistBacktestC14ParamGridCatalog::provenance(),
                'manifest_rows' => WatchlistBacktestC14ParamGridCatalog::manifestRows(),
                'reference_row_code' => WatchlistBacktestC14ParamGridCatalog::REFERENCE_ROW_CODE,
                ...
                'artifact_version' => 'WATCHLIST_C14_IS_CALIBRATION_V1',
                'artifact_scope' => 'WEEKLY_SWING_DOWNSIDE_STABILITY_C14_VARIABLE_RISK_EXIT_IS_ONLY',
                'failed_status' => 'C14_GRID_FAILED_IS_QUALITY',
                'requires_c01_catalog' => true,
                'requires_c02_catalog' => true,
                'requires_c03_catalog' => true,
                'requires_c04_catalog' => true,
            ...
            WatchlistBacktestC15ParamGridCatalog::CATALOG_CODE => [
                'catalog_code' => WatchlistBacktestC15ParamGridCatalog::CATALOG_CODE,
                'catalog_version' => WatchlistBacktestC15ParamGridCatalog::CATALOG_VERSION,
                'catalog_count' => WatchlistBacktestC15ParamGridCatalog::CATALOG_COUNT,
                'catalog_hash' => WatchlistBacktestC15ParamGridCatalog::hash(),
                'parameter_axes' => WatchlistBacktestC15ParamGridCatalog::parameterAxes(),
                'axis_rationale' => WatchlistBacktestC15ParamGridCatalog::axisRationale(),
                'provenance' => WatchlistBacktestC15ParamGridCatalog::provenance(),
                'manifest_rows' => WatchlistBacktestC15ParamGridCatalog::manifestRows(),
                'reference_row_code' => WatchlistBacktestC15ParamGridCatalog::REFERENCE_ROW_CODE,
                ...
                'artifact_version' => 'WATCHLIST_C15_IS_CALIBRATION_V1',
                'artifact_scope' => 'WEEKLY_SWING_DOWNSIDE_STABILITY_C15_CONTROLLED_PULLBACK_MID_LIQUIDITY_IS_ONLY',
                'failed_status' => 'C15_GRID_FAILED_IS_QUALITY',
                'requires_c01_catalog' => true,
                'requires_c02_catalog' => true,
                'requires_c03_catalog' => true,
                'requires_c04_catalog' => true,
            ...
            WatchlistBacktestC17ParamGridCatalog::CATALOG_CODE => [
                'catalog_code' => WatchlistBacktestC17ParamGridCatalog::CATALOG_CODE,
                'catalog_version' => WatchlistBacktestC17ParamGridCatalog::CATALOG_VERSION,
                'catalog_count' => WatchlistBacktestC17ParamGridCatalog::CATALOG_COUNT,
                'catalog_hash' => WatchlistBacktestC17ParamGridCatalog::hash(),
                'parameter_axes' => WatchlistBacktestC17ParamGridCatalog::parameterAxes(),
                'axis_rationale' => WatchlistBacktestC17ParamGridCatalog::axisRationale(),
                'provenance' => WatchlistBacktestC17ParamGridCatalog::provenance(),
                'manifest_rows' => WatchlistBacktestC17ParamGridCatalog::manifestRows(),
                'reference_row_code' => WatchlistBacktestC17ParamGridCatalog::REFERENCE_ROW_CODE,
                'fixed_top_picks_target' => WatchlistBacktestC17ParamGridCatalog::FIXED_TOP_PICKS_TARGET,
                'fixed_secondary_target' => WatchlistBacktestC17ParamGridCatalog::FIXED_SECONDARY_TARGET,
                'execution_axis_policy' => WatchlistBacktestC17ParamGridCatalog::exitAxisPolicy(),
                'artifact_version' => 'WATCHLIST_C17_IS_CALIBRATION_V1',
                'artifact_scope' => 'WEEKLY_SWING_DOWNSIDE_STABILITY_C17_QUALITY_PRESERVING_SAMPLE_RECOVERY_IS_ONLY',
                'failed_status' => 'C17_GRID_FAILED_IS_QUALITY',
                'requires_c01_catalog' => true,
                'requires_c02_catalog' => true,
                'requires_c03_catalog' => true,
                'requires_c04_catalog' => true,
