...
class WatchlistBacktestC139WeeklySwingWatchlistProductionLiveRuntimeActivationExecutionReviewService
{
    public const RUN_CODE = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW';
    public const PHASE_LABEL = 'PR-27 / C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW';
    public const ARTIFACT_TYPE = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW';

    public const DEFAULT_C138_ARTIFACT = 'storage/app/watchlist/backtest/c138-weekly-swing-watchlist-production-live-runtime-activation-authorization-review.json';
    public const DEFAULT_EXPECTED_C138_HASH = 'e3954d308b8540bbf7d10ce716848ee816383201';
    public const DEFAULT_EXPECTED_C138_FILE_SHA1 = '1FDC5A1BCF18AD32204FCACCDE6EFDD3747D28D0';
    public const DEFAULT_OUTPUT_PATH = 'storage/app/watchlist/backtest/c139-weekly-swing-watchlist-production-live-runtime-activation-execution-review.json';

    private const PRIMARY_CANDIDATE = 'C61_E02_B01_HYBRID_ALL_GUARDS_PRELOCK_CANDIDATE';
    private const BACKUP_CANDIDATE = 'C61_B01_A02_MARKET_SECTOR_DEFENSIVE_CONFIRMATION';
    private const COMPARATOR_CANDIDATE = 'C61_A01_B01_WEAK_REGIME_QUALITY_FIRST';

    private const EXPECTED_C138_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_PASSED_AUTHORIZED_PRIMARY_AND_BACKUP';
    private const EXPECTED_C138_PHASE_LABEL = 'PR-26 / C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW';
    private const EXPECTED_C138_NEXT_RECOMMENDATION = self::RUN_CODE;
    private const C140_RECOMMENDATION = 'C140_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW';
    ...
    private const TEMPORARY_NEGATIVE_REMAINS_STATUS = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_REJECTED_TEMPORARY_NEGATIVE_ARTIFACT_REMAINS';
    private const C138_LOCK_MISMATCH_STATUS = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_REJECTED_C138_ARTIFACT_LOCK_MISMATCH';
    private const C138_FILE_SHA1_MISMATCH_STATUS = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_REJECTED_C138_FILE_SHA1_LOCK_MISMATCH';
    private const C138_STATUS_MISMATCH_STATUS = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_REJECTED_C138_STATUS_MISMATCH';
    private const C138_PHASE_LABEL_MISMATCH_STATUS = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_REJECTED_C138_PHASE_LABEL_MISMATCH';
    private const C138_NEXT_RECOMMENDATION_MISMATCH_STATUS = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_REJECTED_C138_NEXT_RECOMMENDATION_MISMATCH';
    private const C138_AUTHORIZATION_INCOMPLETE_STATUS = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_REJECTED_C138_ACTIVATION_AUTHORIZATION_INCOMPLETE';
    private const CANDIDATE_SCOPE_MISMATCH_STATUS = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH';
    private const LIVE_OR_PRODUCTION_MUTATION_STATUS = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_REJECTED_LIVE_OR_PRODUCTION_MUTATION_ATTEMPT';
    private const C138_CONVERT_FROM_JSON_STATUS = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_REJECTED_C138_CONVERT_FROM_JSON_COMPATIBILITY_VIOLATION';
    private const FEATURE_FLAG_NOT_DEFAULT_OFF_STATUS = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_REJECTED_FEATURE_FLAGS_NOT_DEFAULT_OFF';

    private const REQUIRED_C138_TRUE_FIELDS = [
    ...
    private const REQUIRED_C138_FALSE_FIELDS = [
    ...
    public function execute(
        string $c138Artifact = self::DEFAULT_C138_ARTIFACT,
        string $expectedC138Hash = self::DEFAULT_EXPECTED_C138_HASH,
        string $expectedC138FileSha1 = self::DEFAULT_EXPECTED_C138_FILE_SHA1,
        string $outputPath = self::DEFAULT_OUTPUT_PATH,
        array $options = []
    ): array {
        $overwrite = (bool) ($options['overwrite'] ?? false);
        $artifact = $this->baseArtifact((string) ($options['created_at'] ?? gmdate('c')));
        $load = $this->loadArtifactLock($c138Artifact, $expectedC138Hash, $expectedC138FileSha1);
        $artifact['source_artifact_locks'] = $this->sourceArtifactLocks($load);
        $artifact = array_merge($artifact, $this->topLevelLockAliases($load));

        if (! $load['exists'] || ! is_array($load['payload'])) {
            return $this->blocked($artifact, self::C138_LOCK_MISMATCH_STATUS, 'C138 artifact missing or unreadable.', $outputPath, $overwrite);
        }
        if (! $load['convert_from_json_pass']) {
            $artifact = $this->completeSections($artifact, $load, $options, false);
            $artifact['c138_convert_from_json_duplicate_keys'] = $load['case_insensitive_duplicate_keys'];

            return $this->rejected($artifact, self::C138_CONVERT_FROM_JSON_STATUS, 'C138 artifact is not PowerShell ConvertFrom-Json compatible.', $outputPath, $overwrite);
        }
        if (! $load['hash_match']) {
            return $this->blocked($artifact, self::C138_LOCK_MISMATCH_STATUS, 'C138 artifact_hash mismatch.', $outputPath, $overwrite);
        }
        if (! $load['file_sha1_match']) {
            return $this->blocked($artifact, self::C138_FILE_SHA1_MISMATCH_STATUS, 'C138 file SHA1 mismatch.', $outputPath, $overwrite);
        }

        $c138 = $load['payload'];
        if (($c138['status'] ?? null) !== self::EXPECTED_C138_STATUS || ($c138['reason_code'] ?? null) !== self::EXPECTED_C138_STATUS) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::C138_STATUS_MISMATCH_STATUS, 'C138 status/reason is not activation-authorization passed.', $outputPath, $overwrite);
        }
        if (($c138['phase_label'] ?? null) !== self::EXPECTED_C138_PHASE_LABEL) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::C138_PHASE_LABEL_MISMATCH_STATUS, 'C138 phase label mismatch.', $outputPath, $overwrite);
        }
        if (! $this->c138NextRecommendationMatches($c138)) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::C138_NEXT_RECOMMENDATION_MISMATCH_STATUS, 'C138 next recommendation is not C139.', $outputPath, $overwrite);
        }
        if (! $this->c138ActivationAuthorizationComplete($c138)) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::C138_AUTHORIZATION_INCOMPLETE_STATUS, 'C138 production/live activation authorization evidence is incomplete.', $outputPath, $overwrite);
        }
        if (! $this->candidateScopeMatches($c138)) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::CANDIDATE_SCOPE_MISMATCH_STATUS, 'C138 candidate scope does not match locked activation execution scope.', $outputPath, $overwrite);
        }

        $safetyFailure = $this->firstLiveOrMutatingSafetyFlag($c138);
        if ($safetyFailure !== null) {
            $artifact = $this->completeSections($artifact, $load, $options, false);
            $artifact['c138_live_or_mutating_safety_flag_failure'] = $safetyFailure;

            return $this->rejected($artifact, self::LIVE_OR_PRODUCTION_MUTATION_STATUS, 'C138 contains live, mutating, production, or weekly-live flag set to true.', $outputPath, $overwrite);
        }
        if ($this->enabledRuntimeFeatureFlags() !== []) {
            $artifact = $this->completeSections($artifact, $load, $options, false);
            $artifact['enabled_runtime_feature_flags'] = $this->enabledRuntimeFeatureFlags();

            return $this->rejected($artifact, self::FEATURE_FLAG_NOT_DEFAULT_OFF_STATUS, 'C139 requires runtime feature flags to remain default-off.', $outputPath, $overwrite);
        }
        if (! (bool) ($options['operator_approved'] ?? false) || trim((string) ($options['approval_reference'] ?? '')) === '') {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::APPROVAL_MISSING_STATUS, 'C139 requires --operator-approved and non-empty --approval-reference.', $outputPath, $overwrite);
        }
        if (! (bool) ($options['production_live_runtime_activation_execution_confirmed'] ?? false)) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::ACTIVATION_EXECUTION_NOT_CONFIRMED_STATUS, 'C139 requires --production-live-runtime-activation-execution-confirmed.', $outputPath, $overwrite);
        }
        ...
        $artifact['message'] = 'C139 records production/live runtime activation execution review as artifact-only evidence and moves to observation review. It does not activate runtime bridge, execute live output, publish recommendations, or mutate PLAN/CONFIRM.';
        $artifact['diagnostic_conclusion'] = 'C139_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW_PASSED_ARTIFACT_ONLY_NON_LIVE_NON_MUTATING';
        $artifact['next_step_recommendation'] = self::C140_RECOMMENDATION;
    ...
    private function baseArtifact(string $createdAt): array
    {
        $artifact = [
            ...
            'phase_checkpoint' => 'PR-27',
            'internal_checkpoint' => 'C139',
            'status' => 'C139_NOT_RUN',
            'reason_code' => 'C139_NOT_RUN',
            ...
            'c138_lock_valid' => false,
            'c138_activation_authorization_valid' => false,
            'c138_convert_from_json_pass' => false,
    ...
    private function passingTopLevelState(): array
    {
        return [
            ...
            'production_live_runtime_activation_executed' => false,
            'c138_lock_valid' => true,
            'c138_activation_authorization_valid' => true,
            'c138_convert_from_json_pass' => true,
    ...
    {
        $c138 = is_array($load['payload'] ?? null) ? $load['payload'] : [];
        $temporaryNegativePaths = (array) ($options['temporary_negative_artifact_paths'] ?? []);

        $artifact['c138_lock_validation_summary'] = $this->c138LockValidationSummary($load, $c138);
        $artifact['c138_activation_authorization_carry_forward_summary'] = $this->c138ActivationAuthorizationCarryForwardSummary($c138, $pass);
        $artifact['candidate_scope_freeze_summary'] = $this->candidateScopeFreezeSummary($c138, $pass);
        ...
        $artifact['c139_execution_decision'] = $this->activationExecutionDecision($pass);
        ...
        $artifact['c139_candidate_activation_execution_scorecard'] = $this->candidateScorecard($pass);
        ...
        $artifact['failure_attribution_summary'] = $this->failureAttributionSummary($pass ? [] : [(string) ($artifact['status'] ?? 'C139_PENDING')]);
    ...
    private function c138ActivationAuthorizationComplete(array $c138): bool
    {
        foreach (self::REQUIRED_C138_TRUE_FIELDS as $field) {
            if (! (bool) ($c138[$field] ?? false)) {
                return false;
            }
        }
        foreach (self::REQUIRED_C138_FALSE_FIELDS as $field) {
            if ((bool) ($c138[$field] ?? false)) {
                return false;
            }
        }
    ...
    private function c138NextRecommendationMatches(array $c138): bool
    {
        foreach ([
            ['next_step_recommendation'],
            ['next_activation_execution_decision', 'next_recommendation'],
            ['c138_activation_authorization_decision', 'next_recommendation'],
            ['planned_next_summary', 'planned_next_review'],
        ] as $path) {
            $value = $this->valueAt($c138, $path);
            if ($value !== null && $value !== self::EXPECTED_C138_NEXT_RECOMMENDATION) {
                return false;
            }
        }

        return ($c138['next_step_recommendation'] ?? null) === self::EXPECTED_C138_NEXT_RECOMMENDATION;
    }
    ...
    private function c138LockValidationSummary(array $load, array $c138): array
    {
        return [
            ...
            'expected_status' => self::EXPECTED_C138_STATUS,
            'actual_status' => $c138['status'] ?? null,
            'expected_phase_label' => self::EXPECTED_C138_PHASE_LABEL,
            'actual_phase_label' => $c138['phase_label'] ?? null,
            'expected_next_recommendation' => self::EXPECTED_C138_NEXT_RECOMMENDATION,
            'next_recommendation_match' => $this->c138NextRecommendationMatches($c138),
            'c138_lock_valid' => $load['hash_match'] && $load['file_sha1_match'] && $load['convert_from_json_pass'],
        ];
    }

    private function c138ActivationAuthorizationCarryForwardSummary(array $c138, bool $pass): array
    {
        return [
            'validation_completed' => true,
            'c138_activation_authorization_review_pass' => (bool) ($c138['weekly_swing_watchlist_production_live_runtime_activation_authorization_review_pass'] ?? false),
            'c138_activation_authorization_confirmed' => (bool) ($c138['activation_authorization_confirmed'] ?? false),
            'c138_activation_authorized' => (bool) ($c138['activation_authorized'] ?? false),
            'c138_ready_for_activation_execution_review' => (bool) ($c138['ready_for_production_live_runtime_activation_execution_review'] ?? false),
            'c137_pre_activation_boundary_valid' => (bool) ($c138['c137_pre_activation_boundary_valid'] ?? false),
            'c136_go_decision_finalization_valid' => (bool) ($c138['c136_go_decision_finalization_valid'] ?? false),
            'c135_activation_operator_go_no_go_valid' => (bool) ($c138['c135_activation_operator_go_no_go_valid'] ?? false),
            'c134_activation_observation_result_review_valid' => (bool) ($c138['c134_activation_observation_result_review_valid'] ?? false),
            'c133_activation_observation_review_valid' => (bool) ($c138['c133_activation_observation_review_valid'] ?? false),
            'c132_activation_execution_review_valid' => (bool) ($c138['c132_activation_execution_review_valid'] ?? false),
            'c131_activation_approval_valid' => (bool) ($c138['c131_activation_approval_valid'] ?? false),
            'c130_activation_readiness_valid' => (bool) ($c138['c130_activation_readiness_valid'] ?? false),
            'c129_final_closure_valid' => (bool) ($c138['c129_final_closure_valid'] ?? false),
            'c138_activation_authorization_valid' => $this->c138ActivationAuthorizationComplete($c138),
            'c139_activation_execution_review_can_start' => $pass,
        ];
    }
    ...
    private function activationExecutionDecision(bool $pass): array
    {
        return [
            'review_pass' => $pass,
            'validation_completed' => true,
            'c138_activation_authorization_valid' => $pass,
            'weekly_swing_watchlist_production_live_runtime_activation_execution_review_executed' => true,
            ...
            'next_recommendation' => $pass ? self::C140_RECOMMENDATION : 'C139_TARGETED_C138_ACTIVATION_AUTHORIZATION_OR_CONFIRMATION_REPAIR',
            'decision_reason' => $pass ? 'C139 records artifact-only execution review and permits observation review next.' : 'C139 cannot proceed until C138 lock, activation authorization, execution confirmation, feature flags, and safety gates pass.',
        ];
    }
    ...
    private function nextObservationDecision(bool $pass): array
    {
        return [
            'review_pass' => $pass,
            'next_recommendation' => $pass ? self::C140_RECOMMENDATION : 'C139_TARGETED_C138_ACTIVATION_AUTHORIZATION_OR_CONFIRMATION_REPAIR',
            'next_scope' => $pass ? 'production/live runtime activation observation review only; still no runtime bridge activation, live output generation, official publication, or PLAN/CONFIRM mutation in C139' : 'targeted C138 authorization, execution confirmation, feature flag, or cleanup repair only',
        ];
    }
    ...
    private function activationExecutionManifest(bool $pass): array
    {
        return [
            ...
            'source_artifact' => self::EXPECTED_C138_STATUS,
            'source_artifact_path' => self::DEFAULT_C138_ARTIFACT,
            'source_artifact_hash' => self::DEFAULT_EXPECTED_C138_HASH,
            'source_file_sha1' => self::DEFAULT_EXPECTED_C138_FILE_SHA1,
            'primary_candidate_code' => self::PRIMARY_CANDIDATE,
            'backup_candidate_code' => self::BACKUP_CANDIDATE,
            'comparator_candidate_code' => self::COMPARATOR_CANDIDATE,
            'primary_candidate_role' => 'primary_production_live_runtime_activation_execution_review_candidate',
            'backup_candidate_role' => 'backup_production_live_runtime_activation_execution_review_candidate',
            'comparator_candidate_role' => 'comparator_only_not_promoted',
            'c138_activation_authorization_carried_forward' => $pass,
    ...
    {
        return [
            'c138_activation_authorization_artifact_locked' => true,
    ...
    private function candidateScorecard(bool $pass): array
    {
        ...
        return [
            array_merge($base, [
                ...
                'c139_role' => 'primary_production_live_runtime_activation_execution_review_candidate',
                'primary_candidate_ready_for_production_live_runtime_activation_observation_review' => $pass,
            ]),
            array_merge($base, [
                'candidate_code' => self::BACKUP_CANDIDATE,
                'c139_role' => 'backup_production_live_runtime_activation_execution_review_candidate',
                'backup_candidate_ready_for_production_live_runtime_activation_observation_review' => $pass,
            ]),
            array_merge($base, [
                'candidate_code' => self::COMPARATOR_CANDIDATE,
                'c139_role' => 'comparator_only_candidate',
                'production_live_runtime_activation_execution_review_pass' => false,
                'ready_for_production_live_runtime_activation_observation_review' => false,
                'comparator_candidate_ready_for_production_live_runtime_activation_observation_review' => false,
                'a01_remains_comparator_only' => true,
    ...
    {
        return [
            'progress_marker' => 'PR-27_C139_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW',
            'c138_activation_authorization_carried_forward' => true,
            'c139_production_live_runtime_activation_execution_review_executed' => true,
            'c139_ready_for_activation_observation_review' => $pass,
    ...
    {
        return [
            'planned_next_review' => $pass ? self::C140_RECOMMENDATION : 'C139_TARGETED_C138_ACTIVATION_AUTHORIZATION_OR_CONFIRMATION_REPAIR',
            'planned_next_scope' => $pass ? 'production/live runtime activation observation review only; not official weekly swing output publication or PLAN/CONFIRM mutation in C139' : 'targeted repair before production/live activation execution review can be recorded',
    ...
    private function documentationHygieneGuardSummary(array $load): array
    {
        return [
            ...
            'c138_convert_from_json_pass' => $load['convert_from_json_pass'],
            'top_level_case_insensitive_duplicate_keys' => $load['case_insensitive_duplicate_keys'],
            'c138_artifact_not_modified' => true,
            'c60_c138_artifacts_not_modified' => true,
            'c139_is_execution_review_not_live_state_change' => true,
            'phase_label' => self::PHASE_LABEL,
        ];
    }
    ...
    {
        return [
            'C139 validates C138 artifact_hash and file SHA1 locks before production/live runtime activation execution review is recorded.',
            'C139 validates C138 activation authorization and next recommendation to C139.',
            'C139 requires --operator-approved, a non-empty --approval-reference, and --production-live-runtime-activation-execution-confirmed.',
            'C139 keeps runtime feature flags default-off and checks the kill-switch surface.',
            'C139 keeps E02 primary, B01 backup, and A01 comparator-only.',
            'C139 records artifact-only activation execution review and moves to observation review.',
            'C139 does not activate runtime bridge, execute production runtime wiring, generate official weekly swing output, publish recommendations, or mutate PLAN/CONFIRM.',
            'C139 may only recommend C140 production/live runtime activation observation review as the next controlled step.',
        ];
    }
    ...
    {
        return [
            'expected_c138_hash' => $load['expected_hash'],
            'actual_c138_hash' => $load['actual_hash'],
            'c138_hash_match' => $load['hash_match'],
            'expected_c138_file_sha1' => $load['expected_file_sha1'],
            'actual_c138_file_sha1' => $load['actual_file_sha1'],
            'c138_file_sha1_match' => $load['file_sha1_match'],
            'c138_convert_from_json_pass' => $load['convert_from_json_pass'],
        ];
    }
