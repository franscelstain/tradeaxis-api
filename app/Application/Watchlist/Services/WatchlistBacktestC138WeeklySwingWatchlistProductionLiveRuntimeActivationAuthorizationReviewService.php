...
class WatchlistBacktestC138WeeklySwingWatchlistProductionLiveRuntimeActivationAuthorizationReviewService
{
    public const RUN_CODE = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW';
    public const PHASE_LABEL = 'PR-26 / C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW';
    public const ARTIFACT_TYPE = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW';

    public const DEFAULT_C137_ARTIFACT = 'storage/app/watchlist/backtest/c137-weekly-swing-watchlist-production-live-runtime-activation-pre-activation-boundary-review.json';
    public const DEFAULT_EXPECTED_C137_HASH = 'da4f273d8b60a5cc07e0950a59a8673ac9ad8e1d';
    public const DEFAULT_EXPECTED_C137_FILE_SHA1 = 'F1599D92D69EBC4AB820B61CB8C0F421A9C7EFB9';
    public const DEFAULT_OUTPUT_PATH = 'storage/app/watchlist/backtest/c138-weekly-swing-watchlist-production-live-runtime-activation-authorization-review.json';

    private const PRIMARY_CANDIDATE = 'C61_E02_B01_HYBRID_ALL_GUARDS_PRELOCK_CANDIDATE';
    private const BACKUP_CANDIDATE = 'C61_B01_A02_MARKET_SECTOR_DEFENSIVE_CONFIRMATION';
    private const COMPARATOR_CANDIDATE = 'C61_A01_B01_WEAK_REGIME_QUALITY_FIRST';

    private const EXPECTED_C137_STATUS = 'C137_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_PRE_ACTIVATION_BOUNDARY_REVIEW_PASSED_BOUNDARY_CLEARED_PRIMARY_AND_BACKUP';
    private const EXPECTED_C137_PHASE_LABEL = 'PR-25 / C137_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_PRE_ACTIVATION_BOUNDARY_REVIEW';
    private const EXPECTED_C137_NEXT_RECOMMENDATION = self::RUN_CODE;
    private const C139_RECOMMENDATION = 'C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW';
    ...
    private const TEMPORARY_NEGATIVE_REMAINS_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_TEMPORARY_NEGATIVE_ARTIFACT_REMAINS';
    private const C137_LOCK_MISMATCH_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C137_ARTIFACT_LOCK_MISMATCH';
    private const C137_FILE_SHA1_MISMATCH_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C137_FILE_SHA1_LOCK_MISMATCH';
    private const C137_STATUS_MISMATCH_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C137_STATUS_MISMATCH';
    private const C137_PHASE_LABEL_MISMATCH_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C137_PHASE_LABEL_MISMATCH';
    private const C137_NEXT_RECOMMENDATION_MISMATCH_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C137_NEXT_RECOMMENDATION_MISMATCH';
    private const C137_PRE_ACTIVATION_BOUNDARY_INVALID_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C137_PRE_ACTIVATION_BOUNDARY_INVALID';
    private const C137_CONVERT_FROM_JSON_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C137_CONVERT_FROM_JSON_COMPATIBILITY_VIOLATION';
    private const CANDIDATE_SCOPE_MISMATCH_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH';
    private const LIVE_OR_PRODUCTION_MUTATION_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_LIVE_OR_PRODUCTION_MUTATION_ATTEMPT';
    private const FEATURE_FLAG_NOT_DEFAULT_OFF_STATUS = 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_FEATURE_FLAGS_NOT_DEFAULT_OFF';

    private const REQUIRED_C137_TRUE_FIELDS = [
        ...
        'c137_pre_activation_boundary_review_only',
        'c137_not_activation_authorization',
        'c137_not_live_runtime_state_change',
    ...
    private const REQUIRED_C137_FALSE_FIELDS = [
    ...
    public function execute(
        string $c137Artifact = self::DEFAULT_C137_ARTIFACT,
        string $expectedC137Hash = self::DEFAULT_EXPECTED_C137_HASH,
        string $expectedC137FileSha1 = self::DEFAULT_EXPECTED_C137_FILE_SHA1,
        string $outputPath = self::DEFAULT_OUTPUT_PATH,
        array $options = []
    ): array {
        $overwrite = (bool) ($options['overwrite'] ?? false);
        $artifact = $this->baseArtifact((string) ($options['created_at'] ?? gmdate('c')));
        $load = $this->loadArtifactLock($c137Artifact, $expectedC137Hash, $expectedC137FileSha1);
        $artifact['source_artifact_locks'] = $this->sourceArtifactLocks($load);
        $artifact = array_merge($artifact, $this->topLevelLockAliases($load));

        if (! $load['exists'] || ! is_array($load['payload'])) {
            return $this->blocked($artifact, self::C137_LOCK_MISMATCH_STATUS, 'C137 artifact missing or unreadable.', $outputPath, $overwrite);
        }
        if (! $load['convert_from_json_pass']) {
            $artifact = $this->completeSections($artifact, $load, $options, false);
            $artifact['c137_convert_from_json_duplicate_keys'] = $load['case_insensitive_duplicate_keys'];

            return $this->rejected($artifact, self::C137_CONVERT_FROM_JSON_STATUS, 'C137 artifact is not PowerShell ConvertFrom-Json compatible.', $outputPath, $overwrite);
        }
        if (! $load['hash_match']) {
            return $this->blocked($artifact, self::C137_LOCK_MISMATCH_STATUS, 'C137 artifact_hash mismatch.', $outputPath, $overwrite);
        }
        if (! $load['file_sha1_match']) {
            return $this->blocked($artifact, self::C137_FILE_SHA1_MISMATCH_STATUS, 'C137 file SHA1 mismatch.', $outputPath, $overwrite);
        }

        $c137 = $load['payload'];
        if (($c137['status'] ?? null) !== self::EXPECTED_C137_STATUS || ($c137['reason_code'] ?? null) !== self::EXPECTED_C137_STATUS) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::C137_STATUS_MISMATCH_STATUS, 'C137 status/reason is not activation authorization ready.', $outputPath, $overwrite);
        }
        if (($c137['phase_label'] ?? null) !== self::EXPECTED_C137_PHASE_LABEL) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::C137_PHASE_LABEL_MISMATCH_STATUS, 'C137 phase label mismatch.', $outputPath, $overwrite);
        }
        if (! $this->c137NextRecommendationMatches($c137)) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::C137_NEXT_RECOMMENDATION_MISMATCH_STATUS, 'C137 next recommendation is not C138.', $outputPath, $overwrite);
        }
        if (! $this->c137PreActivationBoundaryValid($c137)) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::C137_PRE_ACTIVATION_BOUNDARY_INVALID_STATUS, 'C137 production/live activation pre-activation boundary evidence is incomplete.', $outputPath, $overwrite);
        }
        if (! $this->candidateScopeMatches($c137)) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::CANDIDATE_SCOPE_MISMATCH_STATUS, 'C137 candidate scope does not match locked activation authorization scope.', $outputPath, $overwrite);
        }

        $safetyFailure = $this->firstLiveOrMutatingSafetyFlag($c137);
        if ($safetyFailure !== null) {
            $artifact = $this->completeSections($artifact, $load, $options, false);
            $artifact['c137_live_or_mutating_safety_flag_failure'] = $safetyFailure;

            return $this->rejected($artifact, self::LIVE_OR_PRODUCTION_MUTATION_STATUS, 'C137 contains live, mutating, production, or weekly-live flag set to true.', $outputPath, $overwrite);
        }
        if ($this->enabledRuntimeFeatureFlags() !== []) {
            $artifact = $this->completeSections($artifact, $load, $options, false);
            $artifact['enabled_runtime_feature_flags'] = $this->enabledRuntimeFeatureFlags();

            return $this->rejected($artifact, self::FEATURE_FLAG_NOT_DEFAULT_OFF_STATUS, 'C138 requires runtime feature flags to remain default-off.', $outputPath, $overwrite);
        }
        if (! (bool) ($options['operator_approved'] ?? false) || trim((string) ($options['approval_reference'] ?? '')) === '') {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::APPROVAL_MISSING_STATUS, 'C138 requires --operator-approved and non-empty --approval-reference.', $outputPath, $overwrite);
        }
        if (($options['activation_authorization_confirmed'] ?? true) !== true) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), self::AUTHORIZATION_NOT_CONFIRMED_STATUS, 'C138 requires explicit activation authorization confirmation.', $outputPath, $overwrite);
        }
        ...
        $artifact['message'] = 'C138 authorizes production/live runtime activation for E02 primary and B01 backup as artifact-only evidence. This still does not execute activation, activate runtime bridge, produce live output, publish recommendations, or mutate PLAN/CONFIRM.';
        $artifact['diagnostic_conclusion'] = 'READY_FOR_C139_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW';
        $artifact['next_step_recommendation'] = self::C139_RECOMMENDATION;
    ...
    private function baseArtifact(string $createdAt): array
    {
        return array_merge(array_fill_keys(self::REQUIRED_FALSE_SAFETY_FLAGS, false), [
            ...
            'phase_checkpoint' => 'PR-26',
            'internal_checkpoint' => 'C138',
            'status' => 'C138_NOT_RUN',
            'reason_code' => 'C138_NOT_RUN',
            ...
            'c137_lock_valid' => false,
            'c137_pre_activation_boundary_valid' => false,
            'c137_convert_from_json_pass' => false,
    ...
    private function passingTopLevelState(): array
    {
        return [
            ...
            'production_live_runtime_activation_executed' => false,
            'c137_lock_valid' => true,
            'c137_pre_activation_boundary_valid' => true,
            'c137_convert_from_json_pass' => true,
    ...
    {
        $c137 = is_array($load['payload']) ? $load['payload'] : [];
        $temporaryNegativePaths = (array) ($options['temporary_negative_artifact_paths'] ?? []);

        return array_merge($artifact, [
            'c137_lock_validation_summary' => $this->c137LockValidationSummary($load),
            'c137_pre_activation_boundary_carry_forward_summary' => $this->c137PreActivationBoundaryCarryForwardSummary($c137),
            'candidate_scope_freeze_summary' => $this->candidateScopeFreezeSummary($c137, $pass),
            ...
            'c138_activation_authorization_decision' => $this->activationAuthorizationDecision($pass),
            ...
            'c138_candidate_activation_execution_readiness_scorecard' => $this->candidateScorecard($pass),
    ...
    private function c137LockValidationSummary(array $load): array
    {
        $c137 = is_array($load['payload']) ? $load['payload'] : [];
        ...
        return [
            ...
            'expected_status' => self::EXPECTED_C137_STATUS,
            'actual_status' => $c137['status'] ?? null,
            'expected_phase_label' => self::EXPECTED_C137_PHASE_LABEL,
            'actual_phase_label' => $c137['phase_label'] ?? null,
            'expected_next_recommendation' => self::EXPECTED_C137_NEXT_RECOMMENDATION,
            'next_recommendation_match' => $this->c137NextRecommendationMatches($c137),
            'c137_lock_valid' => $load['hash_match'] && $load['file_sha1_match'] && $load['convert_from_json_pass'],
        ];
    }

    private function c137PreActivationBoundaryCarryForwardSummary(array $c137): array
    {
        return [
            'validation_completed' => true,
            'c137_pre_activation_boundary_review_pass' => (bool) ($c137['weekly_swing_watchlist_production_live_runtime_activation_pre_activation_boundary_review_pass'] ?? false),
            'c137_pre_activation_boundary_cleared' => (bool) ($c137['pre_activation_boundary_cleared'] ?? false),
            'c137_ready_for_activation_authorization_review' => (bool) ($c137['ready_for_production_live_runtime_activation_authorization_review'] ?? false),
            'c136_go_decision_finalization_valid' => (bool) ($c137['c136_go_decision_finalization_valid'] ?? false),
            'c135_activation_operator_go_no_go_valid' => (bool) ($c137['c135_activation_operator_go_no_go_valid'] ?? false),
            'c134_activation_observation_result_review_valid' => (bool) ($c137['c134_activation_observation_result_review_valid'] ?? false),
            'c133_activation_observation_review_valid' => (bool) ($c137['c133_activation_observation_review_valid'] ?? false),
            'c132_activation_execution_review_valid' => (bool) ($c137['c132_activation_execution_review_valid'] ?? false),
            'c131_activation_approval_valid' => (bool) ($c137['c131_activation_approval_valid'] ?? false),
            'c130_activation_readiness_valid' => (bool) ($c137['c130_activation_readiness_valid'] ?? false),
            'c129_final_closure_valid' => (bool) ($c137['c129_final_closure_valid'] ?? false),
            'c137_pre_activation_boundary_valid' => $this->c137PreActivationBoundaryValid($c137),
            'c138_activation_authorization_can_start' => $this->c137PreActivationBoundaryValid($c137),
        ];
    }

    private function candidateScopeFreezeSummary(array $c137, bool $pass): array
    {
        return [
            'candidate_scope_locked' => true,
            'candidate_scope_valid' => $this->candidateScopeMatches($c137),
            'primary_candidate_code' => self::PRIMARY_CANDIDATE,
            'backup_candidate_code' => self::BACKUP_CANDIDATE,
            'comparator_candidate_code' => self::COMPARATOR_CANDIDATE,
    ...
    private function activationAuthorizationDecision(bool $pass): array
    {
        return [
            ...
            'next_recommendation' => $pass ? self::C139_RECOMMENDATION : 'C138_TARGETED_C137_PRE_ACTIVATION_BOUNDARY_REPAIR',
            'decision_reason' => $pass ? 'C138 authorizes activation as artifact-only evidence and permits activation execution review next.' : 'C138 cannot proceed until C137 lock, approval, authorization confirmation, cleanup, candidate, and safety gates pass.',
        ];
    }
    ...
    private function nextActivationExecutionDecision(bool $pass): array
    {
        return [
            'review_pass' => $pass,
            'next_recommendation' => $pass ? self::C139_RECOMMENDATION : 'C138_TARGETED_C137_PRE_ACTIVATION_BOUNDARY_REPAIR',
            'next_scope' => $pass ? 'production/live runtime activation execution review only; C138 itself still does not execute activation, activate runtime bridge, generate live output, publish recommendations, or mutate PLAN/CONFIRM' : 'targeted C137 pre-activation boundary lock, approval, or safety repair only',
        ];
    }
    ...
    private function activationAuthorizationManifest(array $load, bool $pass): array
    {
        return [
            ...
            'source_artifact' => 'C137_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_PRE_ACTIVATION_BOUNDARY_REVIEW',
    ...
    private function activationAuthorizationChecklist(): array
    {
        return [
            ...
            'c137_source_lock_reviewed' => true,
    ...
    private function candidateScorecard(bool $pass): array
    {
        ...
        return [
            array_merge($base, [
                ...
                'c138_role' => 'primary_production_live_runtime_activation_authorized_candidate',
                'primary_candidate_ready_for_production_live_runtime_activation_execution_review' => $pass,
            ]),
            array_merge($base, [
                'candidate_code' => self::BACKUP_CANDIDATE,
                'c138_role' => 'backup_production_live_runtime_activation_authorized_candidate',
                'backup_candidate_ready_for_production_live_runtime_activation_execution_review' => $pass,
            ]),
            array_merge($base, [
                'candidate_code' => self::COMPARATOR_CANDIDATE,
                'c138_role' => 'comparator_only_candidate',
                'production_live_runtime_activation_authorization_review_pass' => false,
                'activation_authorized' => false,
                'ready_for_production_live_runtime_activation_execution_review' => false,
                'comparator_candidate_ready_for_production_live_runtime_activation_execution_review' => false,
    ...
    private function runtimeConfigReviewSummary(): array
    {
        return [
            ...
            'production_config_default_unchanged' => true,
            'c137_artifact_identified' => is_file(self::DEFAULT_C137_ARTIFACT),
            'c138_service_identified' => is_file('app/Application/Watchlist/Services/WatchlistBacktestC138WeeklySwingWatchlistProductionLiveRuntimeActivationAuthorizationReviewService.php'),
            'c138_command_identified' => is_file('app/Console/Commands/Watchlist/RunBacktestC138WeeklySwingWatchlistProductionLiveRuntimeActivationAuthorizationReviewCommand.php'),
    ...
    {
        return [
            'progress_marker' => 'PR-26_C138_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW',
            'c137_pre_activation_boundary_review_carried_forward' => true,
            'c138_production_live_runtime_activation_authorization_review_executed' => true,
            'c138_activation_authorized' => $pass,
            'c138_ready_for_activation_execution_review' => $pass,
            'production_live_runtime_activation_executed' => false,
    ...
    {
        return [
            'planned_next_review' => $pass ? self::C139_RECOMMENDATION : 'C138_TARGETED_C137_PRE_ACTIVATION_BOUNDARY_REPAIR',
            'planned_next_scope' => $pass ? 'production/live runtime activation execution review only; not official weekly swing output publication or PLAN/CONFIRM mutation in C138' : 'targeted repair before production/live activation can be authorized',
            'planned_next_required_inputs' => $pass ? [
                'locked C138 artifact hash',
                'locked C138 file SHA1',
    ...
    private function documentationHygieneGuardSummary(array $load): array
    {
        return [
            ...
            'c137_convert_from_json_pass' => $load['convert_from_json_pass'],
            'top_level_case_insensitive_duplicate_keys' => $load['case_insensitive_duplicate_keys'],
            'c137_artifact_not_modified' => true,
            'c60_c137_artifacts_not_modified' => true,
            'c138_is_activation_authorization_review_not_activation_execution' => true,
            'phase_label' => self::PHASE_LABEL,
        ];
    }
    ...
    {
        return [
            'C138 validates C137 artifact_hash and file SHA1 locks before production/live runtime activation authorization review is recorded.',
            'C138 validates C137 pre-activation boundary clearance and next recommendation to C138.',
            'C138 requires --operator-approved, a non-empty --approval-reference, and explicit activation authorization confirmation.',
            'C138 keeps runtime feature flags default-off and checks the kill-switch surface.',
            'C138 keeps E02 primary, B01 backup, and A01 comparator-only.',
            'C138 records artifact-only activation authorization and moves to activation execution review.',
            'C138 does not execute activation, activate runtime bridge, execute production runtime wiring, generate official weekly swing output, publish recommendations, or mutate PLAN/CONFIRM.',
            'C138 may only recommend C139 production/live runtime activation execution review as the next controlled step.',
        ];
    }

    private function c137PreActivationBoundaryValid(array $c137): bool
    {
        foreach (self::REQUIRED_C137_TRUE_FIELDS as $field) {
            if (($c137[$field] ?? null) !== true) {
                return false;
            }
        }
        foreach (self::REQUIRED_C137_FALSE_FIELDS as $field) {
            if (($c137[$field] ?? null) !== false) {
                return false;
            }
        }

        foreach ([
            [['c137_pre_activation_boundary_decision', 'review_pass'], true],
            [['c137_pre_activation_boundary_decision', 'next_recommendation'], self::EXPECTED_C137_NEXT_RECOMMENDATION],
            [['next_activation_authorization_decision', 'review_pass'], true],
            [['next_activation_authorization_decision', 'next_recommendation'], self::EXPECTED_C137_NEXT_RECOMMENDATION],
            ...
            if ($this->valueAt($c137, $path) !== $expected) {
                return false;
            }
        }
    ...
    private function c137NextRecommendationMatches(array $c137): bool
    {
        foreach ([
            $c137['next_step_recommendation'] ?? null,
            $c137['next_activation_authorization_decision']['next_recommendation'] ?? null,
            $c137['planned_next_summary']['planned_next_review'] ?? null,
        ] as $value) {
            if ($value !== null && $value !== self::EXPECTED_C137_NEXT_RECOMMENDATION) {
                return false;
            }
        }

        return ($c137['next_step_recommendation'] ?? null) === self::EXPECTED_C137_NEXT_RECOMMENDATION;
    }
    ...
    {
        foreach (array_values(array_unique(array_merge(self::REQUIRED_C137_FALSE_FIELDS, self::REQUIRED_FALSE_SAFETY_FLAGS))) as $field) {
    ...
    {
        return [
            'expected_c137_hash' => $load['expected_hash'],
            'actual_c137_hash' => $load['actual_hash'],
            'c137_hash_match' => $load['hash_match'],
            'expected_c137_file_sha1' => $load['expected_file_sha1'],
            'actual_c137_file_sha1' => $load['actual_file_sha1'],
            'c137_file_sha1_match' => $load['file_sha1_match'],
            'c137_convert_from_json_pass' => $load['convert_from_json_pass'],
        ];
    }
