...
class WeeklySwingTailRiskS01RemediationDraftService
{
    public const RUN_CODE = 'WS_TAIL_RISK_S01_SINGLE_ALLOWED_REMEDIATION_DRAFT';
    public const APPROVAL_REFERENCE = 'WS_TAIL_RISK_S01_OPERATOR_APPROVED_SINGLE_REMEDIATION_DRAFT';
    public const SOURCE_PARAM_SET_ID = 20;
    public const SOURCE_BT_PARAM_ID = 174;
    public const SOURCE_EVAL_ID = 212;
    public const SOURCE_PARAMS_HASH = 'dde1467e62dd6c586ad7234479d5ae23794a759a';
    public const SOURCE_OFFICIAL_IS_ARTIFACT_HASH = '0e1823c01c48f64072f9d07347cad0e7f304c4d1';
    public const SOURCE_OFFICIAL_IS_FILE_SHA1 = '6a501f8851aa2539db2af72db90cfdb2109f4ab8';
    ...
    public function execute(
        ...
        if (! $operatorApproved || $approvalReference !== self::APPROVAL_REFERENCE) {
            return $this->blocked('WS_TAIL_RISK_S01_REMEDIATION_OPERATOR_APPROVAL_MISSING');
        }
        foreach (['watchlist_param_sets', 'watchlist_bt_eval', 'watchlist_bt_param_grid'] as $table) {
            if (! Schema::hasTable($table)) {
                return $this->blocked('WS_TAIL_RISK_S01_REMEDIATION_SCHEMA_NOT_READY');
            }
        }
        $sourceArtifact = $this->verifySourceArtifact($sourceArtifactPath);
        if (! ($sourceArtifact['valid'] ?? false)) {
            return $this->blocked('WS_TAIL_RISK_S01_REMEDIATION_SOURCE_ARTIFACT_INVALID');
        }
        ...
        if (! $sourceDraft || ! $sourceEval
            ...
            return $this->blocked('WS_TAIL_RISK_S01_REMEDIATION_SOURCE_IDENTITY_INVALID');
        }
        $sourcePayload = json_decode((string) $sourceDraft->params_json, true);
        if (! is_array($sourcePayload)) {
            return $this->blocked('WS_TAIL_RISK_S01_REMEDIATION_SOURCE_PARAMSET_INVALID');
        }
        $sourceValidation = $this->validator->validate($sourcePayload);
        if (! ($sourceValidation['valid'] ?? false)
            || ($sourceValidation['canonical_hash'] ?? '') !== self::SOURCE_PARAMS_HASH) {
            return $this->blocked('WS_TAIL_RISK_S01_REMEDIATION_SOURCE_PARAMSET_INVALID');
        }
        ...
        try {
            ...
            if (count($rows) !== 1) {
                throw new \RuntimeException('WS_TAIL_RISK_S01_REMEDIATION_CATALOG_COUNT_MISMATCH');
            }
            $row = $rows[0];
            $payload = $sourceValidation['canonical_payload'];
            $payload['paramset_code'] =
                WatchlistBacktestTailRiskS01RemediationParamGridCatalog::ROW_CODE;
            ...
            $provenance = [
                ...
                'selection_changed_from_h1' => false,
                'loss_threshold_source' => 'UNCHANGED_CANONICAL_MONTHLY_AVERAGE_FLOOR',
            ...
            if (($import['status'] ?? '') !== 'DRAFT_PERSISTED') {
                throw new \RuntimeException(
                    'WS_TAIL_RISK_S01_REMEDIATION_DRAFT_PERSISTENCE_FAILED: '
            ...
            return $this->blocked('WS_TAIL_RISK_S01_REMEDIATION_PERSISTENCE_FAILED', [
                'error' => $exception->getMessage(),
            ]);
        }
        $after = $this->forbiddenBoundaryCounts();
        if ($before !== $after) {
            return $this->blocked('WS_TAIL_RISK_S01_REMEDIATION_FORBIDDEN_MUTATION');
        }

        $result = [
            'schema_version' => 'WS_TAIL_RISK_S01_REMEDIATION_DRAFT_V1',
            'run_code' => self::RUN_CODE,
            'phase_label' => self::RUN_CODE,
            'status' => 'WS_TAIL_RISK_S01_SINGLE_REMEDIATION_DRAFT_PERSISTED',
            'reason_code' => 'WS_TAIL_RISK_S01_FINAL_BOUNDED_REMEDIATION_LOCKED',
            'approval_reference' => $approvalReference,
            'separate_new_strategy_scope' => true,
            'r02_reopened' => false,
            'single_allowed_remediation' => true,
            ...
            'next_recommendation' =>
                'WS_TAIL_RISK_S01_RUN_SINGLE_REMEDIATION_OFFICIAL_IS_THEN_CLOSE_S01_IF_FAILED',
        ];
    ...
    private function verifySourceArtifact(string $path): array
    {
        ...
        return ['valid' => $this->identity->stableHash($payload)
            === self::SOURCE_OFFICIAL_IS_ARTIFACT_HASH];
    }
    ...
    private function forbiddenBoundaryCounts(): array
    {
        return [
            ...
            'active_paramsets' => DB::table('watchlist_param_sets')
                ->where('policy_code', 'WS')->where('status', 'ACTIVE')->count(),
    ...
    private function writeJson(string $path, array $payload, bool $overwrite): array
    {
        ...
        if (! is_dir($directory) && ! mkdir($directory, 0775, true) && ! is_dir($directory)) {
            throw new \RuntimeException('WS_TAIL_RISK_S01_REMEDIATION_OUTPUT_DIRECTORY_FAILED');
        }
        if (is_file($path)) {
            ...
            if (! $overwrite) {
                throw new \RuntimeException('WS_TAIL_RISK_S01_REMEDIATION_OUTPUT_EXISTS');
            }
        }
        ...
        if (file_put_contents($temporary, $json, LOCK_EX) === false
            ...
            @unlink($temporary);
            throw new \RuntimeException('WS_TAIL_RISK_S01_REMEDIATION_OUTPUT_WRITE_FAILED');
        }
