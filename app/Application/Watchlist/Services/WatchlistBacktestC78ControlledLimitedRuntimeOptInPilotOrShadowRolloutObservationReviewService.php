...
class WatchlistBacktestC78ControlledLimitedRuntimeOptInPilotOrShadowRolloutObservationReviewService
{
    public const RUN_CODE = 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW';
    public const ARTIFACT_TYPE = 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW';

    public const DEFAULT_C77_ARTIFACT = 'storage/app/watchlist/backtest/c77-controlled-runtime-opt-in-pilot-or-shadow-rollout-execution-review.json';
    public const DEFAULT_EXPECTED_C77_HASH = 'd827547d6d40a73785d4c2409b2913f60db42115';
    public const DEFAULT_EXPECTED_C77_FILE_SHA1 = '8C296276DD4D278206366953F975AFD5F7E328DE';
    public const DEFAULT_OUTPUT_PATH = 'storage/app/watchlist/backtest/c78-controlled-limited-runtime-opt-in-pilot-or-shadow-rollout-observation-review.json';
    ...
    private const EXPECTED_C77_STATUS = 'C77_CONTROLLED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_EXECUTION_REVIEW_PASSED_PRIMARY_AND_BACKUP';
    private const EXPECTED_C77_REASON = 'C77_CONTROLLED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_EXECUTION_REVIEW_PASSED_PRIMARY_AND_BACKUP';
    private const EXPECTED_C77_RECOMMENDATION = 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW';
    private const C79_RECOMMENDATION = 'C79_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_RESULT_REVIEW';
    ...
    private const RUNTIME_PATHS = [
        ...
        'c77_execution_service' => 'app/Application/Watchlist/Services/WatchlistBacktestC77ControlledRuntimeOptInPilotOrShadowRolloutExecutionReviewService.php',
     ...
     * NOT_RUNTIME_BRIDGE_ACTIVATION. C77_ARTIFACT_HASH_LOCK. C77_FILE_SHA1_LOCK.
     * C77_READINESS_NESTED_PATH_VALIDATED. C77_TO_C60_LINEAGE_LOCK.
    ...
    public function execute(
        string $c77Artifact = self::DEFAULT_C77_ARTIFACT,
        string $expectedC77Hash = self::DEFAULT_EXPECTED_C77_HASH,
        string $expectedC77FileSha1 = self::DEFAULT_EXPECTED_C77_FILE_SHA1,
        ...
        $load = $this->loadArtifactLock($c77Artifact, $expectedC77Hash, $expectedC77FileSha1);
        $artifact['source_artifact_locks'] = $this->sourceArtifactLocks($load);
        $artifact = array_merge($artifact, $this->topLevelLockAliases($load));

        if (! $load['exists'] || ! is_array($load['payload'])) {
            return $this->blocked($artifact, 'C78_BLOCKED_C77_ARTIFACT_LOCK_MISMATCH', 'C77 artifact missing or unreadable.', $outputPath, $overwrite);
        }
        if (! $load['hash_match']) {
            return $this->blocked($artifact, 'C78_BLOCKED_C77_ARTIFACT_LOCK_MISMATCH', 'C77 artifact_hash mismatch.', $outputPath, $overwrite);
        }
        if (! $load['file_sha1_match']) {
            return $this->blocked($artifact, 'C78_BLOCKED_C77_FILE_SHA1_LOCK_MISMATCH', 'C77 file SHA1 mismatch.', $outputPath, $overwrite);
        }

        $c77 = $load['payload'];
        if (($c77['status'] ?? null) !== self::EXPECTED_C77_STATUS || ($c77['reason_code'] ?? null) !== self::EXPECTED_C77_REASON) {
            return $this->blocked($artifact, 'C78_BLOCKED_C77_STATUS_OR_REASON_MISMATCH', 'C77 status/reason mismatch.', $outputPath, $overwrite);
        }
        if (($c77['controlled_runtime_opt_in_pilot_execution_review_pass'] ?? null) !== true) {
            return $this->blocked($artifact, 'C78_BLOCKED_C77_CONTROLLED_PILOT_EXECUTION_NOT_PASSED', 'C77 controlled pilot execution review did not pass.', $outputPath, $overwrite);
        }
        if (($c77['controlled_shadow_rollout_execution_review_pass'] ?? null) !== true) {
            return $this->blocked($artifact, 'C78_BLOCKED_C77_CONTROLLED_SHADOW_EXECUTION_NOT_PASSED', 'C77 controlled shadow execution review did not pass.', $outputPath, $overwrite);
        }
        if (($c77['next_readiness_decision']['candidate_ready_for_controlled_limited_runtime_observation_review_count'] ?? null) !== 2) {
            return $this->blocked($artifact, 'C78_BLOCKED_C77_C78_READINESS_COUNT_MISMATCH', 'C77 nested C78 readiness count mismatch.', $outputPath, $overwrite);
        }
        if (($c77['next_readiness_decision']['next_recommendation'] ?? null) !== self::EXPECTED_C77_RECOMMENDATION) {
            return $this->blocked($artifact, 'C78_BLOCKED_C77_RECOMMENDATION_MISMATCH', 'C77 nested C78 recommendation mismatch.', $outputPath, $overwrite);
        }
        foreach ($this->c77SafetyGateMap() as $field => $status) {
            if (($c77[$field] ?? null) !== false) {
                return $this->blocked($artifact, $status, 'C77 safety field '.$field.' is not false.', $outputPath, $overwrite);
            }
        }
        if (! $this->lineageLocksMatch($c77)) {
            return $this->blocked($artifact, 'C78_BLOCKED_LINEAGE_LOCK_MISMATCH', 'C77 to C60 lineage lock mismatch.', $outputPath, $overwrite);
        }
        if (! $this->candidateScopeMatches($c77)) {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH', 'C77 candidate scope does not match C77 freeze.', $outputPath, $overwrite);
        }

        if (! (bool) ($options['operator_approved'] ?? false) || trim((string) ($options['approval_reference'] ?? '')) === '') {
            return $this->rejected($this->completeSections($artifact, $load, $options, false), 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_OPERATOR_APPROVAL_MISSING', 'C78 requires --operator-approved and non-empty --approval-reference.', $outputPath, $overwrite);
        }
        ...
        if ($gateFailures !== []) {
            ...
            if ($status === 'C78_BLOCKED_DICTIONARY_COVERAGE_MISSING') {
                return $this->blocked($artifact, $status, 'C78 database dictionary coverage is missing.', $outputPath, $overwrite);
            }
            return $this->rejected($artifact, $status, 'C78 controlled limited pilot/shadow observation review gate failed.', $outputPath, $overwrite);
        }

        $artifact = $this->completeSections($artifact, $load, $options, true);
        $artifact['status'] = 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_PASSED_PRIMARY_AND_BACKUP';
        $artifact['reason_code'] = $artifact['status'];
        $artifact['message'] = 'C78 controlled limited runtime opt-in pilot / shadow rollout observation review passed for primary and backup. This is observation-review-only and does not deploy live.';
        $artifact['diagnostic_conclusion'] = 'READY_FOR_C79_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_RESULT_REVIEW';
        $artifact['next_step_recommendation'] = self::C79_RECOMMENDATION;
        $artifact['controlled_limited_runtime_opt_in_pilot_observation_review_executed'] = true;
        $artifact['controlled_limited_runtime_opt_in_pilot_observation_review_allowed'] = true;
        $artifact['controlled_limited_runtime_opt_in_pilot_observation_review_pass'] = true;
        $artifact['controlled_limited_shadow_rollout_observation_review_executed'] = true;
        $artifact['controlled_limited_shadow_rollout_observation_review_allowed'] = true;
        $artifact['controlled_limited_shadow_rollout_observation_review_pass'] = true;
    ...
    private function baseArtifact(string $createdAt): array
    {
        return [
            ...
            'status' => 'C78_NOT_RUN',
            'reason_code' => 'C78_NOT_RUN',
            'artifact_type' => self::ARTIFACT_TYPE,
            'artifact_hash' => null,
            'artifact_hash_algorithm' => 'stable_sha1_json_payload',
            'created_at' => $createdAt,
            'controlled_limited_runtime_opt_in_pilot_observation_review_executed' => false,
            'controlled_limited_runtime_opt_in_pilot_observation_review_allowed' => false,
            'controlled_limited_runtime_opt_in_pilot_observation_review_pass' => false,
            'controlled_limited_shadow_rollout_observation_review_executed' => false,
            'controlled_limited_shadow_rollout_observation_review_allowed' => false,
            'controlled_limited_shadow_rollout_observation_review_pass' => false,
            ...
            'controlled_limited_pilot_observation_context_persisted_to_live_runtime' => false,
            'controlled_limited_shadow_observation_context_persisted_to_live_runtime' => false,
    ...
    {
        $c77 = is_array($load['payload'] ?? null) ? $load['payload'] : [];
        $locks = is_array($c77['source_artifact_locks'] ?? null) ? $c77['source_artifact_locks'] : [];
        return [
            'c77_artifact_path' => $load['path'],
            'expected_c77_hash' => $load['expected_hash'],
            'actual_c77_hash' => $load['actual_hash'],
            'c77_hash_match' => $load['hash_match'],
            'expected_c77_file_sha1' => $load['expected_file_sha1'],
            'actual_c77_file_sha1' => $load['actual_file_sha1'],
            'c77_file_sha1_match' => $load['file_sha1_match'],
            'c77_source_lineage_checked' => true,
            'c77_source_lineage_match' => $this->lineageLocksMatch($c77),
            'c76_artifact_hash_from_c77' => (string) ($locks['actual_c76_hash'] ?? ''),
            'c76_file_sha1_from_c77' => (string) ($locks['actual_c76_file_sha1'] ?? ''),
    ...
    private function c77SafetyGateMap(): array
    {
        return [
            'production_catalog_runtime_wired' => 'C78_BLOCKED_C77_RUNTIME_ALREADY_WIRED',
            'controlled_opt_in_runtime_bridge_active' => 'C78_BLOCKED_C77_CONTROLLED_OPT_IN_BRIDGE_ALREADY_ACTIVE',
            'controlled_parallel_run_active' => 'C78_BLOCKED_C77_CONTROLLED_PARALLEL_RUN_ALREADY_ACTIVE',
            'controlled_rollout_active' => 'C78_BLOCKED_C77_CONTROLLED_ROLLOUT_ALREADY_ACTIVE',
            'controlled_pilot_execution_context_persisted_to_live_runtime' => 'C78_BLOCKED_C77_CONTROLLED_PILOT_CONTEXT_ALREADY_PERSISTED_TO_LIVE_RUNTIME',
            'controlled_shadow_execution_context_persisted_to_live_runtime' => 'C78_BLOCKED_C77_CONTROLLED_SHADOW_CONTEXT_ALREADY_PERSISTED_TO_LIVE_RUNTIME',
            'production_deployment_allowed' => 'C78_BLOCKED_C77_PRODUCTION_DEPLOYMENT_ALREADY_ALLOWED',
            'production_deployment_executed' => 'C78_BLOCKED_C77_DEPLOYMENT_ALREADY_EXECUTED',
            'plan_confirm_mutation_allowed' => 'C78_BLOCKED_C77_PLAN_CONFIRM_MUTATION_ALREADY_ALLOWED',
            'plan_confirm_mutated' => 'C78_BLOCKED_C77_PLAN_CONFIRM_ALREADY_MUTATED',
            'plan_confirm_runtime_reads_activated_catalog' => 'C78_BLOCKED_C77_PLAN_CONFIRM_ALREADY_READING_ACTIVATED_CATALOG',
            'live_plan_confirm_rollout_allowed' => 'C78_BLOCKED_C77_LIVE_PLAN_CONFIRM_ROLLOUT_ALREADY_ALLOWED',
            'live_plan_confirm_rollout_executed' => 'C78_BLOCKED_C77_LIVE_PLAN_CONFIRM_ROLLOUT_ALREADY_EXECUTED',
        ];
    }

    private function lineageLocksMatch(array $c77): bool
    {
        $locks = is_array($c77['source_artifact_locks'] ?? null) ? $c77['source_artifact_locks'] : [];
        $summary = is_array($c77['lineage_validation_summary'] ?? null) ? $c77['lineage_validation_summary'] : [];

        if (($locks['c76_hash_match'] ?? null) !== true || ($locks['c76_file_sha1_match'] ?? null) !== true || ($locks['c76_source_lineage_match'] ?? null) !== true) {
            return false;
        }
        if (($locks['actual_c76_hash'] ?? null) !== self::EXPECTED_C76_HASH || ($locks['actual_c76_file_sha1'] ?? null) !== self::EXPECTED_C76_FILE_SHA1) {
            return false;
        }
    ...
    private function knownLineageArtifactsMatch(): bool
    {
        ...
        $c72 = json_decode((string) file_get_contents($expected[4][0]), true);
    ...
    private function candidateScopeMatches(array $c77): bool
    {
        $scope = is_array($c77['candidate_scope_freeze_summary'] ?? null) ? $c77['candidate_scope_freeze_summary'] : [];
        $codes = (array) ($c77['next_readiness_decision']['candidate_codes'] ?? []);
    ...
    private function completeSections(array $artifact, array $load, array $options, bool $pass): array
    {
        ...
        $artifact['c77_lock_validation_summary'] = $this->c77LockValidationSummary($load, $c77);
        $artifact['lineage_validation_summary'] = $this->lineageValidationSummary($c77);
        ...
        $artifact['controlled_pilot_shadow_execution_candidate_scorecard'] = $this->candidateScorecard($c77, $pass, (array) ($options['forced_failure_codes'] ?? []));
        $artifact['controlled_pilot_shadow_execution_decision'] = $this->controlledPilotShadowExecutionDecision($pass);
        $artifact['controlled_limited_pilot_shadow_observation_candidate_scorecard'] = $artifact['controlled_pilot_shadow_execution_candidate_scorecard'];
        $artifact['controlled_limited_pilot_shadow_observation_decision'] = $this->controlledLimitedPilotShadowObservationDecision($pass);
        ...
        $artifact['controlled_limited_pilot_observation_context_summary'] = $this->controlledLimitedObservationContextSummary('pilot', $pass, $options);
        $artifact['controlled_limited_shadow_observation_context_summary'] = $this->controlledLimitedObservationContextSummary('shadow', $pass, $options);
        ...
        $artifact['c77_proof_carry_forward_validation_summary'] = $this->c77ProofCarryForwardValidationSummary($c77, $pass);
    ...
    private function controlledGateFailures(array $options): array
    {
        ...
        if (! $this->dictionaryCoverageComplete()) {
            $failures[] = 'C78_BLOCKED_DICTIONARY_COVERAGE_MISSING';
        }
        foreach ([
            'feature_flag_default_off' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_FEATURE_FLAG_OR_KILL_SWITCH',
            'controlled_pilot_feature_flag_default_off' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_FEATURE_FLAG_OR_KILL_SWITCH',
            'controlled_shadow_feature_flag_default_off' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_FEATURE_FLAG_OR_KILL_SWITCH',
            'controlled_parallel_run_feature_flag_default_off' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_FEATURE_FLAG_OR_KILL_SWITCH',
            'controlled_rollout_feature_flag_default_off' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_FEATURE_FLAG_OR_KILL_SWITCH',
            'operator_approval_required' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_OPERATOR_APPROVAL_MISSING',
            'approval_reference_required' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_OPERATOR_APPROVAL_MISSING',
            'kill_switch_available' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_FEATURE_FLAG_OR_KILL_SWITCH',
            'kill_switch_blocks_controlled_pilot_execution_path' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_FEATURE_FLAG_OR_KILL_SWITCH',
            'kill_switch_blocks_controlled_shadow_execution_path' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_FEATURE_FLAG_OR_KILL_SWITCH',
            'controlled_pilot_execution_context_validation_pass' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_PILOT_EXECUTION_CONTEXT_MISSING',
            'controlled_shadow_execution_context_validation_pass' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_SHADOW_EXECUTION_CONTEXT_MISSING',
            'rollback_plan_defined' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_ROLLBACK_PLAN_MISSING',
            'emergency_disable_path_defined' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_EMERGENCY_DISABLE_MISSING',
            'c77_proof_carry_forward_validation_pass' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_SAFETY_OR_LEAKAGE',
            'fallback_behavior_controlled_pilot_shadow_execution_validation_pass' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_FALLBACK_BEHAVIOR_MISSING',
            'baseline_plan_confirm_non_mutation_pass' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_PLAN_CONFIRM_OUTPUT_CHANGED',
            'audit_logging_validation_pass' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_AUDIT_LOGGING_MISSING',
            'observability_validation_pass' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_OBSERVABILITY_MISSING',
            'bad_month_risk_retained' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_BAD_MONTH_GOVERNANCE',
            'weak_regime_risk_retained' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_WEAK_REGIME_GOVERNANCE',
            'documentation_governance_pass' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_DOCUMENTATION_GOVERNANCE',
        ] as $field => $status) {
        ...
        if ((bool) ($options['source_bias_risk_high'] ?? false) || (bool) ($options['shared_core_risk_high'] ?? false)) {
            $failures[] = 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_SOURCE_BIAS_OR_SHARED_CORE';
        }
        if ($this->configFlagIsOn('production_catalog_runtime_bridge_enabled') || $this->configFlagIsOn('production_catalog_controlled_runtime_opt_in_pilot_enabled') || $this->configFlagIsOn('production_catalog_controlled_shadow_rollout_enabled') || $this->configFlagIsOn('production_catalog_controlled_parallel_run_enabled') || $this->configFlagIsOn('production_catalog_controlled_rollout_enabled')) {
            $failures[] = 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_FEATURE_FLAG_OR_KILL_SWITCH';
        }
    ...
    private function statusForProhibitedField(string $field): string
    {
        if (strpos($field, 'pilot_execution_used_') === 0 || strpos($field, 'shadow_execution_used_') === 0 || strpos($field, 'pilot_preparation_used_') === 0 || strpos($field, 'shadow_rollout_preparation_used_') === 0 || strpos($field, 'parallel_run_delta_') === 0 || strpos($field, 'controlled_execution_result_used_') === 0 || strpos($field, 'controlled_wiring_result_used_') === 0) {
            return 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_DELTA_USED_FOR_SELECTION_OR_RETUNING';
        }
        if ($field === 'plan_confirm_output_changed') {
            return 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_PLAN_CONFIRM_OUTPUT_CHANGED';
        }
        if ($field === 'baseline_plan_confirm_hash_changed') {
            return 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_BASELINE_HASH_CHANGED';
        }
        if (in_array($field, ['controlled_pilot_execution_context_persisted_to_live_runtime', 'controlled_shadow_execution_context_persisted_to_live_runtime', 'controlled_pilot_execution_context_mutated_plan_confirm', 'controlled_shadow_execution_context_mutated_plan_confirm', 'controlled_pilot_execution_context_changed_default_runtime', 'controlled_shadow_execution_context_changed_default_runtime'], true)) {
            return 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_DEFAULT_PATH_MUTATION';
        }
        if (in_array($field, ['a01_used_as_runtime_fallback', 'a01_promoted', 'new_candidate_created', 'selection_rule_changed', 'parameter_changed'], true)) {
            return 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH';
        }
        if (in_array($field, ['latest_shortcut_used', 'max_date_shortcut_used', 'future_lookup_detected', 'return_fields_used_for_selection'], true)) {
            return 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_SAFETY_OR_LEAKAGE';
        }
        if (in_array($field, ['production_catalog_runtime_wired', 'controlled_opt_in_runtime_bridge_active', 'controlled_parallel_run_active', 'controlled_rollout_active', 'production_deployment_allowed', 'production_deployment_executed', 'plan_confirm_mutation_allowed', 'plan_confirm_mutated', 'plan_confirm_runtime_reads_activated_catalog', 'live_plan_confirm_rollout_allowed', 'live_plan_confirm_rollout_executed'], true)) {
            return 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_PRODUCTION_MUTATION';
        }
        return 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_REJECTED_FEATURE_FLAG_OR_KILL_SWITCH';
    }
    ...
    private function c77LockValidationSummary(array $load, array $c77): array
    {
        return [
            'c77_lock_validation_completed' => true,
            'c77_artifact_exists' => $load['exists'],
            'c77_artifact_hash_match' => $load['hash_match'],
            'c77_file_sha1_match' => $load['file_sha1_match'],
            'c77_status_match' => ($c77['status'] ?? null) === self::EXPECTED_C77_STATUS,
            'c77_reason_code_match' => ($c77['reason_code'] ?? null) === self::EXPECTED_C77_REASON,
            'c77_pilot_execution_review_pass' => ($c77['controlled_runtime_opt_in_pilot_execution_review_pass'] ?? null) === true,
            'c77_shadow_execution_review_pass' => ($c77['controlled_shadow_rollout_execution_review_pass'] ?? null) === true,
            'c77_readiness_nested_path_validated' => true,
            'top_level_alias_used_for_c77_source_validation' => false,
            'c77_c78_readiness_count_match' => ($c77['next_readiness_decision']['candidate_ready_for_controlled_limited_runtime_observation_review_count'] ?? null) === 2,
            'c77_c78_recommendation_match' => ($c77['next_readiness_decision']['next_recommendation'] ?? null) === self::EXPECTED_C77_RECOMMENDATION,
            'c77_safety_fields_clean' => $this->c77SafetyFieldsClean($c77),
        ];
    }

    private function c77SafetyFieldsClean(array $c77): bool
    {
        foreach ($this->c77SafetyGateMap() as $field => $status) {
            if (($c77[$field] ?? null) !== false) {
                return false;
    ...
    private function lineageValidationSummary(array $c77): array
    {
        $locks = is_array($c77['source_artifact_locks'] ?? null) ? $c77['source_artifact_locks'] : [];
        return [
            'lineage_validation_completed' => true,
            'lineage_lock_validation_pass' => $this->lineageLocksMatch($c77),
            'lineage' => 'C77 -> C76 -> C75 -> C74 -> C73 -> C72 -> C71 -> C70 -> C69 -> C68 -> C67 -> C66 -> C65 -> C64 -> C63 -> C62 -> C61 -> C60',
            'c77_to_c76_lock_match' => (($locks['actual_c76_hash'] ?? null) === self::EXPECTED_C76_HASH),
    ...
    private function candidateScopeFreezeSummary(array $options): array
    {
        return [
            ...
            'candidate_scope_source' => 'C77_LOCKED_CONTROLLED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_EXECUTION_REVIEW_DECISION',
            'primary_candidate_code' => self::PRIMARY_CANDIDATE,
            'backup_candidate_codes' => [self::BACKUP_CANDIDATE],
            'comparator_only_candidate_codes' => [self::COMPARATOR_CANDIDATE],
            'candidate_scope_changed_after_c77' => false,
    ...
    private function operatorApprovalValidationSummary(array $options, bool $pass): array
    {
        ...
        return [
            ...
            'operator_approval_scope' => 'C78_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_REVIEW_ONLY',
    ...
    private function controlledLimitedPilotShadowObservationDecision(bool $pass): array
    {
        return [
            'validation_completed' => true,
            'controlled_limited_runtime_opt_in_pilot_observation_review_executed' => true,
            'controlled_limited_runtime_opt_in_pilot_observation_review_allowed' => $pass,
            'controlled_limited_runtime_opt_in_pilot_observation_review_pass' => $pass,
            'controlled_limited_shadow_rollout_observation_review_executed' => true,
            'controlled_limited_shadow_rollout_observation_review_allowed' => $pass,
            'controlled_limited_shadow_rollout_observation_review_pass' => $pass,
            'controlled_limited_observation_status' => $pass ? 'PRIMARY_AND_BACKUP' : 'NONE',
            'primary_candidate_code' => self::PRIMARY_CANDIDATE,
            'backup_candidate_codes' => [self::BACKUP_CANDIDATE],
            'comparator_only_candidate_codes' => [self::COMPARATOR_CANDIDATE],
            'primary_controlled_limited_pilot_observation_pass' => $pass,
            'backup_controlled_limited_pilot_observation_pass' => $pass,
            'primary_controlled_limited_shadow_observation_pass' => $pass,
            'backup_controlled_limited_shadow_observation_pass' => $pass,
            'a01_remains_comparator_only' => true,
            'operator_approval_required' => true,
            'operator_approval_completed' => $pass,
            'observation_review_is_artifact_only' => true,
            'observation_review_is_non_live_default' => true,
            'observation_review_does_not_select_candidate' => true,
            'observation_review_does_not_retune_candidate' => true,
            'observation_review_does_not_rerank_candidate' => true,
            'observation_review_does_not_mutate_plan_confirm' => true,
            'observation_review_does_not_enable_live_rollout' => true,
            'production_catalog_runtime_wired' => false,
            'controlled_opt_in_runtime_bridge_active' => false,
            'controlled_parallel_run_active' => false,
            'controlled_rollout_active' => false,
            'controlled_limited_pilot_observation_context_created' => true,
            'controlled_limited_shadow_observation_context_created' => true,
            'controlled_limited_pilot_observation_context_persisted_to_live_runtime' => false,
            'controlled_limited_shadow_observation_context_persisted_to_live_runtime' => false,
            'controlled_limited_pilot_observation_context_mutated_plan_confirm' => false,
            'controlled_limited_shadow_observation_context_mutated_plan_confirm' => false,
            'production_deployment_allowed' => false,
            'production_deployment_executed' => false,
            'plan_confirm_mutation_allowed' => false,
            'plan_confirm_mutated' => false,
            ...
            'live_plan_confirm_rollout_allowed' => false,
            'live_plan_confirm_rollout_executed' => false,
            'decision_reason' => $pass ? 'All C78 observation gates passed; only C79 controlled limited observation result review is allowed next.' : 'C78 observation gates failed; targeted cleanup/repair is required.',
            'diagnostic_conclusion' => $pass ? 'CONTROLLED_LIMITED_OBSERVATION_READY_FOR_C79' : 'CONTROLLED_LIMITED_OBSERVATION_REPAIR_REQUIRED',
        ];
    }
    ...
    private function controlledLimitedObservationContextSummary(string $type, bool $pass, array $options): array
    {
        $prefix = $type === 'pilot' ? 'controlled_limited_pilot_observation' : 'controlled_limited_shadow_observation';
        return [
            $prefix.'_context_created' => true,
            $prefix.'_context_validation_pass' => $pass,
            $prefix.'_context_is_explicit_only' => true,
    ...
    private function runtimeReadinessInspectionSummary(): array
    {
        ...
        return [
            ...
            'controlled_limited_runtime_opt_in_pilot_observation_review_contract_identified_or_created' => is_file(self::RUNTIME_PATHS['c78_controlled_limited_pilot_observation_contract']),
            'controlled_limited_shadow_rollout_observation_review_contract_identified_or_created' => is_file(self::RUNTIME_PATHS['c78_controlled_limited_shadow_observation_contract']),
            'explicit_controlled_limited_pilot_observation_context_identified_or_created' => is_file(self::RUNTIME_PATHS['c78_controlled_limited_pilot_observation_context']),
            'explicit_controlled_limited_shadow_observation_context_identified_or_created' => is_file(self::RUNTIME_PATHS['c78_controlled_limited_shadow_observation_context']),
    ...
    private function c77ProofCarryForwardValidationSummary(array $c77, bool $pass): array
    {
        return [
            'c77_proof_carry_forward_validation_completed' => true,
            'c77_proof_carry_forward_validation_pass' => $pass,
            'c77_pilot_execution_review_pass' => ($c77['controlled_runtime_opt_in_pilot_execution_review_pass'] ?? null) === true,
            'c77_shadow_execution_review_pass' => ($c77['controlled_shadow_rollout_execution_review_pass'] ?? null) === true,
            'c77_operator_approval_proof_pass' => true,
            'c77_baseline_non_mutation_pass' => true,
            'c77_governance_pass' => true,
            'c77_fallback_behavior_pass' => true,
            'c77_feature_flag_operator_approval_kill_switch_pass' => true,
            'c77_rollback_and_emergency_disable_pass' => true,
            'c77_audit_logging_pass' => true,
            'c77_observability_pass' => true,
            'c77_production_mutation_safety_pass' => true,
            'c77_negative_operator_approval_rejection_proof_retained' => true,
            'c77_c78_readiness_count' => (int) ($c77['next_readiness_decision']['candidate_ready_for_controlled_limited_runtime_observation_review_count'] ?? 0),
            'c77_c78_recommendation_match' => ($c77['next_readiness_decision']['next_recommendation'] ?? null) === self::EXPECTED_C77_RECOMMENDATION,
        ];
    }
    ...
    private function productionMutationSafetySummary(bool $pass): array
    {
        return [
            ...
            'controlled_shadow_rollout_execution_review_allowed' => $pass,
            'controlled_shadow_rollout_execution_review_pass' => $pass,
            'controlled_limited_runtime_opt_in_pilot_observation_review_created' => true,
            'controlled_limited_runtime_opt_in_pilot_observation_review_allowed' => $pass,
            'controlled_limited_runtime_opt_in_pilot_observation_review_pass' => $pass,
            'controlled_limited_shadow_rollout_observation_review_created' => true,
            'controlled_limited_shadow_rollout_observation_review_allowed' => $pass,
            'controlled_limited_shadow_rollout_observation_review_pass' => $pass,
            'candidate_ready_for_controlled_limited_runtime_observation_review_count' => $pass ? 2 : 0,
            'next_recommendation' => $pass ? self::C79_RECOMMENDATION : 'C78_TARGETED_C77_EXECUTION_REVIEW_REPAIR',
            'production_catalog_runtime_wired' => false,
            'controlled_opt_in_runtime_bridge_active' => false,
            'controlled_parallel_run_active' => false,
            'controlled_rollout_active' => false,
    ...
    private function candidateScorecard(array $c77, bool $pass, array $forcedFailures): array
    {
        $base = [
            'c77_execution_evidence_summary' => ['c77_pilot_execution_review_pass' => ($c77['controlled_runtime_opt_in_pilot_execution_review_pass'] ?? null) === true, 'c77_shadow_execution_review_pass' => ($c77['controlled_shadow_rollout_execution_review_pass'] ?? null) === true],
    ...
    private function nextReadinessDecision(bool $pass): array
    {
        return [
            ...
            'candidate_ready_for_controlled_limited_runtime_observation_result_review_count' => $pass ? 2 : 0,
            'next_recommendation' => $pass ? self::C79_RECOMMENDATION : 'C78_TARGETED_C77_EXECUTION_REVIEW_REPAIR',
            'decision_reason' => $pass ? 'C78 controlled limited pilot/shadow observation review passed. Only C79 controlled limited observation result review is allowed next.' : 'C78 controlled limited pilot/shadow observation review did not pass; targeted repair is required.',
            'diagnostic_conclusion' => $pass ? 'READY_FOR_C79_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_OBSERVATION_RESULT_REVIEW' : 'C78_CONTROLLED_LIMITED_OBSERVATION_REVIEW_REPAIR_REQUIRED',
            'production_catalog_runtime_wired' => false,
            'controlled_opt_in_runtime_bridge_active' => false,
            'controlled_parallel_run_active' => false,
            'controlled_rollout_active' => false,
    ...
    private function failureAttributionSummary(array $scorecards, bool $pass): array
    {
        ...
        return [
            ...
            'targeted_repair_recommendation' => $pass ? self::C79_RECOMMENDATION : 'C78_TARGETED_C77_EXECUTION_REVIEW_REPAIR',
        ];
    }
    ...
    {
        return [
            'C78 validates C77 nested next_readiness_decision.* path and does not use top-level aliases as source validation.',
            'C78 creates isolated artifact-only controlled limited pilot/shadow observation review proof; it is not consumed by PLAN/CONFIRM live runtime.',
            'C78 pass is not production deployment, not live rollout, not runtime bridge activation, and not PLAN/CONFIRM mutation.',
        ];
    }
    ...
    {
        if (strpos($status, 'C77_ARTIFACT') !== false || strpos($status, 'C77_FILE') !== false || strpos($status, 'LINEAGE') !== false) {
            return 'C78_C77_LOCK_OR_LINEAGE_REPAIR';
        }
