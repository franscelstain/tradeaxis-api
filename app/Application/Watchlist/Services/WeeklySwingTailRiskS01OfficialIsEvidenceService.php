...
class WeeklySwingTailRiskS01OfficialIsEvidenceService
{
    public const RUN_CODE = 'WS_TAIL_RISK_S01_VERSIONED_OFFICIAL_IS_EVIDENCE';
    public const APPROVAL_REFERENCE = 'WS_TAIL_RISK_S01_OPERATOR_APPROVED_OFFICIAL_IS_ONLY';
    ...
    public function execute(
        ...
        if ($fromDate !== self::CANONICAL_IS_FROM || $toDate !== self::CANONICAL_IS_TO) {
            return $this->blocked('WS_TAIL_RISK_S01_CANONICAL_IS_WINDOW_MISMATCH');
        }
        $schema = $this->schemaReadiness();
        if (! $schema['pass']) {
            return $this->blocked('WS_TAIL_RISK_S01_SCHEMA_NOT_READY', [
        ...
        if (! $draft || (string) $draft->status !== 'DRAFT'
            ...
            return $this->blocked('WS_TAIL_RISK_S01_DRAFT_NOT_FOUND');
        }
        ...
        if (! is_array($payload) || ! is_array($provenance)) {
            return $this->blocked('WS_TAIL_RISK_S01_DRAFT_PAYLOAD_INVALID');
        }
        $source = is_array($provenance['source'] ?? null) ? $provenance['source'] : [];
        $isInitial = ($source['stage'] ?? '')
            === WeeklySwingTailRiskS01DraftCatalogService::RUN_CODE;
        $isRemediation = ($source['stage'] ?? '')
            === WeeklySwingTailRiskS01RemediationDraftService::RUN_CODE;
        if ((! $isInitial && ! $isRemediation)
            ...
            return $this->blocked('WS_TAIL_RISK_S01_DRAFT_PROVENANCE_INVALID');
        }
        if ($isInitial
            && ($source['diagnostic_artifact_hash'] ?? '')
                !== WeeklySwingTailRiskS01DraftCatalogService::DIAGNOSTIC_ARTIFACT_HASH) {
            return $this->blocked('WS_TAIL_RISK_S01_INITIAL_PROVENANCE_INVALID');
        }
        if ($isRemediation
            && (($source['single_allowed_remediation'] ?? false) !== true
                ...
                || (int) ($source['source_param_set_id'] ?? 0)
                    !== WeeklySwingTailRiskS01RemediationDraftService::SOURCE_PARAM_SET_ID
                || (int) ($source['source_eval_id'] ?? 0)
                    !== WeeklySwingTailRiskS01RemediationDraftService::SOURCE_EVAL_ID
                || ($source['source_official_is_artifact_hash'] ?? '')
                    !== WeeklySwingTailRiskS01RemediationDraftService::SOURCE_OFFICIAL_IS_ARTIFACT_HASH
                || ($source['selection_changed_from_h1'] ?? true) !== false)) {
            return $this->blocked('WS_TAIL_RISK_S01_REMEDIATION_PROVENANCE_INVALID');
        }
        $validation = $this->validator->validate($payload);
        if (! ($validation['valid'] ?? false)
            || (string) $draft->params_hash !== (string) ($validation['canonical_hash'] ?? '')) {
            return $this->blocked('WS_TAIL_RISK_S01_DRAFT_VALIDATION_FAILED', [
                'validation' => $validation,
            ]);
        }
        $selection = $validation['canonical_payload']['research_selection'] ?? null;
        $execution = $validation['canonical_payload']['research_execution'] ?? null;
        if (! is_array($selection) || ! is_array($execution)
            ...
            return $this->blocked('WS_TAIL_RISK_S01_RESEARCH_CONTRACT_INVALID');
        }
        ...
        if (($binding['catalog_code'] ?? '') !== $expectedCatalogCode) {
            return $this->blocked('WS_TAIL_RISK_S01_CATALOG_BINDING_INVALID');
        }
        ...
        if (! ($bindingVerification['valid'] ?? false)) {
            return $this->blocked('WS_TAIL_RISK_S01_PARAMSET_BINDING_INVALID', [
        ...
        foreach (['catalog_version', 'catalog_hash', 'row_code', 'row_hash'] as $field) {
            if ((string) ($binding[$field] ?? '')
                ...
                return $this->blocked('WS_TAIL_RISK_S01_PARAMSET_BINDING_DRIFT', [
                    'binding_field' => $field,
                ]);
            }
        }
        ...
        if ($this->identity->stableHash($selection)
            ...
            return $this->blocked('WS_TAIL_RISK_S01_CATALOG_CONTRACT_MISMATCH');
        }

        $runtime = $this->runtimeAdapter->adapt($validation['canonical_payload']);
        ...
        foreach ([
            ...
            if ((string) ($draft->{$field} ?? '')
                ...
                return $this->blocked('WS_TAIL_RISK_S01_EXECUTION_IDENTITY_MISMATCH', [
        ...
        if (! ($calendar['is_ready'] ?? false)) {
            return $this->blocked('WS_TAIL_RISK_S01_CALENDAR_UNAVAILABLE', [
                'calendar' => $calendar,
            ]);
        }
        $boundariesBefore = $this->forbiddenBoundaryCounts();
        $spoolDirectory = dirname($outputPath).DIRECTORY_SEPARATOR.'.ws-s01-official-evidence-spool';
        $spoolRunKey = 'ws-s01-'.sha1(implode('|', [
        ...
        if ($boundariesBefore !== $boundariesAfter) {
            throw new \RuntimeException('WS_TAIL_RISK_S01_FORBIDDEN_MUTATION_DETECTED');
        }
        ...
        if ((int) ($evaluation['eval_id'] ?? 0) < 1
            ...
            return $this->blocked('WS_TAIL_RISK_S01_OFFICIAL_EVIDENCE_NOT_PERSISTED', [
        ...
        if ((string) ($evaluation['paramset_hash'] ?? '') !== (string) $draft->params_hash
            ...
            return $this->blocked('WS_TAIL_RISK_S01_PERSISTED_IS_IDENTITY_MISMATCH', [
                'evaluation' => $evaluation,
            ]);
        }
        $routeProof = [
            ...
            'selection_signal_date_only' => ($selection['signal_date_only'] ?? false) === true,
            'selection_oos_not_used' => ($selection['oos_used'] ?? true) === false,
            'execution_fixed_before_entry' =>
                ($execution['fixed_before_entry'] ?? false) === true,
            'execution_future_route_not_used' =>
                ($execution['future_derived_route_used'] ?? true) === false,
            'execution_oos_not_used' => ($execution['oos_used'] ?? true) === false,
        ];
        $routeProof['pass'] = ! in_array(false, $routeProof, true);
        if (! $routeProof['pass']) {
            return $this->blocked('WS_TAIL_RISK_S01_EXECUTION_ROUTE_NOT_PROVEN', [
        ...
        $artifact = [
            'schema_version' => 'WS_TAIL_RISK_S01_OFFICIAL_IS_V1',
            'run_code' => self::RUN_CODE,
            'phase_label' => self::RUN_CODE,
            'status' => $isPass
                ? ($isRemediation
                    ? 'WS_TAIL_RISK_S01_REMEDIATION_OFFICIAL_IS_PASSED_OOS_IDENTITY_REVIEW_ALLOWED'
                    : 'WS_TAIL_RISK_S01_OFFICIAL_IS_PASSED_OOS_IDENTITY_REVIEW_ALLOWED')
                : ($isRemediation
                    ? 'WS_TAIL_RISK_S01_REMEDIATION_OFFICIAL_IS_FAILED_S01_CLOSED'
                    : 'WS_TAIL_RISK_S01_OFFICIAL_IS_FAILED_OOS_FORBIDDEN'),
            'reason_code' => $isPass
                ? ($isRemediation
                    ? 'WS_TAIL_RISK_S01_REMEDIATION_PASSED_ALL_CANONICAL_IS_GATES'
                    : 'WS_TAIL_RISK_S01_CANDIDATE_PASSED_ALL_CANONICAL_IS_GATES')
                : ($isRemediation
                    ? 'WS_TAIL_RISK_S01_REMEDIATION_FAILED_NO_MORE_REMEDIATION'
                    : 'WS_TAIL_RISK_S01_CANDIDATE_FAILED_CANONICAL_IS_GATES'),
            ...
            'research_execution_hash' => $this->identity->stableHash($execution),
            ...
            'oos_rows_before' => $boundariesBefore['watchlist_bt_oos_eval_ws'],
            'oos_rows_after' => $boundariesAfter['watchlist_bt_oos_eval_ws'],
            'oos_mutated' => false,
            ...
            'next_recommendation' => $isPass
                ? 'WS_TAIL_RISK_S02_REVIEW_WINNER_IS_IDENTITY_BEFORE_SINGLE_OFFICIAL_OOS'
                : ($isRemediation
                    ? 'WS_TAIL_RISK_S01_CLOSE_FAILED_NO_MORE_REMEDIATION_OOS_FORBIDDEN'
                    : 'WS_TAIL_RISK_S01_WAIT_FOR_ALL_THREE_RESULTS_THEN_AT_MOST_ONE_REMEDIATION'),
        ];
    ...
    {
        return [
            'watchlist_bt_oos_eval_ws' => DB::table('watchlist_bt_oos_eval_ws')->count(),
        ...
        foreach (glob($directory.DIRECTORY_SEPARATOR.$runKey.'.*') ?: [] as $path) {
            if (is_file($path) && ! @unlink($path)) {
                throw new \RuntimeException(
                    'WS_TAIL_RISK_S01_PREVIOUS_SPOOL_CLEANUP_FAILED: '.$path
                );
            }
        }
    }
