...
class WeeklySwingBreakoutIntegrityB01OfficialIsEvidenceService
{
    public const RUN_CODE =
        'WS_BREAKOUT_INTEGRITY_B01_VERSIONED_OFFICIAL_IS_EVIDENCE';
    public const APPROVAL_REFERENCE =
        'WS_BREAKOUT_INTEGRITY_B01_OPERATOR_APPROVED_OFFICIAL_IS_ONLY';
    ...
    public function execute(
        ...
        if ($fromDate !== self::CANONICAL_IS_FROM
            ...
            return $this->blocked(
                'WS_BREAKOUT_INTEGRITY_B01_CANONICAL_IS_WINDOW_MISMATCH'
            );
        }
        ...
        if (! $draft
            ...
            return $this->blocked(
                'WS_BREAKOUT_INTEGRITY_B01_DRAFT_NOT_FOUND'
            );
        }
        ...
        $source = is_array($provenance['source'] ?? null)
            ...
            : [];
        if (($source['stage'] ?? '')
                !== WeeklySwingBreakoutIntegrityB01DraftCatalogService::RUN_CODE
            || ($source['separate_new_strategy_scope'] ?? false) !== true
            || ($source['p01_reopened'] ?? true) !== false
            || ($source['source_anchor_is_best_of_failed_binding'] ?? true)
                !== false
            ...
            || (int) ($source['source_param_set_id'] ?? 0)
                !== WeeklySwingBreakoutIntegrityB01DiagnosticService
                    ::SOURCE_PARAM_SET_ID
            || ($source['source_params_hash'] ?? '')
                !== WeeklySwingBreakoutIntegrityB01DiagnosticService
                    ::SOURCE_PARAMSET_HASH
            || ($source['diagnostic_artifact_hash'] ?? '')
                !== WeeklySwingBreakoutIntegrityB01DraftCatalogService
            ...
            return $this->blocked(
                'WS_BREAKOUT_INTEGRITY_B01_DRAFT_PROVENANCE_INVALID'
            );
        }
        ...
        if (! ($validation['valid'] ?? false)
            ...
            return $this->blocked(
                'WS_BREAKOUT_INTEGRITY_B01_DRAFT_VALIDATION_FAILED',
        ...
        if (! is_array($selection)
            ...
            return $this->blocked(
                'WS_BREAKOUT_INTEGRITY_B01_RESEARCH_CONTRACT_INVALID'
            );
        }
        ...
        if (($binding['catalog_code'] ?? '')
                !== WatchlistBacktestBreakoutIntegrityB01ParamGridCatalog
                    ::CATALOG_CODE) {
            return $this->blocked(
                'WS_BREAKOUT_INTEGRITY_B01_CATALOG_BINDING_INVALID'
            );
        }
        ...
        if (! ($bindingVerification['valid'] ?? false)) {
            return $this->blocked(
                'WS_BREAKOUT_INTEGRITY_B01_PARAMSET_BINDING_INVALID',
                ['binding_verification' => $bindingVerification]
            );
        }
        foreach ([
            'catalog_version', 'catalog_hash', 'row_code', 'row_hash',
        ] as $field) {
            if ((string) ($binding[$field] ?? '')
                !== (string) ($bindingVerification[$field] ?? '')) {
                return $this->blocked(
                    'WS_BREAKOUT_INTEGRITY_B01_PARAMSET_BINDING_DRIFT',
                    ['binding_field' => $field]
                );
            }
        }
        ...
        foreach ([
            ...
            if ((string) ($draft->{$field} ?? '')
                ...
                return $this->blocked(
                    'WS_BREAKOUT_INTEGRITY_B01_EXECUTION_IDENTITY_MISMATCH',
        ...
        if (! ($calendar['is_ready'] ?? false)) {
            return $this->blocked(
                'WS_BREAKOUT_INTEGRITY_B01_CALENDAR_UNAVAILABLE',
                ['calendar' => $calendar]
            );
        }
        $boundariesBefore = $this->forbiddenBoundaryCounts();
        $spoolDirectory = dirname($outputPath)
            .DIRECTORY_SEPARATOR.'.ws-b01-official-evidence-spool';
        $spoolRunKey = 'ws-b01-'.sha1(implode('|', [
            (string) $draft->params_hash,
            (string) $btParamId,
            $fromDate,
            $toDate,
        ...
        if ((int) ($evaluation['eval_id'] ?? 0) < 1
            ...
            || ! in_array(
                (string) (
                    $evaluation['official_evidence_persistence_status'] ?? ''
                ),
                ['INSERTED', 'IDEMPOTENT'],
                true
            )) {
            return $this->blocked(
                'WS_BREAKOUT_INTEGRITY_B01_OFFICIAL_EVIDENCE_NOT_PERSISTED',
        ...
        if ((string) ($evaluation['paramset_hash'] ?? '')
            ...
            || strcmp(
                (string) (
                    $evaluation['max_requested_market_data_date'] ?? ''
                ),
                self::CANONICAL_IS_TO
            ) > 0) {
            return $this->blocked(
                'WS_BREAKOUT_INTEGRITY_B01_PERSISTED_IS_IDENTITY_MISMATCH',
                ['evaluation' => $evaluation]
            );
        }
        ...
        $routeProof = [
            ...
            'selection_uses_exact_breakout_floor' =>
                ($selection['thresholds']['min_close_to_hh20_pct'] ?? null)
                    === -0.05,
        ...
        if (! $routeProof['pass']) {
            return $this->blocked(
                'WS_BREAKOUT_INTEGRITY_B01_EXECUTION_ROUTE_NOT_PROVEN',
        ...
        $artifact = [
            'schema_version' => 'WS_BREAKOUT_INTEGRITY_B01_OFFICIAL_IS_V1',
            'run_code' => self::RUN_CODE,
            'phase_label' => self::RUN_CODE,
            'status' => $isPass
                ? 'WS_BREAKOUT_INTEGRITY_B01_OFFICIAL_IS_PASSED_OOS_IDENTITY_REVIEW_ALLOWED'
                : 'WS_BREAKOUT_INTEGRITY_B01_OFFICIAL_IS_FAILED_OOS_FORBIDDEN',
            'reason_code' => $isPass
                ? 'WS_BREAKOUT_INTEGRITY_B01_PASSED_ALL_CANONICAL_IS_GATES'
                : 'WS_BREAKOUT_INTEGRITY_B01_FAILED_CANONICAL_IS_GATES',
            ...
            'row_code' => (string) $bindingVerification['row_code'],
            'primary_hypothesis_code' =>
                WatchlistBacktestBreakoutIntegrityB01ParamGridCatalog
                    ::PRIMARY_HYPOTHESIS_CODE,
            'candidate_code' => (string) $selection['hypothesis_code'],
            'research_rule_code' => (string) $selection['rule_code'],
            'minimum_close_to_hh20_pct' =>
                (float) $selection['thresholds']['min_close_to_hh20_pct'],
            ...
            'next_recommendation' => $isPass
                ? 'WS_BREAKOUT_INTEGRITY_B01_REVIEW_WINNER_IS_IDENTITY_BEFORE_SINGLE_OFFICIAL_OOS'
                : 'WS_BREAKOUT_INTEGRITY_B01_AT_MOST_ONE_REMEDIATION_REVIEW',
        ];
    ...
    private function clearPreviousSpoolRun(
        string $directory,
        string $runKey
    ): void {
        if (! is_dir($directory)) {
        ...
        foreach (
            glob($directory.DIRECTORY_SEPARATOR.$runKey.'.*') ?: []
            as $path
        ) {
            if (is_file($path) && ! @unlink($path)) {
                throw new \RuntimeException(
                    'WS_BREAKOUT_INTEGRITY_B01_PREVIOUS_SPOOL_CLEANUP_FAILED: '
                    .$path
                );
            }
        }
    ...
    private function writeArtifact(
        ...
        if ($path === '') {
            throw new \RuntimeException(
                'WS_BREAKOUT_INTEGRITY_B01_OUTPUT_PATH_REQUIRED'
            );
        }
