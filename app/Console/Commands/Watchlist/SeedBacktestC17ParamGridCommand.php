...
class SeedBacktestC17ParamGridCommand extends Command
{
    protected $signature = 'watchlist:backtest-c17-param-grid-seed';

    protected $description = 'Seed immutable deterministic Weekly Swing C17 quality-preserving sample-recovery catalog without mutating historical catalogs.';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $result = $repository->seedCatalog(WatchlistBacktestC17ParamGridCatalog::rows());

            $snapshotsAfter = $this->historicalSnapshots($repository);
            if ($snapshotsBefore !== $snapshotsAfter) {
                throw new \RuntimeException('WS_BT_R1_MUTATION_REJECTED: historical R1/R2/C01/C02/C03/C04/C05/C06/C07/C14/C15/C16 snapshot changed during C17 seed.');
            }
        } catch (\Throwable $e) {
        ...
        $this->line('catalog_code='.WatchlistBacktestC17ParamGridCatalog::CATALOG_CODE);
        $this->line('catalog_version='.WatchlistBacktestC17ParamGridCatalog::CATALOG_VERSION);
        $this->line('catalog_count='.WatchlistBacktestC17ParamGridCatalog::CATALOG_COUNT);
        $this->line('catalog_hash='.WatchlistBacktestC17ParamGridCatalog::hash());
    ...
    private function historicalSnapshots(WatchlistBacktestParamGridRepository $repository): array
    {
        return [
            ...
            'c16' => $repository->catalogSnapshot(WatchlistBacktestC16ParamGridCatalog::CATALOG_CODE),
        ];
    }
    ...
    private function assertHistoricalCatalogs(array $snapshots): void
    {
        $expected = [
            ...
            'c16' => [WatchlistBacktestC16ParamGridCatalog::CATALOG_COUNT, WatchlistBacktestC16ParamGridCatalog::hash()],
        ];
        ...
        foreach ($expected as $label => [$count, $hash]) {
            ...
            if ((int) ($snapshot['catalog_count'] ?? 0) !== $count || (string) ($snapshot['catalog_hash'] ?? '') !== $hash) {
                throw new \RuntimeException('WS_BT_R2_CATALOG_PERSISTED_SET_MISMATCH: '.strtoupper($label).' must exist with its immutable count/hash before C17 can be seeded.');
            }
        }
    }
    ...
    private function reasonCode(\Throwable $e): string
    {
        ...
        return 'WS_BT_C17_PARAM_GRID_SEED_FAILED';
    }
}
