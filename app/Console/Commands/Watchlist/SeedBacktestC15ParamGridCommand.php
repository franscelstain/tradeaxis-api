...
class SeedBacktestC15ParamGridCommand extends Command
{
    protected $signature = 'watchlist:backtest-c15-param-grid-seed';

    protected $description = 'Seed immutable deterministic Weekly Swing C15 controlled-pullback mid-liquidity catalog without mutating R1/R2/C01/C02/C03/C04/C05/C06/C07/C14.';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $result = $repository->seedCatalog(WatchlistBacktestC15ParamGridCatalog::rows());

            $snapshotsAfter = $this->historicalSnapshots($repository);
            if ($snapshotsBefore !== $snapshotsAfter) {
                throw new \RuntimeException('WS_BT_R1_MUTATION_REJECTED: historical R1/R2/C01/C02/C03/C04/C05/C06/C07/C14 snapshot changed during C15 seed.');
            }
        } catch (\Throwable $e) {
        ...
        $this->line('catalog_code='.WatchlistBacktestC15ParamGridCatalog::CATALOG_CODE);
        $this->line('catalog_version='.WatchlistBacktestC15ParamGridCatalog::CATALOG_VERSION);
        $this->line('catalog_count='.WatchlistBacktestC15ParamGridCatalog::CATALOG_COUNT);
        $this->line('catalog_hash='.WatchlistBacktestC15ParamGridCatalog::hash());
    ...
    private function assertHistoricalCatalogs(array $snapshots): void
    {
        ...
        foreach ($expected as $label => [$count, $hash]) {
            ...
            if ((int) ($snapshot['catalog_count'] ?? 0) !== $count || (string) ($snapshot['catalog_hash'] ?? '') !== $hash) {
                throw new \RuntimeException('WS_BT_R2_CATALOG_PERSISTED_SET_MISMATCH: '.strtoupper($label).' must exist with its immutable count/hash before C15 can be seeded.');
            }
        }
    }
    ...
    private function reasonCode(\Throwable $e): string
    {
        ...
        return 'WS_BT_C15_PARAM_GRID_SEED_FAILED';
    }
}
