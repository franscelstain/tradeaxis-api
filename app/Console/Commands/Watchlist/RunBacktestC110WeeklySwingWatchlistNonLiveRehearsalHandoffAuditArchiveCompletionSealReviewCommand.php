...
class RunBacktestC110WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveCompletionSealReviewCommand extends Command
{
    protected $signature = 'watchlist:backtest-c110-weekly-swing-watchlist-non-live-rehearsal-handoff-audit-archive-completion-seal-review
    ...
    protected $description = 'Run C110 weekly swing watchlist non-live rehearsal handoff audit archive completion seal review without production deployment, weekly live output, or PLAN/CONFIRM mutation.';

    private WatchlistBacktestC110WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveCompletionSealReviewService $service;

    public function __construct(?WatchlistBacktestC110WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveCompletionSealReviewService $service = null)
    {
        parent::__construct();
        $this->service = $service ?: new WatchlistBacktestC110WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveCompletionSealReviewService();
    }

    public function handle(): int
    {
        if ((bool) $this->option('progress')) {
            $this->line('C110 weekly swing watchlist non-live rehearsal handoff audit archive completion seal review started');
        }

        $result = $this->service->execute(
            (string) $this->option('c109-artifact'),
            (string) $this->option('expected-c109-hash'),
            (string) $this->option('expected-c109-file-sha1'),
        ...
        foreach ([
            ...
            'expected_c109_hash', 'actual_c109_hash', 'c109_hash_match', 'expected_c109_file_sha1', 'actual_c109_file_sha1', 'c109_file_sha1_match',
        ...
        if (strpos((string) ($result['status'] ?? ''), 'C110_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_HANDOFF_AUDIT_ARCHIVE_COMPLETION_SEAL_REVIEW_PASSED') === 0) {
            if ((bool) $this->option('progress')) {
                $this->line('C110 weekly swing watchlist non-live rehearsal handoff audit archive completion seal review completed');
            }
            return 0;
        }
