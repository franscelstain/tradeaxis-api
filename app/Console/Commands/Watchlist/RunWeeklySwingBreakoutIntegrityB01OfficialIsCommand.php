...
class RunWeeklySwingBreakoutIntegrityB01OfficialIsCommand extends Command
{
    protected $signature = 'watchlist:weekly-swing-breakout-integrity-b01-official-is
        {--param-set-id= : Exact B01 DRAFT param_set_id}
        {--from=2023-01-02 : Canonical IS start date}
        {--to=2025-05-21 : Canonical IS end date}
        {--approval-reference= : Exact B01 Official-IS approval reference}
        {--operator-approved : Confirm Official IS only; no OOS, promotion, PLAN, or production}
        {--output=storage/app/watchlist/backtest/ws-breakout-integrity-b01-official-is.json : Output artifact}
        {--overwrite : Replace an existing local B01 IS artifact}';

    protected $description = 'Run strict versioned Official IS for the exact B01 DRAFT; OOS is neither read nor run.';
    ...
    public function handle(): int
    {
        ...
        if ($paramSetId === false || $paramSetId < 1) {
            ...
            $this->line(
                'reason_code=WS_BREAKOUT_INTEGRITY_B01_PARAM_SET_ID_INVALID'
            );

            return 1;
        }
        $result = $this->laravel
            ->make(WeeklySwingBreakoutIntegrityB01OfficialIsEvidenceService::class)
            ->execute(
                (int) $paramSetId,
        ...
        foreach ([
            ...
            'minimum_close_to_hh20_pct', 'params_hash',
