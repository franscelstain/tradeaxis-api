...
class SeedBacktestC14ParamGridCommand extends Command
{
    protected $signature = 'watchlist:backtest-c14-param-grid-seed';

    protected $description = 'Seed immutable deterministic Weekly Swing C14 variable risk-exit catalog without mutating R1/R2/C01/C02/C03/C04/C05/C06/C07.';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $c07Before = $repository->catalogSnapshot(WatchlistBacktestC07ParamGridCatalog::CATALOG_CODE);
            $this->assertHistoricalCatalogs($r1Before, $r2Before, $c01Before, $c02Before, $c03Before, $c04Before, $c05Before, $c06Before, $c07Before);

            $result = $repository->seedCatalog(WatchlistBacktestC14ParamGridCatalog::rows());
            ...
            $c07After = $repository->catalogSnapshot(WatchlistBacktestC07ParamGridCatalog::CATALOG_CODE);
            if ($r1Before !== $r1After || $r2Before !== $r2After || $c01Before !== $c01After
                || $c02Before !== $c02After || $c03Before !== $c03After || $c04Before !== $c04After
                || $c05Before !== $c05After || $c06Before !== $c06After || $c07Before !== $c07After) {
                throw new \RuntimeException('WS_BT_R1_MUTATION_REJECTED: historical R1/R2/C01/C02/C03/C04/C05/C06/C07 snapshot changed during C14 seed.');
            }
        } catch (\Throwable $e) {
        ...
        $this->line('catalog_code='.WatchlistBacktestC14ParamGridCatalog::CATALOG_CODE);
        $this->line('catalog_version='.WatchlistBacktestC14ParamGridCatalog::CATALOG_VERSION);
        $this->line('catalog_count='.WatchlistBacktestC14ParamGridCatalog::CATALOG_COUNT);
        $this->line('catalog_hash='.WatchlistBacktestC14ParamGridCatalog::hash());
    ...
    private function assertHistoricalCatalogs(
        array $r1,
        array $r2,
        array $c01,
        array $c02,
        ...
        $expected = [
            ...
            'C07' => [$c07, WatchlistBacktestC07ParamGridCatalog::CATALOG_COUNT, WatchlistBacktestC07ParamGridCatalog::hash()],
        ];

        foreach ($expected as $label => [$snapshot, $count, $hash]) {
            if ((int) ($snapshot['catalog_count'] ?? 0) !== $count || (string) ($snapshot['catalog_hash'] ?? '') !== $hash) {
                throw new \RuntimeException('WS_BT_R2_CATALOG_PERSISTED_SET_MISMATCH: '.$label.' must exist with its immutable count/hash before C14 can be seeded.');
            }
        }
    }
    ...
    private function reasonCode(\Throwable $e): string
    {
        ...
        return 'WS_BT_C14_PARAM_GRID_SEED_FAILED';
    }
}
