...
class RunBacktestC143WeeklySwingWatchlistProductionLiveRuntimeActivationGoDecisionFinalizationReviewCommand extends Command
{
    protected $signature = 'watchlist:backtest-c143-weekly-swing-watchlist-production-live-runtime-activation-go-decision-finalization-review
    ...
    protected $description = 'Run C143 weekly swing watchlist production/live runtime activation GO decision finalization review without activating runtime bridge, live output, or PLAN/CONFIRM mutation.';

    private WatchlistBacktestC143WeeklySwingWatchlistProductionLiveRuntimeActivationGoDecisionFinalizationReviewService $service;

    public function __construct(?WatchlistBacktestC143WeeklySwingWatchlistProductionLiveRuntimeActivationGoDecisionFinalizationReviewService $service = null)
    {
        parent::__construct();
        $this->service = $service ?: new WatchlistBacktestC143WeeklySwingWatchlistProductionLiveRuntimeActivationGoDecisionFinalizationReviewService();
    }

    public function handle(): int
    {
        if ((bool) $this->option('progress')) {
            $this->line('C143 weekly swing watchlist production/live runtime activation GO decision finalization review started');
        }

        $result = $this->service->execute(
            (string) $this->option('c142-artifact'),
            (string) $this->option('expected-c142-hash'),
            (string) $this->option('expected-c142-file-sha1'),
        ...
        foreach ([
            ...
            'c143_go_decision_finalization_review_only', 'c143_not_live_runtime_state_change',
            ...
            'expected_c142_hash', 'actual_c142_hash', 'c142_hash_match',
            'expected_c142_file_sha1', 'actual_c142_file_sha1', 'c142_file_sha1_match',
        ...
        if (strpos((string) ($result['status'] ?? ''), 'C143_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_GO_DECISION_FINALIZATION_REVIEW_PASSED') === 0) {
            if ((bool) $this->option('progress')) {
                $this->line('C143 weekly swing watchlist production/live runtime activation GO decision finalization review completed');
            }
