...
class RunBacktestC163WeeklySwingWatchlistProductionLiveRuntimePlanConfirmCompletionPostHandoffActivationObservationResultReviewCommand extends Command
{
    protected $signature = 'watchlist:backtest-c163-weekly-swing-watchlist-production-live-runtime-plan-confirm-completion-post-handoff-activation-observation-result-review
        {--c163-post-handoff-activation-observation-artifact=storage/app/watchlist/backtest/c163-weekly-swing-watchlist-production-live-runtime-plan-confirm-completion-post-handoff-activation-observation-review.json}
        {--expected-c163-post-handoff-activation-observation-hash=2c150f14fca84692db091b8b5137ed1e68855ffa}
        {--expected-c163-post-handoff-activation-observation-file-sha1=94ACF854DAF2DF1669B89D487F13496D0019F576}
        {--approval-reference=}
        {--output=storage/app/watchlist/backtest/c163-weekly-swing-watchlist-production-live-runtime-plan-confirm-completion-post-handoff-activation-observation-result-review.json}
        {--operator-approved}
    ...
    protected $description = 'Run C163 weekly swing watchlist production/live runtime PLAN/CONFIRM completion post-handoff activation observation result review.';

    private WatchlistBacktestC163WeeklySwingWatchlistProductionLiveRuntimePlanConfirmCompletionPostHandoffActivationObservationResultReviewService $service;

    public function __construct(?WatchlistBacktestC163WeeklySwingWatchlistProductionLiveRuntimePlanConfirmCompletionPostHandoffActivationObservationResultReviewService $service = null)
    {
        parent::__construct();
        $this->service = $service ?: new WatchlistBacktestC163WeeklySwingWatchlistProductionLiveRuntimePlanConfirmCompletionPostHandoffActivationObservationResultReviewService();
    }

    public function handle(): int
    {
        if ((bool) $this->option('progress')) {
            $this->line('C163 weekly swing watchlist production/live runtime PLAN/CONFIRM completion post-handoff activation observation result review started');
        }

        $result = $this->service->execute(
            (string) $this->option('c163-post-handoff-activation-observation-artifact'),
            (string) $this->option('expected-c163-post-handoff-activation-observation-hash'),
            (string) $this->option('expected-c163-post-handoff-activation-observation-file-sha1'),
            (string) $this->option('output'),
            [
                'overwrite' => (bool) $this->option('overwrite'),
                'operator_approved' => (bool) $this->option('operator-approved'),
                'result_review_confirmed' => (bool) $this->option('result-review-confirmed'),
                'post_handoff_activation_observation_result_confirmed' => (bool) $this->option('post-handoff-activation-observation-result-confirmed'),
                'c163_post_handoff_activation_observation_complete_confirmed' => (bool) $this->option('c163-post-handoff-activation-observation-complete-confirmed'),
        ...
        foreach ([
            ...
            'expected_c163_post_handoff_activation_observation_hash', 'actual_c163_post_handoff_activation_observation_hash', 'c163_post_handoff_activation_observation_hash_match',
            'expected_c163_post_handoff_activation_observation_file_sha1', 'actual_c163_post_handoff_activation_observation_file_sha1', 'c163_post_handoff_activation_observation_file_sha1_match',
        ...
        if (strpos((string) ($result['status'] ?? ''), 'C163_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_PLAN_CONFIRM_COMPLETION_POST_HANDOFF_ACTIVATION_OBSERVATION_RESULT_REVIEW_PASSED') === 0) {
            if ((bool) $this->option('progress')) {
                $this->line('C163 weekly swing watchlist production/live runtime PLAN/CONFIRM completion post-handoff activation observation result review completed');
            }
