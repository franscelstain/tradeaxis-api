...
class RunBacktestC108WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveReviewCommand extends Command
{
    protected $signature = 'watchlist:backtest-c108-weekly-swing-watchlist-non-live-rehearsal-handoff-audit-archive-review
    ...
    protected $description = 'Run C108 weekly swing watchlist non-live rehearsal handoff audit archive review without production deployment, weekly live output, or PLAN/CONFIRM mutation.';

    private WatchlistBacktestC108WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveReviewService $service;

    public function __construct(?WatchlistBacktestC108WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveReviewService $service = null)
    {
        parent::__construct();
        $this->service = $service ?: new WatchlistBacktestC108WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveReviewService();
    }

    public function handle(): int
    {
        if ((bool) $this->option('progress')) {
            $this->line('C108 weekly swing watchlist non-live rehearsal handoff audit archive review started');
        }

        $result = $this->service->execute(
            (string) $this->option('c107-artifact'),
            (string) $this->option('expected-c107-hash'),
            (string) $this->option('expected-c107-file-sha1'),
        ...
        foreach ([
            ...
            'weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_review_executed',
            'weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_review_allowed',
            ...
            'expected_c107_hash', 'actual_c107_hash', 'c107_hash_match', 'expected_c107_file_sha1', 'actual_c107_file_sha1', 'c107_file_sha1_match',
        ...
        if (strpos((string) ($result['status'] ?? ''), 'C108_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_HANDOFF_AUDIT_ARCHIVE_REVIEW_PASSED') === 0) {
            if ((bool) $this->option('progress')) {
                $this->line('C108 weekly swing watchlist non-live rehearsal handoff audit archive review completed');
            }
            return 0;
        }
