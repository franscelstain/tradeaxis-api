...
class RunBacktestC57RegimeFieldReconstructionContinuationIsOnlyCommand extends Command
{
    protected $signature = 'watchlist:backtest-c57-regime-field-reconstruction-continuation-is-only
    ...
    protected $description = 'Run C57 IS-only regime field reconstruction continuation from locked C56/C55/C54/C53/C52 lineage without OOS access.';

    private WatchlistBacktestC57RegimeFieldReconstructionContinuationIsOnlyService $service;

    public function __construct(?WatchlistBacktestC57RegimeFieldReconstructionContinuationIsOnlyService $service = null)
    {
        parent::__construct();
        $this->service = $service ?: new WatchlistBacktestC57RegimeFieldReconstructionContinuationIsOnlyService();
    }

    public function handle(): int
    {
        if ((bool) $this->option('progress')) { $this->line('C57 IS-only regime field reconstruction continuation started'); }
        $result = $this->service->execute(
            (string) $this->option('c56-artifact'), (string) $this->option('expected-c56-hash'),
        ...
        foreach (['status', 'reason_code', 'artifact_path', 'artifact_hash', 'production_ready', 'expected_c56_hash', 'actual_c56_hash', 'c56_hash_match', 'c56_status', 'c56_diagnostic_conclusion', 'c56_next_step_recommendation', 'expected_c55_hash', 'actual_c55_hash', 'c55_hash_match', 'expected_c55_file_sha1', 'actual_c55_file_sha1', 'c55_file_sha1_match', 'expected_c54_hash', 'actual_c54_hash', 'c54_hash_match', 'expected_c54_file_sha1', 'actual_c54_file_sha1', 'c54_file_sha1_match', 'expected_c53_hash', 'actual_c53_hash', 'c53_hash_match', 'expected_c53_file_sha1', 'actual_c53_file_sha1', 'c53_file_sha1_match', 'expected_c52_hash', 'actual_c52_hash', 'c52_hash_match', 'expected_c52_file_sha1', 'actual_c52_file_sha1', 'c52_file_sha1_match', 'diagnostic_conclusion', 'next_step_recommendation'] as $key) {
            if (array_key_exists($key, $result)) { $this->line($key.'='.$this->scalar($result[$key])); }
        }
        foreach ((array) ($result['c58_readiness_decision'] ?? []) as $key => $value) { if (! is_array($value)) { $this->line('c58_'.$key.'='.$this->scalar($value)); } }
        if (($result['status'] ?? null) === 'C57_REGIME_FIELD_RECONSTRUCTION_CONTINUATION_IS_ONLY_COMPLETED') {
            if ((bool) $this->option('progress')) { $this->line('C57 IS-only regime field reconstruction continuation completed'); }
            return 0;
        }
