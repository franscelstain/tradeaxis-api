...
class RunBacktestC67ProductionCatalogActivationReviewCommand extends Command
{
    protected $signature = 'watchlist:backtest-c67-production-catalog-activation-review
    ...
    protected $description = 'Run C67 production catalog activation review from locked C66 evidence without live activation, deployment, or PLAN/CONFIRM mutation.';

    private WatchlistBacktestC67ProductionCatalogActivationReviewService $service;

    public function __construct(?WatchlistBacktestC67ProductionCatalogActivationReviewService $service = null)
    {
        parent::__construct();
        $this->service = $service ?: new WatchlistBacktestC67ProductionCatalogActivationReviewService();
    }

    public function handle(): int
    {
        if ((bool) $this->option('progress')) {
            $this->line('C67 production catalog activation review started');
        }
        ...
        foreach ([
            ...
            'production_catalog_activation_review_executed', 'production_catalog_activation_review_pass',
            'production_catalog_lock_allowed', 'production_catalog_activation_allowed',
            'production_catalog_activation_execution_allowed', 'production_deployment_allowed',
            'plan_confirm_mutation_allowed', 'expected_c66_hash', 'actual_c66_hash', 'c66_hash_match',
        ...
        foreach ((array) ($result['production_catalog_activation_review_decision'] ?? []) as $key => $value) {
            if (! is_array($value)) {
                $this->line('production_catalog_activation_review_'.$key.'='.$this->scalar($value));
            }
        }

        foreach ((array) ($result['c68_readiness_decision'] ?? []) as $key => $value) {
            if (! is_array($value)) {
                $this->line('c68_'.$key.'='.$this->scalar($value));
            }
        }

        if (strpos((string) ($result['status'] ?? ''), 'C67_PRODUCTION_CATALOG_ACTIVATION_REVIEW_PASSED') === 0) {
            if ((bool) $this->option('progress')) {
                $this->line('C67 production catalog activation review completed');
            }
            return 0;
        }
