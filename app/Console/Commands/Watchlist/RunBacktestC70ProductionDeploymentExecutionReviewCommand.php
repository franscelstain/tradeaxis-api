...
class RunBacktestC70ProductionDeploymentExecutionReviewCommand extends Command
{
    protected $signature = 'watchlist:backtest-c70-production-deployment-execution-review
    ...
    protected $description = 'Run C70 controlled production deployment execution review without live deployment or PLAN/CONFIRM mutation.';

    private WatchlistBacktestC70ProductionDeploymentExecutionReviewService $service;

    public function __construct(?WatchlistBacktestC70ProductionDeploymentExecutionReviewService $service = null)
    {
        parent::__construct();
        $this->service = $service ?: new WatchlistBacktestC70ProductionDeploymentExecutionReviewService();
    }

    public function handle(): int
    {
        if ((bool) $this->option('progress')) {
            $this->line('C70 controlled production deployment execution review started');
        }
        ...
        foreach ([
            'status', 'reason_code', 'artifact_path', 'artifact_hash', 'production_deployment_execution_review_executed',
            'production_deployment_execution_review_pass', 'production_ready', 'production_catalog_lock_allowed',
            'production_catalog_activation_allowed', 'production_catalog_activation_execution_allowed',
            'production_catalog_activation_execution_performed', 'production_catalog_activated', 'production_catalog_runtime_wired',
            'production_deployment_prep_allowed', 'production_deployment_execution_review_allowed',
            'controlled_production_deployment_execution_review_allowed', 'controlled_production_deployment_execution_review_pass',
            'production_deployment_allowed', 'production_deployment_executed', 'plan_confirm_wiring_prep_allowed',
            ...
            'expected_c69_hash', 'actual_c69_hash', 'c69_hash_match',
            'expected_c69_file_sha1', 'actual_c69_file_sha1', 'c69_file_sha1_match',
        ...
        foreach ((array) ($result['controlled_deployment_execution_decision'] ?? []) as $key => $value) {
            if (! is_array($value)) {
                $this->line('controlled_deployment_'.$key.'='.$this->scalar($value));
            }
        }

        foreach ((array) ($result['c71_readiness_decision'] ?? []) as $key => $value) {
            if (! is_array($value)) {
                $this->line('c71_'.$key.'='.$this->scalar($value));
            }
        }

        if (strpos((string) ($result['status'] ?? ''), 'C70_PRODUCTION_DEPLOYMENT_EXECUTION_REVIEW_PASSED') === 0) {
            if ((bool) $this->option('progress')) {
                $this->line('C70 controlled production deployment execution review completed');
            }
            return 0;
        }
