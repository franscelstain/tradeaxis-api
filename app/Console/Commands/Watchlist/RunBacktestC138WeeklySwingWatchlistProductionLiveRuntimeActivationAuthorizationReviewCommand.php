...
class RunBacktestC138WeeklySwingWatchlistProductionLiveRuntimeActivationAuthorizationReviewCommand extends Command
{
    protected $signature = 'watchlist:backtest-c138-weekly-swing-watchlist-production-live-runtime-activation-authorization-review
    ...
    protected $description = 'Run C138 weekly swing watchlist production/live runtime activation authorization review without activation execution, runtime bridge activation, live output, or PLAN/CONFIRM mutation.';

    private WatchlistBacktestC138WeeklySwingWatchlistProductionLiveRuntimeActivationAuthorizationReviewService $service;

    public function __construct(?WatchlistBacktestC138WeeklySwingWatchlistProductionLiveRuntimeActivationAuthorizationReviewService $service = null)
    {
        parent::__construct();
        $this->service = $service ?: new WatchlistBacktestC138WeeklySwingWatchlistProductionLiveRuntimeActivationAuthorizationReviewService();
    }

    public function handle(): int
    {
        if ((bool) $this->option('progress')) {
            $this->line('C138 weekly swing watchlist production/live runtime activation authorization review started');
        }

        $result = $this->service->execute(
            (string) $this->option('c137-artifact'),
            (string) $this->option('expected-c137-hash'),
            (string) $this->option('expected-c137-file-sha1'),
        ...
        foreach ([
            ...
            'c137_lock_valid', 'c137_pre_activation_boundary_valid', 'c137_convert_from_json_pass',
            ...
            'c137_pre_activation_boundary_review_only', 'c137_not_activation_authorization',
            ...
            'expected_c137_hash', 'actual_c137_hash', 'c137_hash_match',
            'expected_c137_file_sha1', 'actual_c137_file_sha1', 'c137_file_sha1_match',
        ...
        if (strpos((string) ($result['status'] ?? ''), 'C138_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_PASSED') === 0) {
            if ((bool) $this->option('progress')) {
                $this->line('C138 weekly swing watchlist production/live runtime activation authorization review completed');
            }
