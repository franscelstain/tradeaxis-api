...
class RunBacktestC107WeeklySwingWatchlistNonLiveRehearsalHandoffClosureSealReviewCommand extends Command
{
    protected $signature = 'watchlist:backtest-c107-weekly-swing-watchlist-non-live-rehearsal-handoff-closure-seal-review
    ...
    protected $description = 'Run C107 weekly swing watchlist non-live rehearsal handoff closure seal review without production deployment, weekly live output, or PLAN/CONFIRM mutation.';

    private WatchlistBacktestC107WeeklySwingWatchlistNonLiveRehearsalHandoffClosureSealReviewService $service;

    public function __construct(?WatchlistBacktestC107WeeklySwingWatchlistNonLiveRehearsalHandoffClosureSealReviewService $service = null)
    {
        parent::__construct();
        $this->service = $service ?: new WatchlistBacktestC107WeeklySwingWatchlistNonLiveRehearsalHandoffClosureSealReviewService();
    }

    public function handle(): int
    {
        if ((bool) $this->option('progress')) {
            $this->line('C107 weekly swing watchlist non-live rehearsal handoff closure seal review started');
        }

        $result = $this->service->execute(
            (string) $this->option('c106-artifact'),
            (string) $this->option('expected-c106-hash'),
            (string) $this->option('expected-c106-file-sha1'),
        ...
        foreach ([
            ...
            'weekly_swing_watchlist_non_live_rehearsal_handoff_closure_seal_review_executed',
            'weekly_swing_watchlist_non_live_rehearsal_handoff_closure_seal_review_allowed',
            'weekly_swing_watchlist_non_live_rehearsal_handoff_closure_seal_review_pass',
            ...
            'comparator_candidate_weekly_swing_non_live_rehearsal_handoff_closure_sealed',
            ...
            'expected_c106_hash', 'actual_c106_hash', 'c106_hash_match', 'expected_c106_file_sha1', 'actual_c106_file_sha1', 'c106_file_sha1_match',
        ...
        if (strpos((string) ($result['status'] ?? ''), 'C107_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_HANDOFF_CLOSURE_SEAL_REVIEW_PASSED') === 0) {
            if ((bool) $this->option('progress')) {
                $this->line('C107 weekly swing watchlist non-live rehearsal handoff closure seal review completed');
            }
            return 0;
        }
