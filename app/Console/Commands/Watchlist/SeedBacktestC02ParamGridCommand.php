...
class SeedBacktestC02ParamGridCommand extends Command
{
    protected $signature = 'watchlist:backtest-c02-param-grid-seed';

    protected $description = 'Seed immutable deterministic Weekly Swing downside/stability C02 catalog without mutating R1/R2/C01.';
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $this->assertHistoricalCatalogsIntact($r1Before, $r2Before, $c01Before);

            $result = $repository->seedCatalog(WatchlistBacktestC02ParamGridCatalog::rows());
            ...
            if ($r1Before !== $r1After || $r2Before !== $r2After || $c01Before !== $c01After) {
                throw new \RuntimeException('WS_BT_R1_MUTATION_REJECTED: historical R1/R2/C01 snapshot changed during C02 seed.');
            }
        } catch (\Throwable $e) {
        ...
        $this->line('catalog_code='.WatchlistBacktestC02ParamGridCatalog::CATALOG_CODE);
        $this->line('catalog_version='.WatchlistBacktestC02ParamGridCatalog::CATALOG_VERSION);
        $this->line('catalog_count='.WatchlistBacktestC02ParamGridCatalog::CATALOG_COUNT);
        $this->line('catalog_hash='.WatchlistBacktestC02ParamGridCatalog::hash());
    ...
    private function assertHistoricalCatalogsIntact(array $r1, array $r2, array $c01): void
    {
        if ((int) $r1['catalog_count'] !== WatchlistBacktestParamGridCatalog::CATALOG_COUNT
            ...
            throw new \RuntimeException(
                'WS_BT_R1_MUTATION_REJECTED: R1 must exist with its immutable count/hash before C02 can be seeded.'
            );
        }
        if ((int) $r2['catalog_count'] !== WatchlistBacktestR2ParamGridCatalog::CATALOG_COUNT
            ...
            throw new \RuntimeException(
                'WS_BT_R2_CATALOG_PERSISTED_SET_MISMATCH: R2 must exist with its immutable count/hash before C02 can be seeded.'
            );
        }
        if ((int) $c01['catalog_count'] !== WatchlistBacktestC01ParamGridCatalog::CATALOG_COUNT
            ...
            throw new \RuntimeException(
                'WS_BT_R2_CATALOG_PERSISTED_SET_MISMATCH: C01 must exist with its immutable count/hash before C02 can be seeded.'
            );
        }
    }
