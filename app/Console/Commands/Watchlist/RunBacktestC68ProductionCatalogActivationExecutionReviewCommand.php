...
class RunBacktestC68ProductionCatalogActivationExecutionReviewCommand extends Command
{
    protected $signature = 'watchlist:backtest-c68-production-catalog-activation-execution-review
    ...
    protected $description = 'Run C68 controlled production catalog activation execution review without deployment or PLAN/CONFIRM mutation.';

    private WatchlistBacktestC68ProductionCatalogActivationExecutionReviewService $service;

    public function __construct(?WatchlistBacktestC68ProductionCatalogActivationExecutionReviewService $service = null)
    {
        parent::__construct();
        $this->service = $service ?: new WatchlistBacktestC68ProductionCatalogActivationExecutionReviewService();
    }

    public function handle(): int
    {
        if ((bool) $this->option('progress')) {
            $this->line('C68 production catalog activation execution review started');
        }
        ...
        foreach ([
            ...
            'production_catalog_activation_execution_review_executed', 'production_catalog_activation_execution_review_pass',
        ...
        foreach ((array) ($result['production_catalog_activation_execution_decision'] ?? []) as $key => $value) {
            if (! is_array($value)) {
                $this->line('production_catalog_activation_execution_'.$key.'='.$this->scalar($value));
            }
        }

        foreach ((array) ($result['c69_readiness_decision'] ?? []) as $key => $value) {
            if (! is_array($value)) {
                $this->line('c69_'.$key.'='.$this->scalar($value));
            }
        }

        if (strpos((string) ($result['status'] ?? ''), 'C68_PRODUCTION_CATALOG_ACTIVATION_EXECUTION_REVIEW_PASSED') === 0) {
            if ((bool) $this->option('progress')) {
                $this->line('C68 production catalog activation execution review completed');
            }
            return 0;
        }
