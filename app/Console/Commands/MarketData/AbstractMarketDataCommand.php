...
abstract class AbstractMarketDataCommand extends Command
{
    ...
    protected function renderRunSummary($run, array $sourceContext = null)
    {
        ...
        $this->renderMutationImpactSummary($run);
    ...
    protected function buildRunSummaryPayload($run, array $overrides = [], array $sourceContext = null)
    {
        ...
        $payload = array_merge($payload, $this->mutationImpactPayloadFromNotes($notesMap));
        ...
        foreach ([
            ...
            if (array_key_exists($field, $sourceContext) && $sourceContext[$field] !== null && $sourceContext[$field] !== '') {
                $payload[$field] = $sourceContext[$field];
            }
        }
    ...
    protected function renderMutationImpactSummary($run)
    {
        $notes = $this->parseRunNotes((string) $this->runField($run, 'notes', ''));
        $payload = $this->mutationImpactPayloadFromNotes($notes);
    ...
    protected function mutationImpactPayloadFromNotes(array $notes)
    {
        $payload = [];
        foreach ([
            'bar_mutation_changed_count',
        ...
        $payload['publication_reprocess_summary'] = [
            ...
            'readable_correction_candidate_trade_dates' => $this->parseCsvList($payload['publication_reprocess_readable_correction_candidate_trade_dates'] ?? ''),
            ...
            'correction_ids' => $this->parseCsvList($payload['publication_reprocess_correction_ids'] ?? ''),
            'correction_id' => isset($payload['publication_reprocess_correction_id']) ? (int) $payload['publication_reprocess_correction_id'] : null,
        ];
    ...
    protected function shouldRecoverSourceTelemetry(array $sourceContext)
    {
        ...
        foreach (['source_mode', 'provider', 'timeout_seconds', 'retry_max', 'attempt_count', 'success_after_retry', 'final_http_status', 'final_reason_code', 'retry_exhausted', 'source_final_status', 'source_priority', 'active_source_decision', 'retry_attempt_count', 'failure_class_summary', 'source_protection_state', 'circuit_breaker_open'] as $key) {
            if (array_key_exists($key, $sourceContext) && $sourceContext[$key] !== null && $sourceContext[$key] !== '') {
        ...
        foreach (['source_mode', 'provider', 'source_priority', 'active_source_decision', 'retry_attempt_count', 'failure_class_summary', 'timeout_seconds', 'retry_max', 'attempt_count', 'final_reason_code', 'retry_exhausted', 'source_final_status', 'source_attempt_event_type', 'source_attempt_count'] as $key) {
            if (! array_key_exists($key, $sourceContext) || $sourceContext[$key] === null || $sourceContext[$key] === '') {
                return true;
            }
        }
    ...
    protected function decodeJsonObject($value)
    {
        if ($value === null || trim((string) $value) === '') {
            return [];
        }
