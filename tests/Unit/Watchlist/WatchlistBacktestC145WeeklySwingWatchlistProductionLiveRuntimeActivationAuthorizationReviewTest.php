...
class WatchlistBacktestC145WeeklySwingWatchlistProductionLiveRuntimeActivationAuthorizationReviewTest extends TestCase
{
    private const C144_ARTIFACT = 'storage/app/watchlist/backtest/c144-weekly-swing-watchlist-production-live-runtime-activation-pre-activation-boundary-review.json';
    private const C144_HASH = '68d5bb7d096b09d1defa3a655313ff0a7f658e84';
    private const C144_SHA1 = 'FBC618728E9A8B49A5FBD5CE273EF2159705C816';
    ...
    private const TEMP_NEGATIVE_STATUS = 'C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_TEMPORARY_NEGATIVE_ARTIFACT_REMAINS';
    private const NEXT_C146 = 'C146_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_EXECUTION_REVIEW';
    ...
    {
        parent::setUp();
        $this->output = 'storage/app/watchlist/backtest/.tmp-c145-production-live-runtime-activation-authorization-pass.json';
        $this->cleanupC145TemporaryArtifacts();
        @unlink($this->output);
    }
    ...
    public function test_c145_passes_with_valid_c144_lock_operator_approval_and_authorization_confirmation(): void
    {
        $result = $this->runService();

        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW', $result['run_code']);
        $this->assertSame('PR-33 / C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW', $result['phase_label']);
        ...
        $this->assertTrue($result['c145_activation_authorization_review_only']);
        $this->assertTrue($result['c145_not_activation_execution']);
        $this->assertTrue($result['c145_not_live_runtime_state_change']);
        $this->assertSame(self::NEXT_C146, $result['next_step_recommendation']);
        $this->assertFileExists($this->output);
    }

    public function test_c145_rejects_missing_operator_approval_or_reference(): void
    {
    ...
    public function test_c145_rejects_unconfirmed_activation_authorization(): void
    {
    ...
    public function test_c145_rejects_missing_or_mismatched_c144_artifact_lock(): void
    {
        $missing = $this->runService([
            'c144Artifact' => 'storage/app/watchlist/backtest/missing-c144-for-c145.json',
            'expectedC144Hash' => 'missing',
            'expectedC144FileSha1' => 'missing',
        ]);
        $hashMismatch = $this->runService(['expectedC144Hash' => 'bad-hash']);
        $shaMismatch = $this->runService(['expectedC144FileSha1' => 'BADSHA1']);

        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C144_ARTIFACT_LOCK_MISMATCH', $missing['status']);
        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C144_ARTIFACT_LOCK_MISMATCH', $hashMismatch['status']);
        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C144_FILE_SHA1_LOCK_MISMATCH', $shaMismatch['status']);
    }

    public function test_c145_rejects_c144_status_phase_or_next_recommendation_mismatch(): void
    {
        $status = $this->mutateC144AndExecute(function (array $c144): array {
            $c144['status'] = 'BROKEN_STATUS';
            return $c144;
        }, 'status-broken');
        $phase = $this->mutateC144AndExecute(function (array $c144): array {
            $c144['phase_label'] = 'BROKEN_PHASE';
            return $c144;
        }, 'phase-broken');
        $next = $this->mutateC144AndExecute(function (array $c144): array {
            $c144['next_step_recommendation'] = 'BROKEN_NEXT';
            $c144['next_activation_authorization_decision']['next_recommendation'] = 'BROKEN_NEXT';
            $c144['planned_next_summary']['planned_next_review'] = 'BROKEN_NEXT';
            return $c144;
        }, 'next-broken');

        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C144_STATUS_MISMATCH', $status['status']);
        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C144_PHASE_LABEL_MISMATCH', $phase['status']);
        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C144_NEXT_RECOMMENDATION_MISMATCH', $next['status']);
    }

    public function test_c145_rejects_c144_convert_from_json_duplicate_top_level_keys(): void
    {
        $raw = (string) file_get_contents(self::C144_ARTIFACT);
        $path = 'storage/app/watchlist/backtest/.tmp-c145-source-c144-duplicate-key.json';
        $this->tmpFiles[] = $path;
        ...
        $result = $this->runService([
            'c144Artifact' => $path,
            'expectedC144Hash' => self::C144_HASH,
            'expectedC144FileSha1' => strtoupper(sha1((string) file_get_contents($path))),
        ]);

        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C144_CONVERT_FROM_JSON_COMPATIBILITY_VIOLATION', $result['status']);
        $this->assertFalse($result['c144_convert_from_json_pass']);
    }

    /**
     * @dataProvider c144PreActivationBoundaryMismatchProvider
     */
    public function test_c145_rejects_c144_pre_activation_boundary_state_mismatch(string $field, $value): void
    {
        $result = $this->mutateC144AndExecute(function (array $c144) use ($field, $value): array {
            $this->setValueAt($c144, explode('.', $field), $value);
            return $c144;
        }, 'boundary-'.str_replace('.', '-', $field));

        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_C144_PRE_ACTIVATION_BOUNDARY_INVALID', $result['status'], $field);
    }

    public function c144PreActivationBoundaryMismatchProvider(): array
    {
        return [
            ...
            ['c144_pre_activation_boundary_decision.review_pass', false],
            ...
            ['c144_not_live_runtime_state_change', false],
    ...
    public function test_c145_rejects_candidate_scope_change_or_a01_promotion(): void
    {
        $candidate = $this->mutateC144AndExecute(function (array $c144): array {
            $c144['primary_candidate_code'] = 'BROKEN_PRIMARY';
            return $c144;
        }, 'candidate-primary');
        $a01 = $this->mutateC144AndExecute(function (array $c144): array {
            $c144['a01_promoted'] = true;
            return $c144;
        }, 'candidate-a01');

        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH', $candidate['status']);
        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH', $a01['status']);
    }

    public function test_c145_rejects_live_or_mutating_safety_flag_true_in_c144(): void
    {
        ...
        $result = $this->mutateC144AndExecute(function (array $c144) use ($field): array {
            $c144[$field] = true;
            return $c144;
        }, 'safety-pre-activation-context');

        $this->assertSame('C145_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_REJECTED_LIVE_OR_PRODUCTION_MUTATION_ATTEMPT', $result['status']);
        $this->assertSame($field, $result['c144_live_or_mutating_safety_flag_failure']);
    }

    public function test_c145_rejects_temporary_negative_artifact_remaining(): void
    {
        $path = 'storage/app/watchlist/backtest/c145-no-temporary-artifact-test.json';
        $this->tmpFiles[] = $path;
    ...
    public function test_c145_records_source_locks_manifest_checklist_and_no_live_output(): void
    {
        $result = $this->runService();
        $run = $this->readOutput();
        ...
        $this->assertSame(self::C144_HASH, $result['expected_c144_hash']);
        $this->assertSame(self::C144_HASH, $result['actual_c144_hash']);
        $this->assertTrue($result['c144_hash_match']);
        $this->assertSame(self::C144_SHA1, $result['expected_c144_file_sha1']);
        $this->assertSame(self::C144_SHA1, $result['actual_c144_file_sha1']);
        $this->assertTrue($result['c144_file_sha1_match']);
        $this->assertTrue($result['c144_convert_from_json_pass']);
        $this->assertTrue($result['c144_pre_activation_boundary_carry_forward_summary']['c143_go_decision_finalization_valid']);
        $this->assertTrue($result['c144_pre_activation_boundary_carry_forward_summary']['c142_activation_operator_go_no_go_valid']);
        $this->assertTrue($result['c144_pre_activation_boundary_carry_forward_summary']['c141_activation_observation_result_review_valid']);
        $this->assertTrue($result['c144_pre_activation_boundary_carry_forward_summary']['c138_activation_authorization_valid']);
        $this->assertSame(self::NEXT_C146, $result['next_activation_execution_decision']['next_recommendation']);

        foreach ([
            'source_artifact_locks',
            'c144_lock_validation_summary',
            'c144_pre_activation_boundary_carry_forward_summary',
            'candidate_scope_freeze_summary',
            'operator_approval_validation_summary',
            'temporary_negative_artifact_guard_summary',
            'feature_flag_default_off_summary',
            'c145_activation_authorization_decision',
            ...
            'c145_candidate_activation_execution_readiness_scorecard',
    ...
    public function test_c145_keeps_e02_primary_b01_backup_a01_comparator_and_all_safety_flags_false(): void
    {
        $result = $this->runService();
        $scorecard = $result['c145_candidate_activation_execution_readiness_scorecard'];
    ...
    public function test_c145_output_is_deterministic_enough_for_artifact_hash_validation(): void
    {
        $first = $this->runService(['createdAt' => '2026-07-14T00:00:00+00:00']);
        $secondOutput = 'storage/app/watchlist/backtest/.tmp-c145-production-live-runtime-activation-authorization-pass-second.json';
    ...
    public function test_c145_does_not_mutate_c144_artifact(): void
    {
        $before = strtoupper(sha1((string) file_get_contents(self::C144_ARTIFACT)));

        $this->runService();

        $this->assertSame($before, strtoupper(sha1((string) file_get_contents(self::C144_ARTIFACT))));
    }

    private function runService(array $options = []): array
    {
        $service = new WatchlistBacktestC145WeeklySwingWatchlistProductionLiveRuntimeActivationAuthorizationReviewService();

        return $service->execute(
            (string) ($options['c144Artifact'] ?? self::C144_ARTIFACT),
            (string) ($options['expectedC144Hash'] ?? self::C144_HASH),
            (string) ($options['expectedC144FileSha1'] ?? self::C144_SHA1),
            ...
            [
                ...
                'approval_reference' => (string) ($options['approvalReference'] ?? 'C145_OPERATOR_APPROVED_PRODUCTION_LIVE_RUNTIME_ACTIVATION_AUTHORIZATION_REVIEW_ONLY'),
                'overwrite' => true,
    ...
    private function mutateC144AndExecute(callable $mutator, string $name): array
    {
        $c144 = json_decode((string) file_get_contents(self::C144_ARTIFACT), true);
        $c144 = $mutator(is_array($c144) ? $c144 : []);
        $path = 'storage/app/watchlist/backtest/.tmp-c145-source-c144-'.$name.'.json';
        $this->tmpFiles[] = $path;
        file_put_contents($path, json_encode($c144, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE).PHP_EOL);

        return $this->runService([
            'c144Artifact' => $path,
            'expectedC144Hash' => (string) ($c144['artifact_hash'] ?? ''),
            'expectedC144FileSha1' => strtoupper(sha1((string) file_get_contents($path))),
        ]);
    }
    ...
    private function cleanupC145TemporaryArtifacts(): void
    {
        foreach ((array) glob('storage/app/watchlist/backtest/c145-*test.json') as $file) {
            if (is_file($file)) {
                unlink($file);
            }
        }
        foreach ((array) glob('storage/app/watchlist/backtest/.tmp-c145*.json') as $file) {
            if (is_file($file)) {
                unlink($file);
            }
        }
