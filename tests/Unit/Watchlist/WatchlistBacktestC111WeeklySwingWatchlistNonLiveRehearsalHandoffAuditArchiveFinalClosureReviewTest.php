...
class WatchlistBacktestC111WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveFinalClosureReviewTest extends TestCase
{
    ...
    private const TEMP_NEGATIVE_STATUS = 'C111_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_HANDOFF_AUDIT_ARCHIVE_FINAL_CLOSURE_REVIEW_REJECTED_TEMPORARY_NEGATIVE_ARTIFACT_REMAINS';
    private const NO_NEXT = 'NO_NEXT_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_HANDOFF_AUDIT_ARCHIVE_REVIEW_REQUIRED';
    private const C110_HASH = '17352f926bcf9138be62c9f43a81551f89de0cc7';
    private const C110_SHA1 = '407DB31435BF42C48FD0C7419B7BEBCA138DB127';
    ...
    {
        parent::setUp();
        $this->output = 'storage/app/watchlist/backtest/.tmp-c111-handoff-audit-archive-final-closure-pass.json';
        @unlink($this->output);
    }
    ...
    public function test_c111_passes_with_valid_c110_audit_archive_completion_lock_operator_approval_and_reference(): void
    {
        ...
        $this->assertTrue($result['c110_handoff_audit_archived']);
        ...
        $this->assertTrue($result['weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_final_closure_review_executed']);
        $this->assertTrue($result['weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_final_closure_review_allowed']);
        $this->assertTrue($result['weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_final_closure_review_pass']);
        $this->assertTrue($result['weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_final_closed']);
        ...
        $this->assertSame('C111_HANDOFF_AUDIT_ARCHIVE_FINAL_CLOSED_NON_LIVE_ONLY', $result['diagnostic_conclusion']);
        $this->assertFileExists($this->output);
    }

    public function test_c111_rejects_missing_operator_approval(): void
    {
    ...
    public function test_c111_rejects_missing_approval_reference(): void
    {
    ...
    public function test_c111_rejects_missing_or_mismatched_c110_artifact_lock(): void
    {
        $missing = $this->execute([
            'c110Artifact' => 'storage/app/watchlist/backtest/c111-source-does-not-exist.json',
            'expectedC110Hash' => 'missing',
            'expectedC110FileSha1' => 'missing',
        ]);
        $hashMismatch = $this->execute(['expectedC110Hash' => 'bad-hash']);
        $shaMismatch = $this->execute(['expectedC110FileSha1' => 'BADSHA1']);

        $this->assertSame('C111_BLOCKED_C110_ARTIFACT_LOCK_MISMATCH', $missing['status']);
        $this->assertSame('C111_BLOCKED_C110_ARTIFACT_LOCK_MISMATCH', $hashMismatch['status']);
        $this->assertSame('C111_BLOCKED_C110_FILE_SHA1_LOCK_MISMATCH', $shaMismatch['status']);
    }

    public function test_c111_rejects_c110_status_mismatch(): void
    {
        $result = $this->mutateC110AndExecute(function (array $c110): array {
            $c110['status'] = 'BROKEN_STATUS';
            return $c110;
        }, 'status-broken');

        $this->assertSame('C111_BLOCKED_C110_STATUS_MISMATCH', $result['status']);
    }

    public function test_c111_rejects_c110_reason_code_mismatch(): void
    {
        $result = $this->mutateC110AndExecute(function (array $c110): array {
            $c110['reason_code'] = 'BROKEN_REASON';
            return $c110;
        }, 'reason-broken');

        $this->assertSame('C111_BLOCKED_C110_REASON_CODE_MISMATCH', $result['status']);
    }

    public function test_c111_rejects_c110_next_recommendation_mismatch(): void
    {
        $result = $this->mutateC110AndExecute(function (array $c110): array {
            $c110['next_step_recommendation'] = 'BROKEN_NEXT';
            $c110['next_readiness_decision']['next_recommendation'] = 'BROKEN_NEXT';
            $c110['c110_readiness_decision']['next_recommendation'] = 'BROKEN_NEXT';
            $c110['weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_completion_decision']['next_recommendation'] = 'BROKEN_NEXT';
            $c110['weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_completion_seal_decision']['next_recommendation'] = 'BROKEN_NEXT';
            return $c110;
        }, 'next-broken');

        $this->assertSame('C111_BLOCKED_C110_NEXT_RECOMMENDATION_MISMATCH', $result['status']);
    }

    /**
     * @dataProvider c110AuditArchiveStateMismatchProvider
     */
    public function test_c111_rejects_c110_audit_archive_state_mismatch(string $field, $value): void
    {
        $result = $this->mutateC110AndExecute(function (array $c110) use ($field, $value): array {
            $c110[$field] = $value;
            return $c110;
        }, 'state-'.$field);

        $this->assertSame('C111_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_HANDOFF_AUDIT_ARCHIVE_FINAL_CLOSURE_REVIEW_REJECTED_C110_AUDIT_ARCHIVE_COMPLETION_SEAL_INCOMPLETE', $result['status'], $field);
    }

    public function c110AuditArchiveStateMismatchProvider(): array
    {
        return [
    ...
    public function test_c111_rejects_candidate_scope_change_or_a01_promotion(): void
    {
        $result = $this->mutateC110AndExecute(function (array $c110): array {
            $c110['primary_candidate_code'] = 'BROKEN_PRIMARY';
            return $c110;
        }, 'candidate-primary-broken');
        $a01 = $this->mutateC110AndExecute(function (array $c110): array {
            $c110['a01_promoted'] = true;
            return $c110;
        }, 'candidate-a01-promoted');

        $this->assertSame('C111_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_HANDOFF_AUDIT_ARCHIVE_FINAL_CLOSURE_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH', $result['status']);
        $this->assertSame('C111_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_HANDOFF_AUDIT_ARCHIVE_FINAL_CLOSURE_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH', $a01['status']);
    }

    /**
     * @dataProvider safetyFlagProvider
     */
    public function test_c111_rejects_any_live_or_mutating_safety_flag_true_in_c110(string $field): void
    {
        $result = $this->mutateC110AndExecute(function (array $c110) use ($field): array {
            $c110[$field] = true;
            return $c110;
        }, 'safety-'.$field);

        $this->assertSame('C111_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_HANDOFF_AUDIT_ARCHIVE_FINAL_CLOSURE_REVIEW_REJECTED_LIVE_OR_PRODUCTION_MUTATION_ATTEMPT', $result['status'], $field);
        $this->assertSame($field, $result['c110_live_or_mutating_safety_flag_failure']);
    }
    ...
    public function test_c111_rejects_temporary_negative_artifact_remaining(): void
    {
        $path = 'storage/app/watchlist/backtest/c111-no-temporary-artifact-test.json';
        $this->tmpFiles[] = $path;
    ...
    public function test_c111_records_artifact_hash_source_locks_next_and_required_sections(): void
    {
        $result = $this->runService();
        $run = $this->readOutput();

        $this->assertMatchesRegularExpression('/^[a-f0-9]{40}$/', $result['artifact_hash']);
        $this->assertSame(self::C110_HASH, $result['expected_c110_hash']);
        $this->assertSame(self::C110_HASH, $result['actual_c110_hash']);
        $this->assertTrue($result['c110_hash_match']);
        $this->assertSame(self::C110_SHA1, $result['expected_c110_file_sha1']);
        $this->assertSame(self::C110_SHA1, $result['actual_c110_file_sha1']);
        $this->assertTrue($result['c110_file_sha1_match']);
        $this->assertSame(self::NO_NEXT, $result['next_readiness_decision']['next_recommendation']);

        foreach ([
            'source_artifact_locks',
            'c110_lock_validation_summary',
            'c104_c110_handoff_lineage_final_closure_summary',
            'candidate_scope_freeze_summary',
            'operator_approval_validation_summary',
            'temporary_negative_artifact_guard_summary',
            'c111_readiness_decision',
            'next_readiness_decision',
            'weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_final_closure_decision',
            'weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_final_closure_manifest',
            'c111_candidate_audit_archive_final_closure_scorecard',
            ...
            'c110_handoff_audit_archive_completion_seal_carry_forward_validation_summary',
    ...
    public function test_c111_keeps_e02_primary_b01_backup_a01_comparator_and_no_live_output(): void
    {
        $result = $this->runService();
        $manifest = $result['weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_final_closure_manifest'];
    ...
    public function test_c111_keeps_all_safety_flags_false(): void
    {
        $result = $this->runService();
        ...
        $this->assertFalse($result['c111_readiness_decision']['production_ready']);
        $this->assertFalse($result['c111_readiness_decision']['weekly_swing_watchlist_live_output_enabled']);
    ...
    public function test_c111_writes_final_closure_manifest_artifact_only_and_not_used_for_plan_confirm(): void
    {
        $result = $this->runService();
        $manifest = $result['weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_final_closure_manifest'];

        $this->assertTrue($manifest['manifest_created']);
        $this->assertSame('artifact_only_non_live_handoff_audit_archive_final_closure_review', $manifest['manifest_context']);
        $this->assertSame('C110_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_HANDOFF_AUDIT_ARCHIVE_COMPLETION_SEAL_REVIEW', $manifest['source_artifact']);
        ...
        $this->assertTrue($manifest['handoff_audit_archive_completion_sealed_carried_forward']);
        $this->assertTrue($manifest['handoff_audit_archive_final_closed']);
        $this->assertFalse($manifest['final_closure_used_for_selection']);
        $this->assertFalse($manifest['final_closure_used_for_retuning']);
        $this->assertFalse($manifest['final_closure_used_for_ranking']);
        $this->assertFalse($manifest['final_closure_used_for_plan_confirm_mutation']);
    }

    public function test_c111_output_is_deterministic_enough_for_artifact_hash_validation(): void
    {
        $first = $this->execute(['options' => ['created_at' => '2026-06-30T00:00:00+00:00']]);
        $secondOutput = 'storage/app/watchlist/backtest/.tmp-c111-handoff-audit-archive-final-closure-pass-second.json';
    ...
    {
        $service = new WatchlistBacktestC111WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveFinalClosureReviewService();
        $fixture = $this->lockedC110Fixture();
        return $service->execute(
            (string) ($overrides['c110Artifact'] ?? $fixture['path']),
            (string) ($overrides['expectedC110Hash'] ?? $fixture['hash']),
            (string) ($overrides['expectedC110FileSha1'] ?? $fixture['sha1']),
            (string) ($overrides['output'] ?? $this->output),
            array_merge([
                'overwrite' => true,
                'operator_approved' => $overrides['operatorApproved'] ?? true,
                'approval_reference' => $overrides['approvalReference'] ?? 'C111_OPERATOR_APPROVED_HANDOFF_AUDIT_ARCHIVE_FINAL_CLOSURE_REVIEW_ONLY',
            ], (array) ($overrides['options'] ?? []))
        );
    }

    private function mutateC110AndExecute(callable $mutator, string $name): array
    {
        $fixture = $this->lockedC110Fixture();
        $payload = json_decode((string) file_get_contents($fixture['path']), true);
        $payload = $mutator(is_array($payload) ? $payload : []);
        $path = 'storage/app/watchlist/backtest/.tmp-c111-source-'.$name.'.json';
        $this->tmpFiles[] = $path;
        file_put_contents($path, json_encode($payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE).PHP_EOL);
        return $this->execute([
            'c110Artifact' => $path,
            'expectedC110Hash' => (string) ($payload['artifact_hash'] ?? ''),
            'expectedC110FileSha1' => strtoupper(sha1_file($path)),
        ]);
    }

    private function lockedC110Fixture(): array
    {
        $path = 'storage/app/watchlist/backtest/c110-weekly-swing-watchlist-non-live-rehearsal-handoff-audit-archive-completion-seal-review.json';
