...
class WatchlistBacktestC147WeeklySwingWatchlistProductionLiveRuntimeActivationObservationReviewTest extends TestCase
{
    private const C146_ARTIFACT = 'storage/app/watchlist/backtest/c146-weekly-swing-watchlist-production-live-runtime-activation-execution-review.json';
    private const C146_HASH = 'ff6549aa99b2488ce52184dd818190b124e480ce';
    private const C146_SHA1 = '1291AADFB2CC7691D868AD86604731C2F6F5D9F2';
    ...
    private const TEMP_NEGATIVE_STATUS = 'C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_TEMPORARY_NEGATIVE_ARTIFACT_REMAINS';
    private const NEXT_C148 = 'C148_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_RESULT_REVIEW';
    ...
    {
        parent::setUp();
        $this->output = 'storage/app/watchlist/backtest/.tmp-c147-production-live-runtime-activation-observation-pass.json';
        $this->cleanupC147TemporaryArtifacts();
        @unlink($this->output);
    }
    ...
    public function test_c147_passes_with_valid_c146_activation_execution_review_lock_and_operator_approval(): void
    {
        $result = $this->runService();

        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW', $result['run_code']);
        $this->assertSame('PR-35 / C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW', $result['phase_label']);
        ...
        $this->assertTrue($result['c147_observation_review_only']);
        $this->assertTrue($result['c147_not_live_runtime_state_change']);
        $this->assertSame(self::NEXT_C148, $result['next_step_recommendation']);
        $this->assertFileExists($this->output);
    }

    public function test_c147_rejects_missing_operator_approval_or_reference(): void
    {
    ...
    public function test_c147_rejects_missing_or_mismatched_c146_artifact_lock(): void
    {
        $missing = $this->runService([
            'c146Artifact' => 'storage/app/watchlist/backtest/missing-c146-for-c147.json',
            'expectedC146Hash' => 'missing',
            'expectedC146FileSha1' => 'missing',
        ]);
        $hashMismatch = $this->runService(['expectedC146Hash' => 'bad-hash']);
        $shaMismatch = $this->runService(['expectedC146FileSha1' => 'BADSHA1']);

        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_C146_ARTIFACT_LOCK_MISMATCH', $missing['status']);
        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_C146_ARTIFACT_LOCK_MISMATCH', $hashMismatch['status']);
        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_C146_FILE_SHA1_LOCK_MISMATCH', $shaMismatch['status']);
    }

    public function test_c147_rejects_c146_status_phase_or_next_recommendation_mismatch(): void
    {
        $status = $this->mutateC146AndExecute(function (array $c146): array {
            $c146['status'] = 'BROKEN_STATUS';
            return $c146;
        }, 'status-broken');
        $phase = $this->mutateC146AndExecute(function (array $c146): array {
            $c146['phase_label'] = 'BROKEN_PHASE';
            return $c146;
        }, 'phase-broken');
        $next = $this->mutateC146AndExecute(function (array $c146): array {
            $c146['next_step_recommendation'] = 'BROKEN_NEXT';
            $c146['next_observation_decision']['next_recommendation'] = 'BROKEN_NEXT';
            return $c146;
        }, 'next-broken');

        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_C146_STATUS_MISMATCH', $status['status']);
        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_C146_PHASE_LABEL_MISMATCH', $phase['status']);
        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_C146_NEXT_RECOMMENDATION_MISMATCH', $next['status']);
    }

    public function test_c147_rejects_c146_convert_from_json_duplicate_top_level_keys(): void
    {
        $raw = (string) file_get_contents(self::C146_ARTIFACT);
        $path = 'storage/app/watchlist/backtest/.tmp-c147-source-c146-duplicate-key.json';
        $this->tmpFiles[] = $path;
        ...
        $result = $this->runService([
            'c146Artifact' => $path,
            'expectedC146Hash' => self::C146_HASH,
            'expectedC146FileSha1' => strtoupper(sha1((string) file_get_contents($path))),
        ]);

        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_C146_CONVERT_FROM_JSON_COMPATIBILITY_VIOLATION', $result['status']);
        $this->assertFalse($result['c146_convert_from_json_pass']);
    }

    /**
     * @dataProvider c146ExecutionReviewMismatchProvider
     */
    public function test_c147_rejects_c146_activation_execution_review_state_mismatch(string $field, $value): void
    {
        $result = $this->mutateC146AndExecute(function (array $c146) use ($field, $value): array {
            $c146[$field] = $value;
            return $c146;
        }, 'execution-'.$field);

        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_C146_ACTIVATION_EXECUTION_REVIEW_INCOMPLETE', $result['status'], $field);
    }

    public function c146ExecutionReviewMismatchProvider(): array
    {
        return [
    ...
    public function test_c147_rejects_candidate_scope_change_or_a01_promotion(): void
    {
        $candidate = $this->mutateC146AndExecute(function (array $c146): array {
            $c146['primary_candidate_code'] = 'BROKEN_PRIMARY';
            return $c146;
        }, 'candidate-primary');
        $a01 = $this->mutateC146AndExecute(function (array $c146): array {
            $c146['a01_promoted'] = true;
            return $c146;
        }, 'candidate-a01');

        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH', $candidate['status']);
        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH', $a01['status']);
    }

    public function test_c147_rejects_live_or_mutating_safety_flag_true_in_c146(): void
    {
        $field = 'weekly_swing_watchlist_production_live_runtime_activation_observation_context_persisted_to_live_runtime';
        $result = $this->mutateC146AndExecute(function (array $c146) use ($field): array {
            $c146[$field] = true;
            return $c146;
        }, 'safety-observation-context');

        $this->assertSame('C147_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_REJECTED_LIVE_OR_PRODUCTION_MUTATION_ATTEMPT', $result['status']);
        $this->assertSame($field, $result['c146_live_or_mutating_safety_flag_failure']);
    }

    public function test_c147_rejects_temporary_negative_artifact_remaining(): void
    {
        $path = 'storage/app/watchlist/backtest/c147-no-temporary-artifact-test.json';
        $this->tmpFiles[] = $path;
    ...
    public function test_c147_records_source_locks_manifest_checklist_and_no_live_output(): void
    {
        $result = $this->runService();
        $run = $this->readOutput();
        ...
        $carry = $result['c146_activation_execution_review_carry_forward_summary'];

        $this->assertMatchesRegularExpression('/^[a-f0-9]{40}$/', $result['artifact_hash']);
        $this->assertSame(self::C146_HASH, $result['expected_c146_hash']);
        $this->assertSame(self::C146_HASH, $result['actual_c146_hash']);
        $this->assertTrue($result['c146_hash_match']);
        $this->assertSame(self::C146_SHA1, $result['expected_c146_file_sha1']);
        $this->assertSame(self::C146_SHA1, $result['actual_c146_file_sha1']);
        $this->assertTrue($result['c146_file_sha1_match']);
        $this->assertTrue($result['c146_convert_from_json_pass']);
        $this->assertSame(self::NEXT_C148, $result['next_observation_result_decision']['next_recommendation']);
        ...
        foreach ([
            ...
            'c146_lock_validation_summary',
            'c146_activation_execution_review_carry_forward_summary',
            'candidate_scope_freeze_summary',
            'operator_approval_validation_summary',
            'temporary_negative_artifact_guard_summary',
            'feature_flag_default_off_summary',
            'c147_observation_decision',
            ...
            'c147_candidate_activation_observation_scorecard',
    ...
    public function test_c147_keeps_e02_primary_b01_backup_a01_comparator_and_all_safety_flags_false(): void
    {
        $result = $this->runService();
    ...
    public function test_c147_output_is_deterministic_enough_for_artifact_hash_validation(): void
    {
        $first = $this->runService(['createdAt' => '2026-07-05T00:00:00+00:00']);
        $secondOutput = 'storage/app/watchlist/backtest/.tmp-c147-production-live-runtime-activation-observation-pass-second.json';
    ...
    public function test_c147_does_not_mutate_c146_artifact(): void
    {
        $before = strtoupper(sha1((string) file_get_contents(self::C146_ARTIFACT)));

        $this->runService();

        $this->assertSame($before, strtoupper(sha1((string) file_get_contents(self::C146_ARTIFACT))));
    }

    private function runService(array $options = []): array
    {
        $service = new WatchlistBacktestC147WeeklySwingWatchlistProductionLiveRuntimeActivationObservationReviewService();

        return $service->execute(
            (string) ($options['c146Artifact'] ?? self::C146_ARTIFACT),
            (string) ($options['expectedC146Hash'] ?? self::C146_HASH),
            (string) ($options['expectedC146FileSha1'] ?? self::C146_SHA1),
            (string) ($options['output'] ?? $this->output),
            [
                'operator_approved' => (bool) ($options['operatorApproved'] ?? true),
                'approval_reference' => (string) ($options['approvalReference'] ?? 'C147_OPERATOR_APPROVED_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OBSERVATION_REVIEW_ONLY'),
                'overwrite' => true,
    ...
    private function mutateC146AndExecute(callable $mutator, string $name): array
    {
        $c146 = json_decode((string) file_get_contents(self::C146_ARTIFACT), true);
        $c146 = $mutator(is_array($c146) ? $c146 : []);
        $path = 'storage/app/watchlist/backtest/.tmp-c147-source-c146-'.$name.'.json';
        $this->tmpFiles[] = $path;
        file_put_contents($path, json_encode($c146, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE).PHP_EOL);

        return $this->runService([
            'c146Artifact' => $path,
            'expectedC146Hash' => (string) ($c146['artifact_hash'] ?? ''),
            'expectedC146FileSha1' => strtoupper(sha1((string) file_get_contents($path))),
        ]);
    }
    ...
    private function cleanupC147TemporaryArtifacts(): void
    {
        foreach ((array) glob('storage/app/watchlist/backtest/c147-*test.json') as $file) {
            if (is_file($file)) {
                unlink($file);
            }
        }
        foreach ((array) glob('storage/app/watchlist/backtest/.tmp-c147*.json') as $file) {
            if (is_file($file)) {
                unlink($file);
            }
        }
