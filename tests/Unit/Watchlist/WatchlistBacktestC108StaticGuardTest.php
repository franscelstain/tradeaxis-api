...
class WatchlistBacktestC108StaticGuardTest extends TestCase
{
    public function test_c108_command_is_registered(): void
    {
        $kernel = (string) file_get_contents('app/Console/Kernel.php');

        $this->assertStringContainsString('RunBacktestC108WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveReviewCommand::class', $kernel);
        $this->assertStringContainsString('use App\\Console\\Commands\\Watchlist\\RunBacktestC108WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveReviewCommand;', $kernel);
    }

    public function test_c108_runtime_has_no_forbidden_date_or_future_selection_shortcuts(): void
    {
        $service = (string) file_get_contents('app/Application/Watchlist/Services/WatchlistBacktestC108WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveReviewService.php');
        $command = (string) file_get_contents('app/Console/Commands/Watchlist/RunBacktestC108WeeklySwingWatchlistNonLiveRehearsalHandoffAuditArchiveReviewCommand.php');
        $combined = $service."\n".$command;
    ...
    public function test_c108_docs_state_handoff_audit_archive_only_and_not_live_output(): void
    {
        $doc = (string) file_get_contents('docs/watchlist/audit/WS_C108_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_HANDOFF_AUDIT_ARCHIVE_REVIEW.md');
        $commands = (string) file_get_contents('docs/watchlist/audit/WS_C108_OPERATOR_VALIDATION_COMMANDS.md');
        ...
        foreach ([
            ...
            'storage/app/watchlist/backtest/c108-weekly-swing-watchlist-non-live-rehearsal-handoff-audit-archive-review.json',
        ] as $required) {
            $this->assertStringContainsString($required, $combined, $required);
        }
    }

    public function test_c108_config_defaults_are_off_and_no_production_runtime_wiring_is_created(): void
    {
        ...
        $this->assertStringContainsString('weekly_swing_watchlist_non_live_rehearsal_handoff_audit_archive_context_persisted_to_live_runtime', $combined);
        $this->assertStringContainsString('handoff_audit_archive_context_persisted_to_live_runtime', $combined);
        ...
        $this->assertStringContainsString('C108 weekly swing watchlist non-live rehearsal handoff audit archive review is not production deployment, not live rollout, not runtime bridge activation, not pilot/shadow runtime, not weekly swing live output, and not PLAN/CONFIRM mutation.', $combined);
    }
}
