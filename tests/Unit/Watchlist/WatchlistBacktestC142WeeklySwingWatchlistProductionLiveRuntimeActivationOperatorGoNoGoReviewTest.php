...
class WatchlistBacktestC142WeeklySwingWatchlistProductionLiveRuntimeActivationOperatorGoNoGoReviewTest extends TestCase
{
    private const C141_ARTIFACT = 'storage/app/watchlist/backtest/c141-weekly-swing-watchlist-production-live-runtime-activation-observation-result-review.json';
    private const C141_HASH = 'ea7c4be969c2faf9e4990a135503829b8f6d6518';
    private const C141_SHA1 = 'D9102B54D8719B40266AC8D4E9A0DF5B5BA5EB74';
    ...
    private const GO_NOT_CONFIRMED_STATUS = 'C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_GO_DECISION_NOT_CONFIRMED';
    private const TEMP_NEGATIVE_STATUS = 'C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_TEMPORARY_NEGATIVE_ARTIFACT_REMAINS';
    private const NEXT_C143 = 'C143_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_GO_DECISION_FINALIZATION_REVIEW';
    ...
    {
        parent::setUp();
        $this->output = 'storage/app/watchlist/backtest/.tmp-c142-production-live-runtime-activation-operator-go-no-go-pass.json';
        $this->cleanupC142TemporaryArtifacts();
        @unlink($this->output);
    }
    ...
    public function test_c142_passes_with_valid_c141_lock_operator_approval_and_go_confirmation(): void
    {
        $result = $this->runService();

        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW', $result['run_code']);
        $this->assertSame('PR-30 / C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW', $result['phase_label']);
        ...
        $this->assertTrue($result['c142_operator_go_no_go_review_only']);
        $this->assertTrue($result['c142_not_live_runtime_state_change']);
        $this->assertSame(self::NEXT_C143, $result['next_step_recommendation']);
        $this->assertFileExists($this->output);
    }

    public function test_c142_rejects_missing_operator_approval_or_reference(): void
    {
    ...
    public function test_c142_rejects_unconfirmed_operator_go_decision(): void
    {
    ...
    public function test_c142_rejects_missing_or_mismatched_c141_artifact_lock(): void
    {
        $missing = $this->runService([
            'c141Artifact' => 'storage/app/watchlist/backtest/missing-c141-for-c142.json',
            'expectedC141Hash' => 'missing',
            'expectedC141FileSha1' => 'missing',
        ]);
        $hashMismatch = $this->runService(['expectedC141Hash' => 'bad-hash']);
        $shaMismatch = $this->runService(['expectedC141FileSha1' => 'BADSHA1']);

        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_C141_ARTIFACT_LOCK_MISMATCH', $missing['status']);
        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_C141_ARTIFACT_LOCK_MISMATCH', $hashMismatch['status']);
        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_C141_FILE_SHA1_LOCK_MISMATCH', $shaMismatch['status']);
    }

    public function test_c142_rejects_c141_status_phase_or_next_recommendation_mismatch(): void
    {
        $status = $this->mutateC141AndExecute(function (array $c141): array {
            $c141['status'] = 'BROKEN_STATUS';
            return $c141;
        }, 'status-broken');
        $phase = $this->mutateC141AndExecute(function (array $c141): array {
            $c141['phase_label'] = 'BROKEN_PHASE';
            return $c141;
        }, 'phase-broken');
        $next = $this->mutateC141AndExecute(function (array $c141): array {
            $c141['next_step_recommendation'] = 'BROKEN_NEXT';
            $c141['next_operator_go_no_go_decision']['next_recommendation'] = 'BROKEN_NEXT';
            $c141['planned_next_summary']['planned_next_review'] = 'BROKEN_NEXT';
            return $c141;
        }, 'next-broken');

        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_C141_STATUS_MISMATCH', $status['status']);
        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_C141_PHASE_LABEL_MISMATCH', $phase['status']);
        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_C141_NEXT_RECOMMENDATION_MISMATCH', $next['status']);
    }

    public function test_c142_rejects_c141_convert_from_json_duplicate_top_level_keys(): void
    {
        $raw = (string) file_get_contents(self::C141_ARTIFACT);
        $path = 'storage/app/watchlist/backtest/.tmp-c142-source-c141-duplicate-key.json';
        $this->tmpFiles[] = $path;
        ...
        $result = $this->runService([
            'c141Artifact' => $path,
            'expectedC141Hash' => self::C141_HASH,
            'expectedC141FileSha1' => strtoupper(sha1((string) file_get_contents($path))),
        ]);

        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_C141_CONVERT_FROM_JSON_COMPATIBILITY_VIOLATION', $result['status']);
        $this->assertFalse($result['c141_convert_from_json_pass']);
    }

    /**
     * @dataProvider c141ObservationResultReviewMismatchProvider
     */
    public function test_c142_rejects_c141_activation_observation_result_review_state_mismatch(string $field, $value): void
    {
        $result = $this->mutateC141AndExecute(function (array $c141) use ($field, $value): array {
            $c141[$field] = $value;
            return $c141;
        }, 'observation-result-'.$field);

        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_C141_OBSERVATION_RESULT_REVIEW_INCOMPLETE', $result['status'], $field);
    }

    public function c141ObservationResultReviewMismatchProvider(): array
    {
        return [
    ...
    public function test_c142_rejects_candidate_scope_change_or_a01_promotion(): void
    {
        $candidate = $this->mutateC141AndExecute(function (array $c141): array {
            $c141['primary_candidate_code'] = 'BROKEN_PRIMARY';
            return $c141;
        }, 'candidate-primary');
        $a01 = $this->mutateC141AndExecute(function (array $c141): array {
            $c141['a01_promoted'] = true;
            return $c141;
        }, 'candidate-a01');

        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH', $candidate['status']);
        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_CANDIDATE_SCOPE_MISMATCH', $a01['status']);
    }

    public function test_c142_rejects_live_or_mutating_safety_flag_true_in_c141(): void
    {
        ...
        $result = $this->mutateC141AndExecute(function (array $c141) use ($field): array {
            $c141[$field] = true;
            return $c141;
        }, 'safety-observation-result-review-context');

        $this->assertSame('C142_WEEKLY_SWING_WATCHLIST_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_REJECTED_LIVE_OR_PRODUCTION_MUTATION_ATTEMPT', $result['status']);
        $this->assertSame($field, $result['c141_live_or_mutating_safety_flag_failure']);
    }

    public function test_c142_rejects_temporary_negative_artifact_remaining(): void
    {
        $path = 'storage/app/watchlist/backtest/c142-no-temporary-artifact-test.json';
        $this->tmpFiles[] = $path;
    ...
    public function test_c142_records_source_locks_manifest_checklist_and_no_live_output(): void
    {
        $result = $this->runService();
        $run = $this->readOutput();
        ...
        $this->assertSame(self::C141_HASH, $result['expected_c141_hash']);
        $this->assertSame(self::C141_HASH, $result['actual_c141_hash']);
        $this->assertTrue($result['c141_hash_match']);
        $this->assertSame(self::C141_SHA1, $result['expected_c141_file_sha1']);
        $this->assertSame(self::C141_SHA1, $result['actual_c141_file_sha1']);
        $this->assertTrue($result['c141_file_sha1_match']);
        $this->assertTrue($result['c141_convert_from_json_pass']);
        $this->assertSame(self::NEXT_C143, $result['next_go_decision_finalization_decision']['next_recommendation']);

        foreach ([
            'source_artifact_locks',
            'c141_lock_validation_summary',
            'c141_activation_observation_result_review_carry_forward_summary',
            'candidate_scope_freeze_summary',
            'operator_approval_validation_summary',
            'temporary_negative_artifact_guard_summary',
            'feature_flag_default_off_summary',
            'c142_operator_go_no_go_decision',
            ...
            'c142_candidate_activation_operator_go_no_go_scorecard',
    ...
    public function test_c142_keeps_e02_primary_b01_backup_a01_comparator_and_all_safety_flags_false(): void
    {
        $result = $this->runService();
        $scorecard = $result['c142_candidate_activation_operator_go_no_go_scorecard'];
    ...
    public function test_c142_output_is_deterministic_enough_for_artifact_hash_validation(): void
    {
        $first = $this->runService(['createdAt' => '2026-07-14T00:00:00+00:00']);
        $secondOutput = 'storage/app/watchlist/backtest/.tmp-c142-production-live-runtime-activation-operator-go-no-go-pass-second.json';
    ...
    public function test_c142_does_not_mutate_c141_artifact(): void
    {
        $before = strtoupper(sha1((string) file_get_contents(self::C141_ARTIFACT)));

        $this->runService();

        $this->assertSame($before, strtoupper(sha1((string) file_get_contents(self::C141_ARTIFACT))));
    }

    private function runService(array $options = []): array
    {
        $service = new WatchlistBacktestC142WeeklySwingWatchlistProductionLiveRuntimeActivationOperatorGoNoGoReviewService();

        return $service->execute(
            (string) ($options['c141Artifact'] ?? self::C141_ARTIFACT),
            (string) ($options['expectedC141Hash'] ?? self::C141_HASH),
            (string) ($options['expectedC141FileSha1'] ?? self::C141_SHA1),
            (string) ($options['output'] ?? $this->output),
            [
                'operator_approved' => (bool) ($options['operatorApproved'] ?? true),
                'operator_go_decision_confirmed' => (bool) ($options['operatorGoDecisionConfirmed'] ?? true),
                'approval_reference' => (string) ($options['approvalReference'] ?? 'C142_OPERATOR_APPROVED_PRODUCTION_LIVE_RUNTIME_ACTIVATION_OPERATOR_GO_NO_GO_REVIEW_ONLY'),
                'overwrite' => true,
    ...
    private function mutateC141AndExecute(callable $mutator, string $name): array
    {
        $c141 = json_decode((string) file_get_contents(self::C141_ARTIFACT), true);
        $c141 = $mutator(is_array($c141) ? $c141 : []);
        $path = 'storage/app/watchlist/backtest/.tmp-c142-source-c141-'.$name.'.json';
        $this->tmpFiles[] = $path;
        file_put_contents($path, json_encode($c141, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE).PHP_EOL);

        return $this->runService([
            'c141Artifact' => $path,
            'expectedC141Hash' => (string) ($c141['artifact_hash'] ?? ''),
            'expectedC141FileSha1' => strtoupper(sha1((string) file_get_contents($path))),
        ]);
    }
    ...
    private function cleanupC142TemporaryArtifacts(): void
    {
        foreach ((array) glob('storage/app/watchlist/backtest/c142-*test.json') as $file) {
            if (is_file($file)) {
                unlink($file);
            }
        }
        foreach ((array) glob('storage/app/watchlist/backtest/.tmp-c142*.json') as $file) {
            if (is_file($file)) {
                unlink($file);
            }
        }
