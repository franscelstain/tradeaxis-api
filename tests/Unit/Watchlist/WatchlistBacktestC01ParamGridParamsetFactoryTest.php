...
class WatchlistBacktestC01ParamGridParamsetFactoryTest extends TestCase
{
    public function test_factory_maps_every_explicit_c01_field_without_default_override(): void
    {
        $row = WatchlistBacktestC01ParamGridCatalog::rows()[3];
        $row['param_id'] = 4003;
    ...
    public function test_all_c01_rows_keep_exit_semantics_fixed_and_have_unique_paramset_hashes(): void
    {
        $factory = new WatchlistBacktestParamGridParamsetFactory();
        $hashes = [];

        foreach (WatchlistBacktestC01ParamGridCatalog::rows() as $index => $row) {
            $row['param_id'] = 5000 + $index;
            $snapshot = $factory->make($row);
            $this->assertSame(WatchlistBacktestC01ParamGridCatalog::FIXED_STOP_ATR_MULT, $snapshot['risk']['stop_atr_mult']);
            $this->assertSame(WatchlistBacktestC01ParamGridCatalog::FIXED_MIN_RR, $snapshot['risk']['min_rr']);
            $this->assertSame(WatchlistBacktestC01ParamGridCatalog::FIXED_TOP_PICKS_TARGET, $snapshot['grouping']['top_picks']['max_items']);
            $this->assertSame(WatchlistBacktestC01ParamGridCatalog::FIXED_SECONDARY_TARGET, $snapshot['grouping']['secondary']['max_items']);
    ...
    public function test_c01_paramset_hash_does_not_depend_on_database_surrogate_param_id(): void
    {
        $factory = new WatchlistBacktestParamGridParamsetFactory();
        $left = WatchlistBacktestC01ParamGridCatalog::rows()[1];
        $right = $left;
        $left['param_id'] = 101;
