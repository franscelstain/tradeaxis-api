...
class WatchlistBacktestC05ParamGridParamsetFactoryTest extends TestCase
{
    public function test_factory_maps_c05_fields_and_embeds_soft_candidate_selection_extension(): void
    {
        ...
        $this->assertSame(
            WatchlistBacktestC05ParamGridCatalog::candidateSelectionExtension(),
            $snapshot['bt_grid_resolution']['candidate_selection_extension']
        );
    }

    public function test_c05_extension_is_distinct_from_c04_extension(): void
    {
        $factory = new WatchlistBacktestParamGridParamsetFactory();
        $c04 = WatchlistBacktestC04ParamGridCatalog::rows()[2];
        ...
        $this->assertSame('C04_BALANCED_COMPONENT_AND_TREND_FLOOR', $c04Snapshot['bt_grid_resolution']['candidate_selection_extension']['mode']);
    ...
    public function test_all_c05_rows_keep_exit_semantics_fixed_and_have_unique_paramset_hashes(): void
    {
        $factory = new WatchlistBacktestParamGridParamsetFactory();
        $hashes = [];

        foreach (WatchlistBacktestC05ParamGridCatalog::rows() as $index => $row) {
            $row['param_id'] = 10000 + $index;
            $snapshot = $factory->make($row);
            $this->assertSame(WatchlistBacktestC05ParamGridCatalog::FIXED_STOP_ATR_MULT, $snapshot['risk']['stop_atr_mult']);
            $this->assertSame(WatchlistBacktestC05ParamGridCatalog::FIXED_MIN_RR, $snapshot['risk']['min_rr']);
            $this->assertSame(WatchlistBacktestC05ParamGridCatalog::FIXED_TOP_PICKS_TARGET, $snapshot['grouping']['top_picks']['max_items']);
            $this->assertSame(WatchlistBacktestC05ParamGridCatalog::FIXED_SECONDARY_TARGET, $snapshot['grouping']['secondary']['max_items']);
            $this->assertSame(WatchlistBacktestC05ParamGridCatalog::candidateSelectionExtension(), $snapshot['bt_grid_resolution']['candidate_selection_extension']);
    ...
    public function test_c05_paramset_hash_does_not_depend_on_database_surrogate_param_id(): void
    {
        $factory = new WatchlistBacktestParamGridParamsetFactory();
        $left = WatchlistBacktestC05ParamGridCatalog::rows()[2];
        $right = $left;
        $left['param_id'] = 105;
        $right['param_id'] = 100105;
