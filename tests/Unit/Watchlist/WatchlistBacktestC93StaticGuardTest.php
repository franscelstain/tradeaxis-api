...
class WatchlistBacktestC93StaticGuardTest extends TestCase
{
    public function test_c93_command_is_registered(): void
    {
        $kernel = (string) file_get_contents('app/Console/Kernel.php');

        $this->assertStringContainsString('RunBacktestC93ControlledLimitedRuntimeOptInPilotOrShadowRolloutPostActivationHandoffClosureSealReviewCommand::class', $kernel);
        $this->assertStringContainsString('use App\\Console\\Commands\\Watchlist\\RunBacktestC93ControlledLimitedRuntimeOptInPilotOrShadowRolloutPostActivationHandoffClosureSealReviewCommand;', $kernel);
    }

    public function test_c93_runtime_has_no_forbidden_date_or_future_selection_shortcuts(): void
    {
        $service = (string) file_get_contents('app/Application/Watchlist/Services/WatchlistBacktestC93ControlledLimitedRuntimeOptInPilotOrShadowRolloutPostActivationHandoffClosureSealReviewService.php');
        $command = (string) file_get_contents('app/Console/Commands/Watchlist/RunBacktestC93ControlledLimitedRuntimeOptInPilotOrShadowRolloutPostActivationHandoffClosureSealReviewCommand.php');
        $combined = $service."\n".$command;
    ...
    public function test_c93_docs_state_closure_seal_only_and_not_live_rollout(): void
    {
        $doc = (string) file_get_contents('docs/watchlist/audit/WS_C93_CONTROLLED_LIMITED_RUNTIME_OPT_IN_PILOT_OR_SHADOW_ROLLOUT_POST_ACTIVATION_HANDOFF_CLOSURE_SEAL_REVIEW.md');
        $commands = (string) file_get_contents('docs/watchlist/audit/WS_C93_OPERATOR_VALIDATION_COMMANDS.md');
    ...
    public function test_c93_config_defaults_are_off_and_no_production_runtime_wiring_is_created(): void
    {
        ...
        $this->assertStringContainsString('post_activation_handoff_closure_seal_context_persisted_to_live_runtime', $combined);
        ...
        $this->assertStringContainsString('C93 closure seal is not production deployment, not live rollout, not runtime bridge activation, and not PLAN/CONFIRM mutation.', $combined);
    }
}
