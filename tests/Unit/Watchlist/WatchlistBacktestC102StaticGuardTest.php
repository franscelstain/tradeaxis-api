...
class WatchlistBacktestC102StaticGuardTest extends TestCase
{
    public function test_c102_command_is_registered(): void
    {
        $kernel = (string) file_get_contents('app/Console/Kernel.php');

        $this->assertStringContainsString('RunBacktestC102WeeklySwingWatchlistNonLiveRehearsalGoDecisionFinalizationReviewCommand::class', $kernel);
        $this->assertStringContainsString('use App\\Console\\Commands\\Watchlist\\RunBacktestC102WeeklySwingWatchlistNonLiveRehearsalGoDecisionFinalizationReviewCommand;', $kernel);
    }

    public function test_c102_runtime_has_no_forbidden_date_or_future_selection_shortcuts(): void
    {
        $service = (string) file_get_contents('app/Application/Watchlist/Services/WatchlistBacktestC102WeeklySwingWatchlistNonLiveRehearsalGoDecisionFinalizationReviewService.php');
        $command = (string) file_get_contents('app/Console/Commands/Watchlist/RunBacktestC102WeeklySwingWatchlistNonLiveRehearsalGoDecisionFinalizationReviewCommand.php');
        $combined = $service."\n".$command;
    ...
    public function test_c102_docs_state_go_decision_finalization_only_and_not_live_output(): void
    {
        $doc = (string) file_get_contents('docs/watchlist/audit/WS_C102_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_GO_DECISION_FINALIZATION_REVIEW.md');
        $commands = (string) file_get_contents('docs/watchlist/audit/WS_C102_OPERATOR_VALIDATION_COMMANDS.md');
    ...
    public function test_c102_config_defaults_are_off_and_no_production_runtime_wiring_is_created(): void
    {
        ...
        $this->assertStringContainsString('weekly_swing_watchlist_non_live_rehearsal_go_decision_finalization_context_persisted_to_live_runtime', $combined);
        ...
        $this->assertStringContainsString('C102 weekly swing watchlist non-live rehearsal GO decision finalization review is not production deployment, not live rollout, not runtime bridge activation, not pilot/shadow runtime, not weekly swing live output, and not PLAN/CONFIRM mutation.', $combined);
    }
}
