...
class WatchlistBacktestC103StaticGuardTest extends TestCase
{
    public function test_c103_command_is_registered(): void
    {
        $kernel = (string) file_get_contents('app/Console/Kernel.php');

        $this->assertStringContainsString('RunBacktestC103WeeklySwingWatchlistNonLiveRehearsalCompletionBoundaryReviewCommand::class', $kernel);
        $this->assertStringContainsString('use App\\Console\\Commands\\Watchlist\\RunBacktestC103WeeklySwingWatchlistNonLiveRehearsalCompletionBoundaryReviewCommand;', $kernel);
    }

    public function test_c103_runtime_has_no_forbidden_date_or_future_selection_shortcuts(): void
    {
        $service = (string) file_get_contents('app/Application/Watchlist/Services/WatchlistBacktestC103WeeklySwingWatchlistNonLiveRehearsalCompletionBoundaryReviewService.php');
        $command = (string) file_get_contents('app/Console/Commands/Watchlist/RunBacktestC103WeeklySwingWatchlistNonLiveRehearsalCompletionBoundaryReviewCommand.php');
        $combined = $service."\n".$command;
    ...
    public function test_c103_docs_state_completion_boundary_only_and_not_live_output(): void
    {
        $doc = (string) file_get_contents('docs/watchlist/audit/WS_C103_WEEKLY_SWING_WATCHLIST_NON_LIVE_REHEARSAL_COMPLETION_BOUNDARY_REVIEW.md');
        $commands = (string) file_get_contents('docs/watchlist/audit/WS_C103_OPERATOR_VALIDATION_COMMANDS.md');
    ...
    public function test_c103_config_defaults_are_off_and_no_production_runtime_wiring_is_created(): void
    {
        ...
        $this->assertStringContainsString('weekly_swing_watchlist_non_live_rehearsal_completion_boundary_context_persisted_to_live_runtime', $combined);
        ...
        $this->assertStringContainsString('C103 weekly swing watchlist non-live rehearsal completion boundary review is not production deployment, not live rollout, not runtime bridge activation, not pilot/shadow runtime, not weekly swing live output, and not PLAN/CONFIRM mutation.', $combined);
    }
}
