...
class WatchlistBacktestC03StaticGuardTest extends TestCase
{
    public function test_c03_seed_command_is_registered_explicit_and_not_scheduled(): void
    {
        $command = file_get_contents(base_path('app/Console/Commands/Watchlist/SeedBacktestC03ParamGridCommand.php'));
        $kernel = file_get_contents(base_path('app/Console/Kernel.php'));
        $seeder = file_get_contents(base_path('database/seeders/Watchlist/WatchlistBacktestC03ParamGridSeeder.php'));

        $this->assertStringContainsString('watchlist:backtest-c03-param-grid-seed', $command);
        $this->assertStringContainsString('SeedBacktestC03ParamGridCommand::class', $kernel);
        $this->assertStringContainsString('WatchlistBacktestC03ParamGridCatalog::rows()', $command);
        $this->assertStringContainsString('WatchlistBacktestC03ParamGridCatalog::rows()', $seeder);
        ...
        $this->assertStringNotContainsString("schedule->command('watchlist:backtest-c03-param-grid-seed", $kernel);
    }

    public function test_c03_calibration_path_is_explicit_is_only_and_has_no_oos_dependency(): void
    {
        $files = [
            base_path('app/Application/Watchlist/Services/WatchlistBacktestC03ParamGridCatalog.php'),
        ...
        $this->assertStringContainsString('WS_BT_GRID_DOWNSIDE_STABILITY_C03_2026_06', $content);
        $this->assertStringContainsString('C03_GRID_FAILED_IS_QUALITY', $content);
        $this->assertStringContainsString('WATCHLIST_C03_IS_CALIBRATION_V1', $content);
        $this->assertStringContainsString('WEEKLY_SWING_DOWNSIDE_STABILITY_C03_IS_ONLY', $content);
        ...
        $this->assertStringContainsString('WS_BT_C03_NO_VALID_IS_CANDIDATE', $content);
    ...
    public function test_c03_does_not_continue_r_series_catalog_naming_or_add_unsupported_sector_filter(): void
    {
        $files = [
            base_path('app/Application/Watchlist/Services/WatchlistBacktestC03ParamGridCatalog.php'),
            base_path('app/Console/Commands/Watchlist/SeedBacktestC03ParamGridCommand.php'),
            base_path('database/seeders/Watchlist/WatchlistBacktestC03ParamGridSeeder.php'),
        ];
        $content = implode("\n", array_map('file_get_contents', $files));
    ...
    public function test_every_c03_axis_has_an_explicit_runtime_consumer(): void
    {
