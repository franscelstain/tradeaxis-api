...
class AddCorrectionReexecutionPolicyFields extends Migration
{
    public function up()
    {
        if (! Schema::hasColumn('eod_dataset_corrections', 'execution_count')) {
        ...
        if (! Schema::hasColumn('eod_dataset_corrections', 'last_executed_at')) {
            Schema::table('eod_dataset_corrections', function (Blueprint $table) {
                $table->dateTime('last_executed_at')->nullable()->after('execution_count');
            });
        }

        if (! Schema::hasColumn('eod_dataset_corrections', 'current_consumed_at')) {
            Schema::table('eod_dataset_corrections', function (Blueprint $table) {
    ...
    public function down()
    {
        ...
        $columns = [];
        foreach (['current_consumed_at', 'last_executed_at', 'execution_count'] as $column) {
        ...
        if (! empty($columns)) {
            Schema::table('eod_dataset_corrections', function (Blueprint $table) use ($columns) {
