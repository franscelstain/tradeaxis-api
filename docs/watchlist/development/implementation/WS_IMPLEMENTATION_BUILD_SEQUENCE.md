...
Canonical strategy lifecycle: [`../../authority/strategy/WS_END_TO_END_STRATEGY_LIFECYCLE.md`](../../authority/strategy/WS_END_TO_END_STRATEGY_LIFECYCLE.md).
...
1. **Read recording governance** — [`../../authority/governance/DOCUMENT_RECORDING_STANDARD.md`](../../authority/governance/DOCUMENT_RECORDING_STANDARD.md).
2. **Read Work Baseline Lock governance** — [`../../authority/governance/WORK_BASELINE_LOCK_STANDARD.md`](../../authority/governance/WORK_BASELINE_LOCK_STANDARD.md).
3. **Read stage execution/re-entry governance** — [`../../authority/governance/STAGE_EXECUTION_AND_REWORK_STANDARD.md`](../../authority/governance/STAGE_EXECUTION_AND_REWORK_STANDARD.md).
4. **Read recurring residue/conformance governance** — [`../../authority/governance/IMPLEMENTATION_RESIDUE_AND_CONFORMANCE_STANDARD.md`](../../authority/governance/IMPLEMENTATION_RESIDUE_AND_CONFORMANCE_STANDARD.md).
5. **Read executable integrity governance** — [`../../authority/governance/DOCUMENT_INTEGRITY_GATE_STANDARD.md`](../../authority/governance/DOCUMENT_INTEGRITY_GATE_STANDARD.md).
6. **Read current stage register** — [`WS_IMPLEMENTATION_STAGE_REGISTER.md`](WS_IMPLEMENTATION_STAGE_REGISTER.md).
7. Jika stage pernah dicoba, jalankan mandatory re-entry protocol termasuk membaca known residue/evidence sebelum code change.
8. **Open a new Attempt ID** memakai [`examples/WS_STAGE_ATTEMPT_RECORD_TEMPLATE.md`](examples/WS_STAGE_ATTEMPT_RECORD_TEMPLATE.md).
9. **Issue immutable Work Baseline Lock before code change** dan bind `Stage ID -> Attempt ID -> Baseline ID`.
10. **Run executable integrity gate PRE_ATTEMPT**. Jangan mulai material implementation change jika preflight gagal.
11. **Read strategy owner** — jangan mulai dari code/schema.
12. **Read current implementation guards/contracts** — identifikasi semantic drift lama.
13. **Define attempt objective/hypothesis dan perubahan versus attempt sebelumnya**.
14. **Implement deterministic domain/application behavior**.
15. **Add negative/fail-closed tests** sebelum happy-path dianggap cukup.
16. **Add persistence/API translation** tanpa mengubah semantics.
17. **Run functional + conformance tests** terhadap stage output/exit criteria.
18. **Run Residue & Conformance Check** pada impacted code/config/schema/API/fixture/test/runtime/proof paths; classify residue berdasarkan reachability + semantic impact.
19. Jika `HARMFUL_RESIDUE` ditemukan, buat/remediate finding dan kembali ke implementation/validation; jangan lanjut ke `DONE`.
20. **Run executable integrity gate PRE_ATTEMPT_CLOSE with `--baseline`**.
21. **Close the attempt** memakai canonical attempt record sebagai immutable evidence, termasuk baseline, gate result, residue, coverage, outcome, convergence, learned facts, do-not-repeat, remaining gap, dan next action.
22. **Update stage register** + implementation status/contract tracker append-oriented.
23. **Record material documentation/contract change** di [`../../authority/governance/DOCUMENT_CHANGE_LOG.md`](../../authority/governance/DOCUMENT_CHANGE_LOG.md).
24. Tentukan stage state berikutnya berdasarkan evidence—bukan berdasarkan jumlah gagal.
...
### Mandatory Work Baseline + Executable Integrity Gate

Every implementation/proof attempt must have:

- one immutable `Baseline ID` issued before material changes;
- one `Attempt ID` using the canonical attempt record template;
- pre-attempt integrity gate PASS;
- pre-close integrity gate PASS using `--baseline`;
- stage register pointer to latest Attempt ID, Baseline ID, and gate evidence.

A baseline records the starting authority/source state; it does not prove the implementation is correct. A gate proves structural/integrity invariants; it does not replace semantic tests, residue analysis, or strategy coverage.

### Mandatory Strategy Traceability / Coverage Gate

Sebelum dan sesudah setiap `WS-Bxx`, filter [`../../authority/governance/STRATEGY_TO_IMPLEMENTATION_TRACEABILITY_MATRIX.csv`](../../authority/governance/STRATEGY_TO_IMPLEMENTATION_TRACEABILITY_MATRIX.csv) pada `verification_build_stage = WS-Bxx`.

Wajib:

1. setiap row mempunyai explicit implementation mapping;
2. test/evidence pointer diisi saat tersedia;
3. residue verdict mengikuti recurring residue standard;
4. mandatory row belum `SATISFIED` menjadi backlog stage/rework;
5. stage tidak boleh `DONE` jika masih ada mandatory coverage gap;
6. optional `WS-B07` row boleh `OPTIONAL_NOT_REQUESTED` tanpa memblokir core.

Stage-level completion tidak boleh dipakai untuk menyembunyikan rule-level gap. Final 100% strategy coverage mengikuti [`../../authority/governance/STRATEGY_IMPLEMENTATION_TRACEABILITY_STANDARD.md`](../../authority/governance/STRATEGY_IMPLEMENTATION_TRACEABILITY_STANDARD.md).
...
## Mandatory Attempt Control Flow

Untuk setiap formal `WS-Bxx` attempt:

1. baca `CURRENT_STATE.md` + Stage Register;
2. re-entry: baca Work Record Registry, predecessor attempt/finding/dependency;
3. open Attempt ID = canonical Work ID;
4. issue immutable Baseline Lock;
5. create Change Impact Declaration bila ada material change;
6. run structural + relationship integrity preflight;
7. implement/remediate + tests;
8. residue/conformance check;
9. update Strategy Traceability Matrix + evidence;
10. update Work Record Registry / Dependency Registry;
11. close immutable Attempt Record;
12. bila stage terminal, issue Stage Closure Manifest;
13. regenerate `CURRENT_STATE.md`.

Tidak ada current implementation record yang boleh "terlepas" dari Stage/Attempt/Work/Baseline relationship.
...
- [`../../authority/strategy/WS_PRODUCT_OBJECTIVE_AND_LAYERS.md`](../../authority/strategy/WS_PRODUCT_OBJECTIVE_AND_LAYERS.md)
- [`../../authority/strategy/WS_END_TO_END_STRATEGY_LIFECYCLE.md`](../../authority/strategy/WS_END_TO_END_STRATEGY_LIFECYCLE.md)
...
### Strategy owners
- [`../../authority/strategy/WS_MARKET_DATA_INPUT_REQUIREMENTS.md`](../../authority/strategy/WS_MARKET_DATA_INPUT_REQUIREMENTS.md)
...
- [`guides/WS_IMPLEMENTATION_SCOPE_AND_BOUNDARY.md`](guides/WS_IMPLEMENTATION_SCOPE_AND_BOUNDARY.md)
- [`guides/WS_CANONICAL_FIELD_MATRIX.md`](guides/WS_CANONICAL_FIELD_MATRIX.md)
...
- [`contracts/WS_PARAMSET_JSON_CONTRACT.md`](contracts/WS_PARAMSET_JSON_CONTRACT.md)
- [`contracts/WS_PARAMETER_REGISTRY_COMPLETE.md`](contracts/WS_PARAMETER_REGISTRY_COMPLETE.md)
- [`contracts/WS_PARAMSET_VALIDATOR_SPEC.md`](contracts/WS_PARAMSET_VALIDATOR_SPEC.md)
- [`contracts/WS_REASON_CODES_AND_HASH.md`](contracts/WS_REASON_CODES_AND_HASH.md)
...
### Strategy owners
- [`../../authority/strategy/WS_CANDIDATE_ELIGIBILITY_AND_SETUP.md`](../../authority/strategy/WS_CANDIDATE_ELIGIBILITY_AND_SETUP.md)
- [`../../authority/strategy/WS_CANDIDATE_CLASSIFICATION.md`](../../authority/strategy/WS_CANDIDATE_CLASSIFICATION.md)

### Technical references to align/use
- [`guides/WS_MODULE_MAPPING.md`](guides/WS_MODULE_MAPPING.md)
- [`guides/WS_CANONICAL_FIELD_MATRIX.md`](guides/WS_CANONICAL_FIELD_MATRIX.md)
...
### Strategy owner
- [`../../authority/strategy/WS_PLAN_SCORING_AND_TRADE_PLAN.md`](../../authority/strategy/WS_PLAN_SCORING_AND_TRADE_PLAN.md)

### Technical references to align/use
- [`guides/WS_MODULE_MAPPING.md`](guides/WS_MODULE_MAPPING.md)
- [`guides/WS_RUNTIME_ARTIFACT_FLOW.md`](guides/WS_RUNTIME_ARTIFACT_FLOW.md)
- [`guides/WS_PERSISTENCE_GUIDANCE.md`](guides/WS_PERSISTENCE_GUIDANCE.md)
...
### Strategy owners
- [`../../authority/strategy/WS_TOP_PICKS_RECOMMENDATION.md`](../../authority/strategy/WS_TOP_PICKS_RECOMMENDATION.md)
- [`../../authority/strategy/WS_TOP_PICKS_QUALIFICATION_AND_RANKING.md`](../../authority/strategy/WS_TOP_PICKS_QUALIFICATION_AND_RANKING.md)

### Technical references to align/use
- [`contracts/WS_RECOMMENDATION_INPUT_OUTPUT_CONTRACT.md`](contracts/WS_RECOMMENDATION_INPUT_OUTPUT_CONTRACT.md)
- [`guides/WS_RUNTIME_ARTIFACT_FLOW.md`](guides/WS_RUNTIME_ARTIFACT_FLOW.md)
- [`guides/WS_API_GUIDANCE.md`](guides/WS_API_GUIDANCE.md)
- [`guides/WS_PERSISTENCE_GUIDANCE.md`](guides/WS_PERSISTENCE_GUIDANCE.md)
...
- [`../../authority/strategy/WS_D1_CONFIRM_ACTIONABILITY.md`](../../authority/strategy/WS_D1_CONFIRM_ACTIONABILITY.md)
...
### Strategy owner
- [`../../authority/strategy/WS_HISTORICAL_EVALUATION_STRATEGY.md`](../../authority/strategy/WS_HISTORICAL_EVALUATION_STRATEGY.md)
...
- [`tests/WS_BT_COVERAGE_MATRIX_LOCKED.md`](tests/WS_BT_COVERAGE_MATRIX_LOCKED.md)
- [`tests/WS_UNIVERSE_EQUIVALENCE_CONTRACT_LOCKED.md`](tests/WS_UNIVERSE_EQUIVALENCE_CONTRACT_LOCKED.md)
...
### Strategy owner
- [`../../authority/strategy/WS_IS_SUFFICIENCY_AND_WINNER_FREEZE.md`](../../authority/strategy/WS_IS_SUFFICIENCY_AND_WINNER_FREEZE.md)
...
### Strategy owner
- [`../../authority/strategy/WS_OOS_STRESS_SHADOW_AND_PRODUCTION_PROOF.md`](../../authority/strategy/WS_OOS_STRESS_SHADOW_AND_PRODUCTION_PROOF.md)
...
- [`contracts/WS_BACKTEST_ARTIFACT_MANIFEST_LOCKED.md`](contracts/WS_BACKTEST_ARTIFACT_MANIFEST_LOCKED.md)
...
- attempt result → `../../records/evidence/` as immutable evidence;
- implementation changes → current `development/implementation/` documents/code (or repository code mapped by the stage);
- test/backtest/runtime output → `../../records/evidence/`;
- discovered issue → `../findings/`;
- formal strategy/production decision → `../../records/decisions/`;
- superseded technical/history material → `../../records/history/`.
...
## Relationship Integrity Hard Gate

Before attempt closure and before terminal stage closure, run `WatchlistRelationshipIntegrityGate.php`. It must enforce **all 9 hard relationship invariants** defined by `WORK_CORRELATION_AND_RECORD_REGISTRY_STANDARD.md`; a partial relationship check is not closure evidence. Cross-attempt links must be registered in `records/WORK_RELATIONSHIP_REGISTRY.csv`.
## Recurring Document Role Gate

Pada setiap `WS-Bxx`, file baru atau material documentation change wajib mengikuti [`../../authority/governance/ONE_DOCUMENT_ONE_AUTHORITATIVE_ROLE_STANDARD.md`](../../authority/governance/ONE_DOCUMENT_ONE_AUTHORITATIVE_ROLE_STANDARD.md). Register role pada `DOCUMENT_ROLE_REGISTRY.csv`; sebelum attempt close pastikan executable documentation gate menunjukkan `ONE_DOCUMENT_ONE_AUTHORITATIVE_ROLE=PASS`.

## Current Verification Rebaseline Entry Rule

Before `WS-B00` and every later stage, treat all pre-epoch code/results as **existing but unverified**. Do not skip a stage because C/R/P/B/S/Q history says PASS/DONE. Reuse is allowed only after current mapping + tests + residue + evidence under `WS-REBASELINE-20260819-001`.
