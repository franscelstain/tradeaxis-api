...
- [`../../persistence/02_DB_SCHEMA_MARIADB.md`](../db/DB_SCHEMA_MARIADB.md)
- [`../../persistence/03_DB_INDEXES_AND_CONSTRAINTS.md`](../db/DB_INDEXES_AND_CONSTRAINTS.md)
- [`../../persistence/05_DB_DDL_MARIADB.sql`](../db/DB_DDL_MARIADB.sql)
...
|---|---|---|
| snapshot available? | header snapshot exists | `WS_D1_CONFIRM_ACTIONABILITY.md` |
| snapshot fresh? | header `captured_at` / `inserted_at` semantics | `WS_D1_CONFIRM_ACTIONABILITY.md` + dokumen ini |
| item bound to ticker? | item `ticker_code` | `WS_D1_CONFIRM_ACTIONABILITY.md` + dokumen ini |
| confirm positive/warning inputs | item fields normatif seperti `last_price` | `WS_D1_CONFIRM_ACTIONABILITY.md` |
| plan mutation? | tidak boleh dari tabel snapshot | `WS_RUNTIME_FLOW.md` |
...
Semantics tabel snapshot watchlist-owned ditetapkan di dokumen ini dan dibaca bersama `WS_D1_CONFIRM_ACTIONABILITY.md` serta `WS_DATA_MODEL_MARIADB.md`. SQL artifacts hanya merealisasikan shape yang dikunci di sini dan tidak menggantikan dokumen ini sebagai owner semantics.
