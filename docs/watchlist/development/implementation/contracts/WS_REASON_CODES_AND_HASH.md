...
Reason code berikut adalah reason code foundation untuk optional CONFIRM. Kode ini hanya menjelaskan availability/actionability terhadap final Top Pick + immutable source binding. Kode ini bukan reason code recommendation final, bukan portfolio allocation, bukan execution instruction, dan bukan backtest metric.

Canonical semantic mapping saat alignment:
- missing/stale/incomplete/no-price conditions map to `UNAVAILABLE_RETRYABLE` while entry window is open;
- only valid evaluated failed gates may map to `NOT_ACTIONABLE`;
- `WS_CONFIRM_ELIGIBLE_NON_RECOMMENDED` — **LEGACY / DO NOT EMIT** under current strategy is legacy and must not be emitted by new conformant behavior.

- `WS_CONFIRM_ELIGIBLE_RECOMMENDED`
- `WS_CONFIRM_ELIGIBLE_NON_RECOMMENDED` — **LEGACY / DO NOT EMIT** under current strategy
- `WS_CONFIRM_APPLIED`
- `WS_CONFIRM_NOT_APPLIED`
- `WS_CONFIRM_REJECTED_UNKNOWN_CANDIDATE`
- `WS_CONFIRM_REJECTED_NOT_PLAN_CANDIDATE` — legacy naming; current semantic is non-Top-Pick target rejected
- `WS_CONFIRM_NO_DATA` — current semantic: `UNAVAILABLE_RETRYABLE` while entry window remains open
...
## Campaign-specific reason-code extensions
Campaign-scoped C171/R2/C01 reason-code additions were moved to `../../../records/history/campaign_addenda/WS_CAMPAIGN_REASON_CODE_EXTENSIONS.md`.
