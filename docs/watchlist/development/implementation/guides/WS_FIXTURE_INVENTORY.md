...
|---|---|---|---|
| `paramset_valid.json` + validator fixtures | input/paramset validation | Apakah input salah ditolak dengan benar? | `WS_PARAMSET_JSON_CONTRACT.md`, `WS_CONTRACT_TEST_CHECKLIST.md` |
| `PLAN_FIXTURE_A_TIES_V1.json` | PLAN determinism, plan levels, hash | Apakah run PLAN kaya tetap stabil? | `WS_RUNTIME_FLOW.md`, `WS_REASON_CODES_AND_HASH.md` |
| `plan_items_artificial_ties.json` | tie-break diagnosis | Apakah ranking tetap deterministik saat skor sama? | `WS_CANDIDATE_CLASSIFICATION.md` |
| `plan_items_guard_fail.json` | guard fail | Apakah kandidat gagal ditempatkan dengan reason yang benar? | `WS_RUNTIME_FLOW.md` |
| `plan_items_forced_watch_only.json` | watch-only forcing | Apakah kandidat yang lewat guard tetap bisa diturunkan menjadi watch-only? | `WS_CANDIDATE_CLASSIFICATION.md` |
| `confirm_immutability_pair.json` | overlay / immutability | Apakah CONFIRM membaca PLAN tanpa menulis ulang? | `WS_D1_CONFIRM_ACTIONABILITY.md`, `WS_CONTRACT_TEST_CHECKLIST.md` |
| `confirm_payload_with_unknown_top_level_field.json` | strictness / invalid schema drift | Apakah payload liar ditolak dengan benar? | `WS_D1_CONFIRM_ACTIONABILITY.md`, `WS_CONTRACT_TEST_CHECKLIST.md` |
| `hash_contract_vectors.json` | hash contract | Apakah payload canonical menghasilkan hash yang benar? | `WS_REASON_CODES_AND_HASH.md`, `WS_CONTRACT_TEST_CHECKLIST.md` |
| Governance / backtest fixtures | audit artefak | Apakah jejak evaluasi dan bukti tersedia? | `WS_OOS_STRESS_SHADOW_AND_PRODUCTION_PROOF.md`, `WS_BACKTEST_ARTIFACT_MANIFEST_LOCKED.md` |
...
Untuk kewajiban test minimum, gunakan `WS_CONTRACT_TEST_CHECKLIST.md`.

## Recommendation Fixture Inventory

Fixture recommendation harus mencakup empty-set behavior, deterministic replay, serta boundary antara core recommendation dan optional CONFIRM. CONFIRM-data absence harus mempunyai fixture tersendiri yang membuktikan core output tetap sukses dan state menjadi `UNAVAILABLE_RETRYABLE`.


## Recommendation and Confirm Boundary Fixtures

- `fixtures/recommendation_empty_but_prioritized_groups_nonempty.json`
- `fixtures/confirm_unavailable_retryable.json`
