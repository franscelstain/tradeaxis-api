...
- runtime field dan persistence shape -> `WS_DATA_MODEL_MARIADB.md`
- PLAN branch behavior -> `WS_PLAN_SCORING_AND_TRADE_PLAN.md`
- deterministic selection / grouping -> `WS_CANDIDATE_CLASSIFICATION.md`
- CONFIRM label behavior -> `WS_D1_CONFIRM_ACTIONABILITY.md`
- recommendation contract -> `WS_RECOMMENDATION_INPUT_OUTPUT_CONTRACT.md`
- recommendation algorithm -> `WS_TOP_PICKS_QUALIFICATION_AND_RANKING.md`
- acceptance akhir -> `WS_CONTRACT_TEST_CHECKLIST.md`
...
| CONFIRM unavailable/retryable | `examples/WS_CONFIRM_UNAVAILABLE_RETRYABLE_EXAMPLE.json` |
...
## CONFIRM Output Example — Unavailable / Retryable

Lihat `examples/WS_CONFIRM_UNAVAILABLE_RETRYABLE_EXAMPLE.json`.

Contoh ini menunjukkan bahwa missing current-entry data tidak mengubah Top Pick menjadi gagal atau `NOT_ACTIONABLE`; state tetap retryable selama entry window terbuka.
