...
CONFIRM artifact/section is optional. Required=YES rows under CONFIRM apply only when CONFIRM request/attempt/output exists; absence of CONFIRM is valid core state.
...
| CONFIRM API | `ticker` | YES when CONFIRM present | string | final Top Pick only |
