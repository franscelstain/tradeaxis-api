...
> Existing technical contracts/code/tests are **not automatically conformant**. Behavior-bearing implementation material must be revalidated stage-by-stage before it becomes current proof.
...
`LUMEN_IMPLEMENTATION_STATUS.md` and `LUMEN_CONTRACT_TRACKER.md` are append-oriented implementation/status ledgers. Their historical session text may contain old paths or campaign names and must not be interpreted as current authority.
...
## Current Verification Rebaseline Registry

Before trusting any existing technical document, inspect [`../../authority/governance/CURRENT_VERIFICATION_REGISTRY.csv`](../../authority/governance/CURRENT_VERIFICATION_REGISTRY.csv). Behavior-bearing implementation material begins `NOT_ASSESSED_REVALIDATION_REQUIRED`; old existence/test/result does not imply current conformance.
