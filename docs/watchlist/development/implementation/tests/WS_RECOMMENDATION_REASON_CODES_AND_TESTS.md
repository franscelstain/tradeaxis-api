...
2. ticker non-recommended tidak boleh menjadi valid CONFIRM target; missing data pada final Top Pick menghasilkan availability state non-blocking;
...
- `WS_CONFIRM_NO_DATA` — legacy/technical mapping must resolve to non-blocking `UNAVAILABLE_RETRYABLE`, not negative actionability
