...
## 3A. Exact-Duplicate Retained Sources Must Use the Role-Specific Primary Copy

A semantically single-role legacy source does **not** require a second physical `LS-*` copy when its normalized role-specific `final_primary_path` is byte-identical to the registered source. This compaction is allowed only when:

1. the physical `archived_source_path` and `final_primary_path` both exist at validation time;
2. SHA1(`archived_source_path`) = SHA1(`final_primary_path`) = registered `original_sha1`;
3. the `final_primary_path` is role-appropriate and registered; and
4. the exact removal mapping is recorded in `LEGACY_EXACT_DUPLICATE_COMPACTION_INDEX.csv`.

After compaction, `archived_source_path` MUST be empty and `source_storage_policy` MUST be `DEDUPLICATED_TO_ROLE_PRIMARY`. The `LS-*` identity, original path, original SHA1, semantic audit, and `final_primary_path` remain permanent provenance.

Filename similarity, semantic similarity, normalized-equivalent text, or manual judgement MUST NOT authorize deletion. Non-identical sources remain physical unless another governed disposal rule applies.
...
- `LEGACY_EXACT_DUPLICATE_COMPACTION_INDEX.csv`
- `LEGACY_BUNDLE_EXCEPTION_INDEX.csv`
- `LEGACY_WORK_CORRELATION_INDEX.csv`

The executable documentation integrity gate must verify unique source/extract identities, retained-source hashes where a separate source copy is intentionally retained, exact-deduplicated primary-path hashes for `DEDUPLICATED_TO_ROLE_PRIMARY`, sealed 100% split coverage where a source is removed after decomposition, extract-body hashes, split-mapping hashes, and absence of historical `LX-*` records from active development areas.
