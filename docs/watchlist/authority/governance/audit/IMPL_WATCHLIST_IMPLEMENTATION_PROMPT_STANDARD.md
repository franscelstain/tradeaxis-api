...
## One Document / One Role Requirement

Setiap file yang dibuat atau materially diubah wajib mengikuti `docs/watchlist/authority/governance/ONE_DOCUMENT_ONE_AUTHORITATIVE_ROLE_STANDARD.md`. Tetapkan satu authoritative role; cross-role information harus berupa ID/link/ringkasan pendukung. Jangan membuat composite authority. Sebelum attempt close, pastikan `ONE_DOCUMENT_ONE_AUTHORITATIVE_ROLE=PASS` pada executable documentation gate.
