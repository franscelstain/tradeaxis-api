...
- runtime artifact flow `PLAN -> qualified RECOMMENDATION/TOP_PICKS -> CONFIRM actionability`;
...
3. Watchlist **MUST** mengonsumsi authoritative Market Data product dan tidak mengambil alih provider/acquisition internals.
4. Baseline strategy di [`../../strategy/README.md`](../../strategy/README.md) adalah source of truth untuk implementasi.
...
3. `RECOMMENDATION` implementation **MUST** menghasilkan final qualified `TOP_PICKS` tanpa quota-forcing dan tanpa capital-based reranking.
4. `CONFIRM` implementation **MUST** berasal dari final Top Pick binding dan hanya menentukan current actionability.
5. `CONFIRM` implementation **MUST NOT** membentuk atau mengubah historical recommendation membership/score/rank.
6. API watchlist **MUST NOT** mengekspos perilaku yang menyiratkan buy/sell execution.
7. Persistence watchlist **MUST** memisahkan artefak PLAN, RECOMMENDATION, dan CONFIRM.
8. Backtest/OOS implementation **MUST** mengevaluasi final Top Picks dan mendukung realistic cost, non-zero slippage, ranking-quality metrics, serta forward-shadow gate.
...
3. Input-validation boundary **SHOULD** tetap terisolasi dari recommendation logic dan tunduk pada upstream Market Data contract.
...
Audit implementasi watchlist adalah guardrail terhadap implementasi aplikasi. Audit ini tidak boleh mengambil alih owner rule bisnis dari [`../../strategy/README.md`](../../strategy/README.md).
...
Jika bukti code/app belum tersedia **dan Layer C memang tidak aktif**, audit **MUST** menandai item real-app evidence sebagai `N/A`, bukan `PARTIAL`. Lihat [`WATCHLIST_LAYER_C_APPLICABILITY_RULE.md`](IMPL_WATCHLIST_LAYER_C_APPLICABILITY_RULE.md).
