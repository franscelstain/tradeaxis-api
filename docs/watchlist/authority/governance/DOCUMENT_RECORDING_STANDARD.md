...
> **One-document-one-role rule:** seluruh lifecycle pada dokumen ini tunduk pada [`ONE_DOCUMENT_ONE_AUTHORITATIVE_ROLE_STANDARD.md`](ONE_DOCUMENT_ONE_AUTHORITATIVE_ROLE_STANDARD.md). Setiap file mempunyai tepat satu authoritative role; cross-role references boleh, cross-role authority tidak.

Tidak ada **semantic record** penting yang boleh berubah tanpa jejak.

Aturan utama:

1. satu fakta hanya mempunyai satu role/owner yang jelas, dan satu dokumen hanya mempunyai satu authoritative role;
...
**Current/future `WS-Bxx` implementation records are a stricter special case:** gunakan correlation-first ID dari [`WORK_CORRELATION_AND_RECORD_REGISTRY_STANDARD.md`](WORK_CORRELATION_AND_RECORD_REGISTRY_STANDARD.md), misalnya `E-WS-B04-A003-001`, `F-WS-B04-A003-001`, dan `D-WS-B04-A003-001`. Date-based IDs di atas tetap berlaku untuk governance/general records yang tidak berasal dari implementation attempt.
...
- implementation attempt mempunyai immutable Baseline ID sebelum material change;
- final attempt evidence memakai canonical attempt record fields;
- executable documentation integrity gate dijalankan pada required gates.

Audit checklist aktif harus menilai standard ini sebagai bagian Documentation Layer Guard.

## 21. Strategy Traceability / Coverage Recording Rule
...
## 22. Work Baseline Lock Recording Rule

Setiap implementation/rework/proof attempt tunduk pada [`WORK_BASELINE_LOCK_STANDARD.md`](WORK_BASELINE_LOCK_STANDARD.md).

Aturan recording:

1. baseline diterbitkan **sebelum** implementation change;
2. issued baseline adalah immutable evidence;
3. Attempt ID hanya boleh menunjuk satu Baseline ID;
4. source revision, authority fingerprints, Market Data contract, traceability matrix, dan relevant dependency/schema identity harus tercatat;
5. baseline tidak boleh diubah agar cocok dengan hasil akhir;
6. material authority drift menutup/rebaseline attempt, bukan edit baseline in-place.

## 23. Executable Documentation Integrity Gate Recording Rule

Executable gate mengikuti [`DOCUMENT_INTEGRITY_GATE_STANDARD.md`](DOCUMENT_INTEGRITY_GATE_STANDARD.md).

Final gate result yang dipakai untuk closure/package harus mempunyai command/output identity dan disimpan sebagai evidence atau bagian immutable attempt record. Manual statement `checked/PASS` tanpa executable result tidak cukup untuk check yang sudah diotomasi.

Registered legacy exception mengikuti [`DOCUMENT_INTEGRITY_EXCEPTION_REGISTRY.json`](DOCUMENT_INTEGRITY_EXCEPTION_REGISTRY.json); exception tidak boleh dibuat untuk menyembunyikan current strategy/implementation failure.

## 24. Canonical Stage Attempt Record Rule

Setiap attempt wajib memakai field minimum pada [`../../development/implementation/examples/WS_STAGE_ATTEMPT_RECORD_TEMPLATE.md`](../../development/implementation/examples/WS_STAGE_ATTEMPT_RECORD_TEMPLATE.md).

Final attempt record:

- adalah immutable evidence;
- mengikat Stage ID + Attempt ID + Baseline ID;
- mencatat re-entry lineage, test/run, strategy coverage, residue verdict, integrity-gate result, convergence, root-cause state, do-not-repeat, remaining gap, next action, dan resume point;
- tidak pernah dianggap otomatis menutup stage.

## 25. Work Correlation / Current Record Registry

Current/future `WS-Bxx` records wajib mengikuti [`WORK_CORRELATION_AND_RECORD_REGISTRY_STANDARD.md`](WORK_CORRELATION_AND_RECORD_REGISTRY_STANDARD.md). Attempt ID adalah canonical Work/Correlation ID; semua current record satu attempt harus searchable dengan key yang sama dan diregister pada `records/WORK_RECORD_REGISTRY.csv`.

Historical records sebelum adoption tidak perlu direwrite/backfill.

## 26. Closure / Dependency / Change-impact Records

- terminal stage wajib mengikuti [`STAGE_CLOSURE_MANIFEST_STANDARD.md`](STAGE_CLOSURE_MANIFEST_STANDARD.md);
- verified external/internal dependencies memakai [`DEPENDENCY_REGISTRY_STANDARD.md`](DEPENDENCY_REGISTRY_STANDARD.md);
- material implementation change memakai [`CHANGE_IMPACT_DECLARATION_STANDARD.md`](CHANGE_IMPACT_DECLARATION_STANDARD.md);
- generated status summary mengikuti [`CURRENT_STATE_SUMMARY_STANDARD.md`](CURRENT_STATE_SUMMARY_STANDARD.md).

## Legacy identity classes

- `LS-WS-NNNN` = immutable original legacy source identity.
- `LX-WS-NNNN-<ROLE>-NN` = immutable exact role extract from that source.
- C/R/P/B/S/Q labels remain historical correlation keys only and must not be rewritten as `WS-Bxx-Axxx` Attempt IDs.

## Current Verification Rebaseline Rule

Current verification is governed by [`CURRENT_VERIFICATION_REBASELINE_STANDARD.md`](CURRENT_VERIFICATION_REBASELINE_STANDARD.md) and epoch `WS-REBASELINE-20260819-001`.

- pre-epoch evidence/decision/status keeps historical content but has no current verification effect;
- behavior-bearing existing implementation starts `NOT_ASSESSED_REVALIDATION_REQUIRED`;
- current proof requires current Work/Attempt/Baseline provenance;
- status reset is not a reason to move a file out of its permanent role folder.
