...
## Current Verification Epoch Binding

Every issued implementation/audit Work Baseline MUST include `verification_epoch` and it MUST equal the active value in [`CURRENT_VERIFICATION_EPOCH.json`](CURRENT_VERIFICATION_EPOCH.json), currently `WS-REBASELINE-20260819-001`. A baseline from a prior epoch cannot produce current `SATISFIED` evidence.
