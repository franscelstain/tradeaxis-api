...
## Current Verification Status Is Not a Second Role

`CURRENT_VERIFICATION_REGISTRY.csv` records lifecycle/verification disposition and **does not create a second document role**. A historical-only `EVIDENCE` file remains role `EVIDENCE`; an unverified technical contract remains role `IMPLEMENTATION_CONTRACT`. Placement continues to follow role, not PASS/FAIL/DONE status.

## Verification Registry

[`CURRENT_VERIFICATION_REGISTRY.csv`](CURRENT_VERIFICATION_REGISTRY.csv) stores current status/proof eligibility without changing a document's single authoritative role. Role and verification status are intentionally separate dimensions.
