...
`DONE` hanya sah bila **objective dan exit criteria stage benar-benar tercapai**. Untuk implementation stage, recurring residue/conformance gate pada [`IMPLEMENTATION_RESIDUE_AND_CONFORMANCE_STANDARD.md`](IMPLEMENTATION_RESIDUE_AND_CONFORMANCE_STANDARD.md) juga wajib conformant: unresolved harmful residue memblokir `DONE`.
...
Do not repeat:
Remaining gap:
Known residue / residue finding:
Residue conformance verdict:
Next action:
Resume from:
```
...
Attempt final evidence berada di `../../records/evidence/` dan menjadi immutable setelah issued. Stage register hanya menyimpan pointer/ringkasannya.
...
3. [`IMPLEMENTATION_RESIDUE_AND_CONFORMANCE_STANDARD.md`](IMPLEMENTATION_RESIDUE_AND_CONFORMANCE_STANDARD.md);
4. [`../../development/implementation/WS_IMPLEMENTATION_STAGE_REGISTER.md`](../../development/implementation/WS_IMPLEMENTATION_STAGE_REGISTER.md);
5. latest closed attempt evidence;
6. semua **OPEN/ACCEPTED unresolved findings** yang terkait langsung;
7. current decision/remediation yang masih berlaku;
8. `DOCUMENT_CHANGE_LOG.md` sejak attempt terakhir;
9. residue evidence/known residue dari attempt sebelumnya;
10. historical/superseded record **hanya jika direferensikan** oleh evidence/finding/decision current.
...
Known harmful/compatibility residue:
Latest residue conformance verdict:
...
9. ada explicit reviewed decision di `../../records/decisions/` yang menyetujui terminal closure;
...
[`../../development/implementation/WS_IMPLEMENTATION_STAGE_REGISTER.md`](../../development/implementation/WS_IMPLEMENTATION_STAGE_REGISTER.md)
...
- known residue / latest residue evidence;
- successor jika ada;
- resume point;
- last update.
...
| residue discovered | classify per residue standard; harmful material → finding/remediation; update stage register pointer |
| compatibility residue retained | exact mapping + isolation + tests + evidence |
| stage objective achieved | stage register `DONE` + evidence + residue/conformance gate + status/tracker sync |
...
13. menyatakan implementation stage `DONE` sementara harmful residue masih unresolved;
14. mengklaim legacy path dead hanya dari pendapat/grep tanpa reachability evidence;
15. mempertahankan compatibility residue tanpa current semantic mapping + tests.
...
- recurring residue/conformance check telah dilakukan;
- tidak ada unresolved harmful residue sebelum implementation-stage `DONE`;
- compatibility residue yang dipertahankan mempunyai mapping + tests + evidence;
...
## Work Baseline Lock Before an Attempt

Sebelum attempt berubah dari read-only discovery menjadi implementation/rework/validation work:

1. issue Work Baseline Lock sesuai [`WORK_BASELINE_LOCK_STANDARD.md`](WORK_BASELINE_LOCK_STANDARD.md);
2. assign Baseline ID dan bind ke Attempt ID;
3. simpan baseline sebagai immutable evidence;
4. update stage register `Baseline ID`;
5. jalankan executable integrity gate `PRE_ATTEMPT`.

Baseline tidak dibuat retroaktif. Jika authority relevant berubah selama attempt, catat drift dan rebaseline melalui attempt baru bila material.

## Canonical Attempt Record

Gunakan [`../../development/implementation/examples/WS_STAGE_ATTEMPT_RECORD_TEMPLATE.md`](../../development/implementation/examples/WS_STAGE_ATTEMPT_RECORD_TEMPLATE.md) untuk setiap attempt.

Final record adalah immutable evidence dan wajib memuat Baseline ID + integrity-gate output. Attempt lama tidak diedit ketika rerun dilakukan.

## Executable Integrity Gate Before Closure

Sebelum attempt ditutup dan sebelum stage `DONE`:

- jalankan [`DOCUMENT_INTEGRITY_GATE_STANDARD.md`](DOCUMENT_INTEGRITY_GATE_STANDARD.md);
- gunakan `--baseline` terhadap baseline attempt;
- unresolved gate `FAIL` memblokir closure claim yang membutuhkan conformance;
- narrow registered legacy exception hanya sah sesuai governance registry dan tidak dapat menutupi harmful residue/coverage gap.


## Strategy Traceability Closure Gate

Stage execution juga tunduk pada [`STRATEGY_IMPLEMENTATION_TRACEABILITY_STANDARD.md`](STRATEGY_IMPLEMENTATION_TRACEABILITY_STANDARD.md).

Sebelum implementation stage `DONE`:

- seluruh active mandatory/conditional matrix row yang diverifikasi pada stage tersebut harus `SATISFIED`;
- matrix row yang belum satisfied menjadi explicit rework backlog dan wajib dibaca saat re-entry;
- `DONE` dilarang bila masih ada mandatory `NOT_ASSESSED`, `MAPPED_UNVERIFIED`, `IMPLEMENTED_UNVERIFIED`, `FAILED_REMEDIATION_OPEN`, `WAITING_VERIFIED_DEPENDENCY`, atau `INCONCLUSIVE`;
- evaluation stage dapat `DONE` dengan business/proof verdict `FAIL` hanya bila seluruh traceability rule untuk evaluator/proof tersebut sudah `SATISFIED` dan verdict-nya valid;
- optional CONFIRM row boleh `OPTIONAL_NOT_REQUESTED` tanpa memblokir core stage path.

Stage split/successor tidak boleh membuat unresolved matrix row hilang. Semua residual rule harus dipindahkan secara eksplisit ke verification stage/successor yang sah.

## 16. Correlation, Change Impact, Dependency, and Closure Records

Untuk current/future `WS-Bxx`:

1. Attempt ID = canonical Work/Correlation ID sesuai [`WORK_CORRELATION_AND_RECORD_REGISTRY_STANDARD.md`](WORK_CORRELATION_AND_RECORD_REGISTRY_STANDARD.md).
2. Material change wajib membuka Change Impact Declaration sebelum perubahan dan memverifikasinya sebelum attempt close.
3. Verified dependency yang mengubah/waiting stage wajib mempunyai `DEP-WS-NNN` row di `development/implementation/WS_DEPENDENCY_REGISTRY.csv`.
4. Terminal stage wajib menerbitkan immutable Stage Closure Manifest sesuai [`STAGE_CLOSURE_MANIFEST_STANDARD.md`](STAGE_CLOSURE_MANIFEST_STANDARD.md).
5. Re-entry wajib menggunakan Stage Register + Work Record Registry + Dependency Registry sebelum membuka history yang lebih luas.
6. Relationship integrity gate wajib PASS sebelum attempt close/stage terminal claim.
