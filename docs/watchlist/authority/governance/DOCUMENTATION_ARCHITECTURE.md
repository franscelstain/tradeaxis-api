...
## Legacy normalization boundary

The current root architecture remains `authority -> development -> records`. Original pre-reorganization files are not migrated into current active folders by filename. They are preserved as `LS-*` sources, split into exact `LX-*` role records when materially composite, and only current/active authority or development documents remain outside records.
