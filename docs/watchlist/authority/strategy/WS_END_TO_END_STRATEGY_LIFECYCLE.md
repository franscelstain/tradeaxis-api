...
## EOD Readiness-to-Action Lifecycle Invariant

Cross-stage temporal flow canonical adalah:

`requested_trade_date → Market Data same-date READY/READABLE/FRESH → effective_trade_date → PLAN → qualified TOP_PICKS → intended NEXT_TRADING_SESSION → action-window check → optional CONFIRM`

- Jika authoritative EOD untuk `requested_trade_date` belum siap, `WS-S01` menghasilkan retryable current-run unavailability dan **MUST NOT** meneruskan stale prior-date facts sebagai new PLAN.
- Setelah same-date facts tersedia, Watchlist boleh retry exact `requested_trade_date` walaupun process berjalan pada wall-clock date berikutnya.
- `intended_entry_session` selalu berasal dari producer-owned trading calendar sebagai next governed trading session setelah `effective_trade_date`; weekend/libur tidak dihitung sendiri oleh Watchlist.
- Bila Top Picks selesai sebelum `canonical_entry_cutoff`, action window `OPEN` dan qualified Top Pick dapat membawa `ENTRY_CANDIDATE_NEXT_TRADING_SESSION`.
- Bila Top Picks baru selesai setelah cutoff, hasil boleh disimpan sebagai `VALID_FOR_ANALYSIS_ONLY`/historical EOD recommendation record tetapi action intent harus `ACTION_WINDOW_EXPIRED` dan tidak boleh memakai opportunity yang sudah lewat.
- Expired recommendation **MUST NOT** otomatis digeser ke trading session berikutnya; new opportunity memerlukan EOD recommendation baru untuk governed effective trade date berikutnya.
- Optional CONFIRM hanya dapat beroperasi setelah valid EOD Top Pick ada dan action window masih terbuka; CONFIRM tidak dapat menggantikan EOD recommendation yang gagal terbentuk karena Market Data belum ready.
