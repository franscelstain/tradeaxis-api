...
## Real-World Proof Identity Freeze

Sebelum IS winner dipilih, real-world assumptions berikut menjadi bagian dari frozen candidate/evaluation identity dan tidak boleh diubah setelah outcome dibaca.

- Frozen identity **MUST** memuat supported execution-mode set dan treatment untuk unsupported/unknown mode.
- Frozen identity **MUST** memuat shortened/half-day session comparability behavior tanpa local feature normalization.
- Frozen identity **MUST** memuat complete condition-dependent baseline slippage function, component tiers, non-zero floor, dan adverse-stress profile.
- Frozen identity **MUST** memuat corporate-action economic-return treatment termasuk unsupported complex-action behavior.
- IS evidence **MUST** menghasilkan `SIGNAL_QUALITY` dan `FOLLOW_TOP1`/`FOLLOW_TOP3`/`FOLLOW_TOP5` views dengan same-listing overlap rule yang sudah dibekukan.
- Winner selection **MUST NOT** memperoleh keuntungan tambahan dengan menghitung repeated same-listing recommendation sebagai beberapa concurrent follower positions.
- Edge-concentration stress definitions dan producer-correction sensitivity metrics **MUST** dibekukan sebelum OOS dibuka; threshold proof stage tidak boleh diciptakan setelah melihat OOS flips/concentration.
- Real-world proof identity tetap EOD-only dan **MUST NOT** menambahkan orderbook/realtime dependency untuk menyelamatkan IS result.
