...
## Final EOD Strategy Closure Invariants

- `NO_ACTIONABLE_TOP_PICKS` **MUST** menjadi canonical valid zero-member recommendation outcome ketika seluruh candidate gagal final qualification; human-readable label boleh menyatakan `NO QUALIFIED TOP PICKS`, tetapi state tersebut bukan runtime error.
- Runtime/strategy **MUST NOT** menurunkan threshold, mengubah weight, memaksa quota, melakukan padding, atau carry-forward recommendation lama hanya untuk menghindari `NO_ACTIONABLE_TOP_PICKS`.
- Issued PLAN dan issued recommendation semantic payload **MUST** bersifat immutable; future price outcome, later Market Data correction, strategy revision, atau operator retry tidak boleh retroactively mengubah record yang sudah diterbitkan.
- Dengan exact Market Data publication/revision identity, strategy identity, parameter identity, requested/effective trade date, dan deterministic calculation policy yang sama, core EOD recommendation truth **MUST** reproducible dan menghasilkan candidate state, PLAN, score, qualification, rank, dan Top-Pick membership yang sama.
- Lifecycle timestamp seperti `recommendation_generated_at` boleh berbeda antar-attempt, tetapi perbedaan wall-clock **MUST NOT** mengubah core recommendation truth; hanya explicit temporal/action-window derivation yang boleh berubah bila lifecycle-time input memang berbeda.
- Untuk long-only EOD proof, bila stop dan target sama-sama tersentuh pada satu daily bar setelah valid entry dan causal order tidak dapat ditentukan dari canonical EOD facts, canonical resolution **MUST** konservatif `STOP_FIRST`; strategy tidak boleh membutuhkan intraday/orderbook untuk memecahkan ambiguity tersebut.
