...
## Canonical Next-Trading-Session Terminology

Nama file mempertahankan token `D1` untuk stabilitas referensi dokumentasi, tetapi canonical business meaning **MUST** dibaca sebagai `NEXT_TRADING_SESSION`, bukan `trade_date + 1 calendar day`.

- `intended_entry_session = NEXT_TRADING_SESSION(effective_trade_date)` menggunakan authoritative Market Data calendar.
- Weekend, exchange holiday, shortened/exception session, atau calendar gap tidak boleh diputuskan sendiri oleh CONFIRM/Watchlist.
- Semua referensi `D+1` sebelumnya di dokumen ini adalah shorthand untuk governed next trading session dan tunduk pada section ini bila terjadi ambiguity.

## CONFIRM Relationship to EOD Action Intent

CONFIRM hanya dapat menilai Top Pick yang sudah mempunyai valid EOD recommendation dan current action window yang masih `OPEN`.

- EOD `ENTRY_CANDIDATE_NEXT_TRADING_SESSION` + CONFIRM `ACTIONABLE` = recommendation EOD tetap valid dan current-entry condition terkonfirmasi.
- EOD `ENTRY_CANDIDATE_NEXT_TRADING_SESSION` + CONFIRM `NOT_ACTIONABLE` = jangan entry pada opportunity tersebut; historical EOD Top Pick tetap tidak ditulis ulang.
- EOD action intent + CONFIRM `UNAVAILABLE_RETRYABLE` = recommendation EOD tetap ada, current confirmation belum terbukti, dan retry hanya boleh selama action window masih open.
- Bila action window expired sebelum valid evaluation, state menjadi `EXPIRED_UNCONFIRMED`; CONFIRM tidak boleh menghidupkan kembali opportunity tersebut.
- Jika core EOD recommendation tidak terbentuk karena `MARKET_DATA_UNAVAILABLE_RETRYABLE`, CONFIRM adalah not-applicable dan **MUST NOT** membuat ticker/recommendation pengganti.
- CONFIRM tidak boleh menggeser expired EOD recommendation ke trading session berikutnya; carry-forward hanya melalui separately proven strategy identity, dan canonical baseline saat ini tidak mengaktifkannya.
