...
## Upstream Market-Fact Gap Handling

Eligibility hanya menilai fakta upstream; ia tidak mengisi kekurangan fakta tersebut.

- Eligibility **MUST NOT** mensintesis required factual feature dari raw/related fields ketika canonical producer field missing, sekalipun formula tampak dapat direkonstruksi.
- Missing/invalid hard data-safety/executability fact yang membuat candidate path tidak dapat dinyatakan aman diperlakukan sebagai hard data disqualifier dan berakhir `AVOID` sesuai classification semantics.
- Missing required alpha/quality/score fact dengan upstream row tetap usable dan tanpa hard safety disqualifier dapat berakhir `WATCH_ONLY`, tetapi tidak boleh menerima neutral/default value untuk tetap menjadi recommendation candidate.
- Kebutuhan factual feature baru untuk eligibility menjadi Market Data dependency lebih dahulu; feature tersebut baru dapat menjadi active Weekly Swing rule setelah authoritative producer fact dan controlled strategy identity tersedia.

## Supported Execution Mode and Session Comparability

Candidate eligibility harus membedakan kualitas setup dari kemampuan canonical execution model untuk mewakili market mechanism yang benar-benar berlaku.

- `SUPPORTED_EXECUTION_MODE`: active baseline recommendation candidate **MUST** mempunyai authoritative execution-mode fact yang termasuk dalam frozen supported-mode set untuk normal Regular-Market Weekly Swing execution.
- Unknown execution mode atau mode yang tidak termasuk frozen supported-mode set **MUST** berakhir `AVOID` untuk current recommendation path; keberadaan OHLCV saja tidak cukup untuk menganggap execution semantics didukung.
- Watchlist **MUST NOT** menentukan supported mode dari board/segment label melalui local inference bila Market Data belum menyediakan authoritative execution-mode mapping.
- Future support untuk auction/other execution mechanism adalah material strategy identity baru dan **MUST** mempunyai separately frozen execution model serta IS/OOS/stress/shadow proof sebelum boleh menjadi recommendation-eligible.
- Pada shortened/half-day/exception session, bila seluruh active session-sensitive Market Data features secara producer contract dinyatakan comparable, normal eligibility boleh diterapkan tanpa local normalization.
- Pada shortened/half-day/exception session, bila comparability active session-sensitive feature tidak dapat dibuktikan tetapi tidak ada hard safety/execution disqualifier, affected ticker **MUST** berakhir `WATCH_ONLY` dengan reason `SESSION_FEATURE_COMPARABILITY_UNPROVEN`, bukan menerima local normalization/default value.
- Session yang secara faktual membuat canonical supported execution tidak tersedia tetap merupakan hard executability disqualifier dan **MUST** berakhir `AVOID`.
