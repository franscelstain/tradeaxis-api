...
Finding `../../development/findings/WS_STRATEGY_LAYER_ROLE_LEAKAGE_2026-08-16.md` menunjukkan canonical strategy masih mengandung history/migration wording dan physical implementation details.

## Decision

1. Remove history/migration notes from canonical strategy.
2. Move reason-code, DDL/schema/table, SQL/query, serialization, artifact-layout, and implementation verification details to `../../development/implementation/`.
3. Preserve behavioral evaluation assumptions and acceptance gates in strategy.
4. Rename mixed `12_WS_BACKTEST_SCHEMA_AND_CALIBRATION.md` to `WS_HISTORICAL_EVALUATION_STRATEGY.md` because physical schema is no longer owned by strategy.
5. Preserve pre-cleanup snapshots under `../history/documentation_migrations/2026-08-16_strategy_layer_cleanup/`.
