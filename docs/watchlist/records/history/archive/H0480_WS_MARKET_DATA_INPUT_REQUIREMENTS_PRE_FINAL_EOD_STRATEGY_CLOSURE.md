...
## Real-World Proof Fact Requirements

Watchlist tetap hanya mengonsumsi market facts dari authoritative Market Data. Real-world proof tidak memberi izin untuk membangun factual feature lokal.

- Execution-mechanism/market-mode fact yang menentukan apakah canonical Regular-Market modeled entry/exit didukung **MUST** berasal dari effective-dated Market Data publication; Watchlist tidak boleh memetakan board name ke execution mode melalui hardcoded local table sebagai authority paralel.
- Session-type fact termasuk normal/shortened/half-day/exception session **MUST** berasal dari Market Data calendar/session contract.
- Corporate-action economic facts yang diperlukan untuk return continuity, minimal effective/ex-date or entitlement timing, cash amount bila applicable, dan structural quantity/price terms bila applicable, **MUST** berasal dari point-in-time Market Data corporate-action authority.
- Liquidity, volatility, price/tick, dan session facts yang digunakan condition-dependent slippage model **MUST** berasal dari Market Data; hanya strategy-specific slippage mapping/penalty yang menjadi milik Watchlist.
- Producer correction-sensitivity proof **MUST** membandingkan explicit producer publication/revision identities; Watchlist tidak boleh membuat synthetic corrected dataset sendiri.
- Bila required execution-mode, session-comparability, corporate-action, atau slippage-input fact tidak tersedia, Watchlist **MUST** memakai fail-closed/dependency behavior yang sesuai dan tidak boleh mengganti fakta tersebut dengan local estimate.
- Real-world proof fact requirements tetap **EOD/reference-fact only** dan tidak mensyaratkan historical/live orderbook.
