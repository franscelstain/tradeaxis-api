...
Candidate hasil kalibrasi tidak boleh diteruskan ke activation path tanpa OOS proof yang lulus. Tanpa OOS proof, hasil calibration tetap non-active research/candidate state.
...
Rolling walk-forward dapat digunakan sebagai penguat evidence, tetapi bukan minimum canonical proof kecuali strategy revision kemudian menguncinya. Contoh penguat: train 6 bulan → test 3 bulan secara rolling maju.
...
Perubahan salah satu komponen tersebut menghasilkan evaluation identity berbeda dan tidak boleh disamakan dengan proof lama. Serialization field/string dan physical storage adalah implementation concern.
...
Nama tabel, record layout, artifact allowlist, hash serialization, dan DDL bukan owner strategy dan berada di implementation/evidence contracts.

## Single-Window and Internal Batching Rule (LOCKED)

Satu chronological 70/30 proof adalah satu logical evaluation atas satu explicit range. Long range boleh diproses internal menggunakan bounded reads/batches, tetapi tidak boleh dipecah menjadi beberapa independently calibrated ranges lalu digabungkan setelahnya.
...
Internal batching tidak boleh mengubah split boundary, candidate grid ordering, best-IS selection, atau proof identity.
...
Implementation wajib mempertahankan seluruh invariant strategy pada dokumen ini.
