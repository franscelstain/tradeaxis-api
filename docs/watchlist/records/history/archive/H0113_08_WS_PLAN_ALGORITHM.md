...
4. classify non-eligible ticker sebagai WATCH_ONLY atau AVOID sesuai rule;
5. compute deterministic score components untuk `RECOMMENDATION_CANDIDATES`;
6. compute canonical `score_total`;
7. order recommendation candidates secara deterministik;
8. derive entry reference/band dan predeclared exit/risk plan;
9. freeze final PLAN.
