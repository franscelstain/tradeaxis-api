...
- **Stages:** `WS-S08` untouched OOS → `WS-S09` adverse-friction stress → `WS-S10` forward shadow → `WS-S11` production-use boundary.
- **Consumes:** exactly one frozen best-IS strategy identity.
- **Produces:** proof verdicts for the same material strategy identity.
- **Terminal rule:** production-use review is unavailable unless all required proof stages PASS.
...
### Step 3 — Forward Shadow Full-Flow Validation (`WS-S10`)

Strategy yang lulus OOS + stress harus menjalani forward shadow pada exact frozen strategy identity sebelum production-use review.
