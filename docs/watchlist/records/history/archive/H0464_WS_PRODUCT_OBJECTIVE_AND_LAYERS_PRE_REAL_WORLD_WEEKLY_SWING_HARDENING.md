...
## Canonical Temporal Field Ownership Contract

Bagian ini adalah owner canonical untuk **ownership** field temporal Weekly Swing. Definisi temporal pada section lain tetap berlaku, tetapi bila ada ambiguity mengenai siapa yang menetapkan nilai, siapa yang hanya menyalin, atau siapa yang boleh melakukan derivasi, section ini yang mengendalikan interpretasi.

- Setiap field temporal canonical **MUST** mempunyai tepat satu semantic owner; keberadaan copy field pada payload/domain lain tidak memindahkan ownership.
- `requested_trade_date` — **OWNER: WATCHLIST**. Nilai adalah governed EOD session yang hendak dievaluasi oleh lifecycle Watchlist; caller/scheduler boleh mengusulkan tanggal, tetapi Watchlist yang memvalidasi, resolve, dan merekam request tersebut.
- `effective_trade_date` — **OWNER: MARKET_DATA**. Nilai harus berasal dari authoritative consumer-facing Market Data publication/read model dan tidak boleh dibuat, digeser, atau dikoreksi oleh Watchlist.
- `market_data_published_at` — **OWNER: MARKET_DATA**. Timestamp menyatakan kapan publication identity Market Data yang dikonsumsi menjadi authoritative/readable menurut contract producer.
- `market_data_revision_id` — **OWNER: MARKET_DATA**. Identity revision/publication lineage harus berasal dari producer dan tidak boleh diganti dengan revision identity buatan Watchlist.
- `recommendation_generated_at` — **OWNER: WATCHLIST**. Timestamp dicatat ketika exact recommendation result/version benar-benar selesai dan tersedia; field ini bukan alias `market_data_published_at`.
- `intended_entry_session` — **OWNER: WATCHLIST (STRATEGY-DERIVED)**. Watchlist melakukan derivasi `NEXT_TRADING_SESSION(effective_trade_date)`, tetapi seluruh calendar/session facts yang dipakai derivasi wajib berasal dari authoritative Market Data.
- `canonical_entry_cutoff` — **OWNER: WATCHLIST (STRATEGY-DERIVED)**. Cutoff dihitung dari frozen Weekly Swing minimum decision lead time terhadap authoritative Market Data session/open facts; Market Data tidak menetapkan action cutoff Watchlist.
- `action_window_status` — **OWNER: WATCHLIST**. Minimal `OPEN` atau `EXPIRED`, ditentukan dari `recommendation_generated_at` terhadap `canonical_entry_cutoff` dan tidak boleh dikirim sebagai producer market fact.
- Jika Watchlist menyimpan copy field yang dimiliki Market Data, copy tersebut **MUST** mempertahankan exact producer value dan publication/revision provenance; persistence downstream tidak menciptakan authority baru.
- Watchlist **MUST NOT** fabricate, override, normalize secara semantic, atau infer field temporal milik Market Data; Market Data **MUST NOT** menentukan `recommendation_generated_at`, `canonical_entry_cutoff`, `action_window_status`, atau lifecycle recommendation Watchlist.
- Missing/invalid Market-Data-owned temporal field yang required untuk active path harus menghasilkan unavailable/dependency state sesuai strategy; Watchlist tidak boleh mengisinya dari wall-clock time, prior-date publication, latest row, atau local calendar.
- Timestamp canonical (`market_data_published_at`, `recommendation_generated_at`, dan cutoff timestamp) **MUST** timezone-aware dan dapat dibandingkan sebagai instant yang tidak ambigu; `requested_trade_date`, `effective_trade_date`, dan `intended_entry_session` tetap session/date identities, bukan timestamp.
- Recommendation yang sudah issued tetap terikat pada Market Data publication/revision identity yang dipakai saat pembentukan; producer correction/revision berikutnya tidak boleh menulis ulang recommendation lama secara diam-diam dan harus masuk melalui explicit re-evaluation/new recommendation lineage bila strategy meminta evaluasi ulang.

Ownership contract ini tidak mengubah Watchlist menjadi producer Market Data. Ia hanya memastikan bahwa temporal facts dan Weekly Swing lifecycle timestamps tidak saling mengambil authority.
