...
> **Purpose:** prevent filename-only migration, preserve semantic provenance, and avoid retaining duplicate composite files after verified decomposition.
...
A physical legacy source may remain only when it is not fully split or when an explicit bundle exception requires the composite container. Bundle exceptions are listed in `records/history/LEGACY_BUNDLE_EXCEPTION_INDEX.csv`.

Retention is not current authority. It only means the source has not been replaced by a sealed complete decomposition.
