# Watchlist Weekly Swing — Top Picks Qualification and Ranking
...
- **Consumes:** immutable PLAN `RECOMMENDATION_CANDIDATES`.
- **Produces:** exact final Top Picks membership, score, and rank `1..N`.
- **Next:** `WS-S06+` core proof evaluates this exact output; optional `WS-S05` CONFIRM may evaluate current actionability without blocking the proof path.
...
`score_total` mengikuti formula dan frozen component transforms/weights di `WS_PLAN_SCORING_AND_TRADE_PLAN.md`.
...
## J. User-Visible Top-K Ranking Proof

Karena consumer dapat memusatkan perhatian pada rank teratas, canonical proof harus mengevaluasi presentation subsets tanpa mengubah membership:

- `TOP_1` = rank `1` pada setiap recommendation date bila tersedia;
- `TOP_3` = first `min(3,N)` recommendations;
- `TOP_5` = first `min(5,N)` recommendations;
- `ALL_QUALIFIED` = seluruh final Top Picks.

Subset definition dibekukan sebelum outcome dibaca. Proof harus melaporkan sample, net return, downside, dan benchmark-relative behavior untuk Top-1/Top-3/Top-5/All. Higher-ranked subsets harus menunjukkan positive expected net return dan tidak menunjukkan persistent underperformance terhadap all-qualified set yang mengindikasikan ranking tidak berguna.
...
4. ranking tidak dipengaruhi capital atau optional CONFIRM;
