# WS C07 Operator Validation Commands

Status: EXECUTED_CURRENT_SESSION / IS_QUALITY_FAILED / C07_REJECTED_AS_STRATEGY_CATALOG
Scope: C07 strategy-quality redesign, runtime feature audit, seed, and IS calibration validation
...
PHP lint for C07 files:

```powershell
php -l app/Application/Watchlist/Services/WatchlistBacktestC07ParamGridCatalog.php
...
php -l app/Console/Commands/Watchlist/SeedBacktestC07ParamGridCommand.php
php -l database/seeders/Watchlist/WatchlistBacktestC07ParamGridSeeder.php
php -l app/Console/Kernel.php
php -l tests/Unit/Watchlist/WatchlistBacktestC07ParamGridCatalogTest.php
php -l tests/Unit/Watchlist/WatchlistBacktestC07ParamGridParamsetFactoryTest.php
php -l tests/Unit/Watchlist/WatchlistBacktestC07StaticGuardTest.php
```
...
```powershell
php artisan watchlist:backtest-c07-param-grid-seed
```
...
C07 IS calibration run 1:

```powershell
php artisan watchlist:backtest-is-calibrate --catalog-code=WS_BT_GRID_DOWNSIDE_STABILITY_C07_2026_06 --from=2023-01-02 --to=2025-05-21 --output=storage/app/watchlist/backtest/c07-is-run-1.json --overwrite
```
...
C07 IS calibration run 2:

```powershell
php artisan watchlist:backtest-is-calibrate --catalog-code=WS_BT_GRID_DOWNSIDE_STABILITY_C07_2026_06 --from=2023-01-02 --to=2025-05-21 --output=storage/app/watchlist/backtest/c07-is-run-2.json --overwrite
```
...
The two IS commands exited non-zero because C07 failed strategy quality. That is expected for a no-valid-candidate result and is not a strategy PASS.
...
C07 current result does not meet those strategy-quality pass criteria. OOS has not been run and must not be claimed PASS.
