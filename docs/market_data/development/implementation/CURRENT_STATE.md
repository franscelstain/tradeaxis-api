...
- Required active traceability rows: **3135**
- Coverage denominator: **3111** (PROVISIONAL)
- SATISFIED: **2284**
...
- Verified coverage: **73.42% PROVISIONAL**
- Optional capability rules: **63**

## Current executable stage

- Stage: `MD-B09`
- Latest attempt / baseline: `MD-B09-A002` / `MD-B09-A002-BL001`
- State / verdict: `DONE` / `PASS`
- Residue/rework: `CONFORMANT_NO_HARMFUL_RESIDUE_FOUND_IN_THE_B09_SURFACE`
- Dependency: `MD-DEP-0008 RESOLVED`; `MD-DEP-0004` B09 entry complete, remains `439 / 11` downstream
- Open finding: none — `F-MD-B09-A001-001` RESOLVED
- Change Impact Declaration: `CI-MD-B09-A002-001` — ISSUED
- Denominator: **139** (FINAL for every machine-checked criterion — no transitional applicability, no mixed-classification run)
- SATISFIED / NOT_ASSESSED: **139 / 0**
- Mandatory / conditional-applicable: **139 / 0**
...
- Registered current work records: **152** (BASELINE_LOCK=37, CHANGE_IMPACT_DECLARATION=32, DECISION=7, EVIDENCE=44, FINDING=14, STAGE_CLOSURE_MANIFEST=18)

## Exact resume

- Single exact next executable resume point: harden `MarketDataClassificationConsistencyGate` so an obligation-bearing paragraph filed as `REFERENCE_ONLY` outside a mixed enumerated run is detected, and so a stage pass that unbinds another stage's closed proof fails. Two stages have now been corrected by hand for defects of exactly these shapes; `MD-B09` (2), `MD-B11` (15) and `MD-B12` (5) remain, and doing them by hand first repeats the manual search three more times.
