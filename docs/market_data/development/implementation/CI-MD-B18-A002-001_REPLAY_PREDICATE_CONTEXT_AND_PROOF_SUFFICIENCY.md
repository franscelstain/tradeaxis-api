...
### MD-S065-R0003 controlled revision and ownership transfer declaration BEFORE mutation - 2026-09-25T14:04:28+07:00

Trigger: the owner chose A1 + C1 + D2 for the `MD-S065-R0003` override mechanism left open by `D-MD-B18-A002-009` (`E-MD-B18-A002-077` G09 stop, `F-MD-B18-A002-017`), and explicitly authorised "the minimum bounded controlled revision necessary to encode A1+C1+D2". A1: a governed correction may be approved by `system`, but a configuration override exists only when it is explicitly requested and recorded on that correction. C1: fail closed; when the configuration effective for `D` differs from live executable configuration and no override exists, the run is `BLOCKED`; `assertConsumedConfiguration()` stays fail-closed. D2: primary implementation/proof ownership of `MD-S065-R0003` moves to `MD-B21` (with `MD-S082-R0207`/`R0209`); `MD-B18` stays supporting; the requirement stays `MANDATORY` and is not proven.

Affected areas: strategy `Config_Change_Protocol_LOCKED.md` (`MD-S065`) line 7 only, extended in place with the original sentence kept as its prefix, so the corpus keeps five rows and no rule id is added; `MARKET_DATA_STRATEGY_FREEZE_MANIFEST.json` (successor `MD-STRATEGY-FREEZE-20260925-001`, only the `MD-S065` fingerprint changes); `MARKET_DATA_STRATEGY_AUTHORITY_REGISTRY.csv`; `DOCUMENT_CHANGE_LOG.md` (`DOC-CHG-20260925-001`); traceability matrix row `MD-S065-R0003` only (rule text, fingerprint, primary `MD-B21`, supporting `MD-B18;MD-B04`, appended ownership note); governance tooling `MarketDataConfigFoundationTraceabilitySpec.php` (owner `MD-B18` -> `MD-B21`), `MarketDataReplayVerificationTraceabilitySpec.php`, `MarketDataReplayVerificationProofSpec.php`, `MarketDataReplayVerificationNormalization.php` (denominator/population derived from the matrix by the gates, plus a fail-closed ownership check for the transferred row, as `D005` added for `MD-S020-R0014`), `MarketDataReplayVerificationProofBasis.php` (`INCOMPLETE` -> `TRANSFERRED_OWNERSHIP`, audit only); decision `D-MD-B18-A002-010`; evidence `E-MD-B18-A002-078` (correction of `E-077`'s unsupported "843 recompute runs" statement) and `E-MD-B18-A002-079` (revision, transfer and validation); four document registries; `F-017`; `MD-DEP-0017`; Stage Register `MD-B18`/`MD-B21` rows; generated `CURRENT_STATE.md`.

Not affected: application code, including `resolveForRun`, `getOrCreateOwningRun`, `createPromoteRunFromSeed`, `assertConsumedConfiguration` and the correction commands; schema; migrations; runtime configuration; PHPUnit tests under `tests/`; every other strategy document; `MD-S065` rows `R0001`/`R0002`/`R0004`/`R0005`; `MD-S082-R0207`/`R0209` (not built here); `MD-S050-R0005`/`R0014`, `MD-S019-R0071`; `F-013` status; `G05`/`G07`; formal `0/114` `SATISFIED`; every immutable record (730 before this unit), including `E-077` (sha256 `0bb9cefc...abc02`) and `D-009` (sha256 `a5a430c0...5ac3b`).

Compatibility / residue risk: none at runtime, because no runtime behaviour changes; the platform stays non-conformant with `MD-S065-R0003` (a rerun binds live configuration) until `MD-B21` implements the rule, and that gap is recorded, not closed. The B18 denominator and population change only because one row changes owner; any count change is derived from the gates, not asserted. The `MD-S065-R0002` (`SATISFIED` under `MD-B04`) concern already reported by `D-009` is re-examined and reported, not acted on. Verification: documentation integrity, strategy freeze, traceability matrix, relationship integrity and self-test, classification consistency, applicability, finding consistency, scope/orchestration, B04 traceability/proof, B18 normalization/proof/readiness, `CURRENT_STATE` determinism. The new ownership check must be shown to fail on a landed single-row mutation with green controls before and after and byte restoration. No full `MarketData` suite: no runtime behaviour changes.

### D-010 / DOC-CHG-20260925-001 issued result - 2026-09-25T14:24:23+07:00

Issued `D-MD-B18-A002-010` (A1+C1+D2), `E-MD-B18-A002-078` (correction of `E-077`'s unsupported "843 recompute runs" statement; `E-077` byte-identical) and `E-MD-B18-A002-079` (revision, transfer, probes, validation). `MD-S065` line 7 extended in place, CRLF preserved; successor freeze `MD-STRATEGY-FREEZE-20260925-001` with only the `MD-S065` fingerprint changed (`FBAC0291...` -> `28962247...`); authority registry and change log updated. Matrix changed exactly one physical row (`MD-S065-R0003`: text/fingerprint, primary `MD-B21`, supporting `MD-B18;MD-B04`, ownership note; still `REQUIRED`/`MANDATORY`/`NOT_ASSESSED`, no evidence). Tool-derived: B18 denominator 114 -> 113, stage population 153 -> 152, `bound_inputs` 21 -> 20, proof basis 91 `PROVEN` / 22 `INCOMPLETE` (23 before), readiness 91/113; B04 114/181/645 unchanged; B21 matrix population 54 -> 55 `MANDATORY`. Four ownership probes caught (landed 1 each; failure sets compared against a control that is red only for the pre-existing `CONDITION_EXECUTION_STALE` residue; byte restoration verified). 730 pre-unit immutable records and 90 other strategy documents byte-identical. Two records were regenerated once before registration and citation (`E-078` freeze id, `E-079` line count); both first writes are recorded in `E-079`. No application, PHPUnit test, schema, migration or runtime-config change; full `MarketData` suite not run (no runtime behaviour change). Formal `0` `SATISFIED`.

Single exact resume: MD-B18-A002: `F-MD-B18-A002-017` `G07` -- `MD-S002-R0003`, per consolidated package section 7; not started.
