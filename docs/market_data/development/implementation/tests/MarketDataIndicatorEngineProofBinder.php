...
/**
 * Governed `MD-B14` proof binder.
 *
 *   --validate-only                      report the pre-bind state and exit
 *   --evidence-id=E-MD-B14-A001-NNN      bind every mandatory predicate to that evidence
...
if ($validateOnly) {
    $result = MarketDataIndicatorEngineProofGate::validate($root, false);
    echo json_encode([
        'mode' => 'VALIDATE_ONLY',
...
if ($evidence === null || preg_match(MarketDataIndicatorEngineProofGate::EVIDENCE_PATTERN, $evidence) !== 1) {
    throw new RuntimeException('Use --validate-only, or --evidence-id=E-MD-B14-A001-NNN [--apply].');
}

$matches = glob($root.'/docs/market_data/records/evidence/'.$evidence.'_*');
if (count($matches) !== 1) {
    throw new RuntimeException('Exactly one governed MD-B14 evidence file must exist for '.$evidence
        .', found '.count($matches).'.');
}
...
foreach ($rows as &$row) {
    ...
    if ($row['primary_stage'] !== $spec::STAGE
        ...
        throw new RuntimeException($row['rule_id'].': cannot bind a proof to a row that is not an active MD-B14 mandatory rule.');
    }
...
$check = MarketDataIndicatorEngineProofGate::validate($root, true, ['mandatory' => $boundRows]);
