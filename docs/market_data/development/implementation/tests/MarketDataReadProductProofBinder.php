...
/**
 * Governed `MD-B17` proof binder.
 *
 *   --validate-only                      report the pre-bind state and exit
 *   --evidence-id=E-MD-B17-A002-NNN      bind every mandatory predicate to that evidence
...
if ($validateOnly) {
    $result = MarketDataReadProductProofGate::validate($root, false);
    echo json_encode([
        'mode' => 'VALIDATE_ONLY',
...
if ($evidence === null || preg_match(MarketDataReadProductProofGate::EVIDENCE_PATTERN, $evidence) !== 1) {
    throw new RuntimeException('Use --validate-only, or --evidence-id=E-MD-B17-A002-NNN [--apply].');
}

$matches = glob($root.'/docs/market_data/records/evidence/'.$evidence.'_*');
if (count($matches) !== 1) {
    throw new RuntimeException('Exactly one governed MD-B17 evidence file must exist for '.$evidence
        .', found '.count($matches).'.');
}
...
$bindable = $spec::EXPECTED_DENOMINATOR;
...
foreach ($rows as &$row) {
    ...
    if ($row['primary_stage'] !== $spec::STAGE
        ...
        throw new RuntimeException($row['rule_id'].': cannot bind a proof to a row that is not an active MD-B17 mandatory rule.');
    }
...
if ($seen !== $bindable) {
    throw new RuntimeException('Binder reached '.$seen.' of '.$bindable.' provable predicates.');
}
if ($bound !== $bindable) {
    throw new RuntimeException('Binding must be atomic: bound '.$bound.' of '.$bindable.'.');
}
...
if (count($stageRows) !== $spec::EXPECTED_DENOMINATOR) {
    throw new RuntimeException('The MD-B17 mandatory denominator is '.count($stageRows)
        .', expected '.$spec::EXPECTED_DENOMINATOR.'.');
}
...
$check = MarketDataReadProductProofGate::validate($root, true, ['mandatory' => $stageRows]);
...
echo json_encode([
    ...
    'denominator' => count($stageRows),
    'bound' => $bound,
    'not_assessed' => count($stageRows) - $bound,
