...
final class MarketDataReplayVerificationProofBasis
{
    ...
    public const INCOMPLETE = [
        ...
        'MD-S050-R0016' => [
            ...
            'basis' => 'PARTIAL -- not PROVEN. G03 (E-MD-B18-A002-067) fixed and proved the publication-mode service half for the inputs it names: after the VERIFIED check, replayAdmissibility() blocks an empty source observation, canonical raw input, temporal, formula/reason registry, read-model, serialization or build identity as REPLAY_BOUND_INPUT_INCOMPLETE naming the field, and the direct write refuses a non-BLOCKED result missing any of the seven input hashes in either mode; those guards and their four probes stand. The closure audit (E-MD-B18-A002-068) found the predicate still open on two required inputs. Gap A (config identity) is now closed (E-MD-B18-A002-069): Platform_Config_Registry_LOCKED.md defines CONFIG_UNBOUND as no non-null config snapshot ID *and hash*, and "publication replay over them is BLOCKED, not PASS"; a run with config_snapshot_id but no config_hash previously resolved config_snapshot_hash to the CONFIG_IDENTITY_UNRECORDED marker (a non-empty string R0016\'s own empty-string check could not catch) and replayed ADMISSIBLE/PASS, and the direct write accepted the marker as a non-empty value. replayAdmissibility() now blocks when configIdentityForRun($run) resolves to that marker, scoped inside the "there is a publication to reproduce" branch so a non-readable run with no publication (an EXPECTED_DEGRADE fixture) is not converted into a config-unbound one; ReplayResultRepository::assertModeInputs() refuses a non-BLOCKED result carrying the marker. Four real-path guards (BLOCKED on a missing hash, no current-publication fallback, direct-write refusal, and the pre-existing full corpus as the legitimate-identity control) and three probes (admission check removed, storage check bypassed, a hypothetical current/latest fallback introduced) each isolated to their own target, byte-restored and sha256-verified. Gap B remains open and is not addressed here: AsKnownReplaySnapshotService reads governance.read_model_version, which config/market_data.php does not define, so every AS_KNOWN replay records it empty and can PASS; the direct-write check omits read_model_version in both modes; AS_KNOWN reason-registry identity is a hash of nominal state names plus an undefined revision -- the same disguised-missing shape, needing an owner decision (fail-closed AS_KNOWN vs. a real bound as-known identity) rather than an implementation choice. Calendar/status and event/factor sub-components remain outside this admission check: their completeness is enforced at binding with named missing paths (ProducerInputCompletionManifest; B18ProducerCalendarRegistryTest, B18ProducerEventFactorCaptureTest, B18ProducerTradingStatusPopulationTest), and a context missing one never becomes VERIFIED, so replay is already BLOCKED as REPLAY_BOUND_INPUT_CONTEXT_UNAVAILABLE. MD-S050-R0016 therefore remains INCOMPLETE.',
        ],
