...
final class MarketDataReplayVerificationProofBasis
{
    ...
    public const INCOMPLETE = [
        ...
        // F-MD-B18-A002-017 (G03 closure audit, E-MD-B18-A002-068): returned to INCOMPLETE. The E-067
        // promotion was premature -- the config hash marker and the AS_KNOWN read-model version remain
        // required inputs that can be missing without BLOCKED.
        'MD-S050-R0016' => [
            'positive' => 'B18ReplayComparisonExhaustivenessTest::test_a_verified_publication_missing_a_required_input_is_blocked',
            'negative' => 'B18ReplayComparisonExhaustivenessTest::test_an_unavailable_observation_identity_is_not_filled_from_current_state',
            'basis' => 'PARTIAL -- not PROVEN. G03 (E-MD-B18-A002-067) fixed and proved the publication-mode service half for the inputs it names: after the VERIFIED check, replayAdmissibility() blocks an empty source observation, canonical raw input, temporal, formula/reason registry, read-model, serialization or build identity as REPLAY_BOUND_INPUT_INCOMPLETE naming the field, and the direct write refuses a non-BLOCKED result missing any of the seven input hashes in either mode; those guards and their four probes stand. The closure audit (E-MD-B18-A002-068) found the predicate still open on two required inputs. (1) Configuration hash: Replay_Verification_Contract_LOCKED.md:29 requires the full configuration snapshot ID/hash, and Platform_Config_Registry_LOCKED.md defines a run with no non-null snapshot ID and hash as CONFIG_UNBOUND, whose publication replay is BLOCKED because a required bound input is absent. A run with config_snapshot_id but no config_hash resolves config_snapshot_hash to the CONFIG_IDENTITY_UNRECORDED marker and replays ADMISSIBLE/PASS; the storage check accepts the marker as non-empty. Blocking it turned 41 exhaustiveness and 11 ReplayVerificationServiceTest tests red, because their fixtures pass on the marker. (2) Read-model version (line 30) in AS_KNOWN: AsKnownReplaySnapshotService reads governance.read_model_version, which config/market_data.php does not define, so every AS_KNOWN replay records it empty and can PASS; the direct-write check omits read_model_version in both modes. AS_KNOWN reason-registry identity is a hash of nominal state names plus an undefined revision -- the same disguised-missing shape. Calendar/status and event/factor sub-components are outside this admission check: their completeness is enforced at binding with named missing paths (ProducerInputCompletionManifest; B18ProducerCalendarRegistryTest, B18ProducerEventFactorCaptureTest, B18ProducerTradingStatusPopulationTest), and a context missing one never becomes VERIFIED, so replay is already BLOCKED as REPLAY_BOUND_INPUT_CONTEXT_UNAVAILABLE.',
        ],
