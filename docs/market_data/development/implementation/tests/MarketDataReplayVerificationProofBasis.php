...
'basis' => 'PARTIAL -- not PROVEN. G03 (E-MD-B18-A002-067) proved the publication-mode service half: after the VERIFIED check, replayAdmissibility() blocks an empty source observation, canonical raw input, temporal, formula/reason registry, read-model, serialization or build identity as REPLAY_BOUND_INPUT_INCOMPLETE, and the direct write refuses a non-BLOCKED result missing any of those in either mode. Gap A (E-MD-B18-A002-069): a config snapshot ID with no hash resolves to the honest CONFIG_IDENTITY_UNRECORDED marker and is now blocked rather than accepted as non-empty, at both admission and direct write. Gap B1 (E-MD-B18-A002-071, D-MD-B18-A002-008): AS_KNOWN read_model_version binds MarketDataReadProductService::READ_MODEL_VERSION instead of a nonexistent config key, required at direct write in both modes. Gap B2 (E-MD-B18-A002-071): AS_KNOWN reason_registry_hash is now the honest REASON_REGISTRY_IDENTITY_UNAVAILABLE marker instead of a hash of two hardcoded arrays; verifyAsKnownAgainstFixture() returns BLOCKED before invoking the production canonicalizer whenever it captures that marker -- which is unconditional, since MD-S085 defines no historical reason-registry identity any as-known replay could ever bind -- and the direct write refuses a non-BLOCKED result carrying it, in both modes. Real-path proof: the base (non-perturbed) as-known scenario is BLOCKED, not PASS; a perturbed-fixture scenario is still BLOCKED, not MISMATCH/FAIL; inserting a new row into the live eod_reason_codes table does not make a historical as-known replay admissible (current state cannot leak in); direct-write refusal in both modes. Four probes (restore the old nominal hash, bypass the BLOCKED decision, a live-table current/latest fallback, storage check removed) each isolated to their own target, byte-restored and sha256-verified; publication-mode controls unaffected by every probe. Bounded check for this review only, by direct code inspection (not independently proven with an executed probe): AS_KNOWN\'s config_snapshot_hash cannot silently resolve empty either -- MarketDataConfigSnapshotRepository::resolveAsKnown() throws CONFIG_SNAPSHOT_NOT_KNOWN_AT_CUTOFF when no snapshot was recorded at or before the cutoff, rather than returning a degenerate row. Remaining, explicitly not verified in this unit: whether AS_KNOWN\'s calendar/status, event/factor, temporal universe, source-observation and canonical-raw-input captures are equally fail-closed rather than merely usually-populated -- calendar/status/event genuinely have historical revision tracking (unlike the reason registry), so no structural "can never be bound" gap is expected, but none of the five was independently probed for a missing-input case in this unit, and MD-S050-R0016 is not promoted on an assumption. MD-S050-R0016 therefore remains INCOMPLETE.',
        ],
