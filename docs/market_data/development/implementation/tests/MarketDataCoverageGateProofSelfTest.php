...
/**
 * Falsifiability control for the `MD-B15` proof gate.
 *
...
$fails = static function (array $overrides) use ($root, $bound) {
    return MarketDataCoverageGateProofGate::validate($root, $bound, $overrides)['status'] === 'FAIL';
};

$checks = [];
$checks[$bound ? 'control_bound' : 'control_prebinding'] =
    MarketDataCoverageGateProofGate::validate($root, $bound)['status'] === 'PASS';
...
$result = [
    'gate' => 'MarketDataCoverageGateProofSelfTest',
    'stage_id' => $spec::STAGE,
