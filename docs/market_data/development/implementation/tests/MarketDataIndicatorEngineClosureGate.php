...
require_once __DIR__.'/MarketDataIndicatorEngineProofGate.php';

/**
 * `MD-B14` stage-closure condition gate.
 *
 ...
 *
 * It is a gate, not a report: any unmet condition exits non-zero. `MD-B01-A014` and the sixteen
 * recorded guard failures before it are the reason this is executable instead of a checklist a
 * reader ticks.
 */
final class MarketDataIndicatorEngineClosureGate
{
    public const EVIDENCE = 'E-MD-B14-A001-001';

    public const ARTIFACT_DIR = 'storage/app/market-data/evidence/MD-B14-A001';

    /** @return array<string,mixed> */
    public static function validate(string $root): array
    {
        $spec = 'MarketDataIndicatorEngineProofSpec';
        $traceability = 'MarketDataIndicatorEngineTraceabilitySpec';
        ...
        foreach ($stageRows as $row) {
            ...
            if ($row['coverage_status'] !== 'SATISFIED'
                || preg_match(MarketDataIndicatorEngineProofGate::EVIDENCE_PATTERN,
        ...
        foreach ($stageRows as $row) {
            ...
            // A rationale is not enough on its own: the condition must be stated, shown false, and
            // held false by something executable. A recorded NOT_APPLICABLE nobody re-checks is how
        ...
        // This stage bound predicates that were all NOT_ASSESSED at entry, and its normalization
        // altered no row owned by another stage. Both are asserted rather than assumed.
        $foreign = [];
        ...
        return [
            'gate' => 'MarketDataIndicatorEngineClosureGate',
            'stage_id' => $spec::STAGE,
...
if (realpath(isset($_SERVER['SCRIPT_FILENAME']) ? $_SERVER['SCRIPT_FILENAME'] : '') === __FILE__) {
    $result = MarketDataIndicatorEngineClosureGate::validate(dirname(__DIR__, 5));
