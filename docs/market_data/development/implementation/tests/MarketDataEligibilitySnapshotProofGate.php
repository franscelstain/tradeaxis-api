...
require_once __DIR__.'/MarketDataEligibilitySnapshotProofSpec.php';

/**
 * Governed `MD-B16` proof gate. Usage: php this-file.php [--bound].
 *
 * Without `--bound` it asserts the stage is pristine: every mandatory predicate unassessed and
 * unbound. With `--bound` it asserts every one of them is SATISFIED against a governed `MD-B16`
 ...
 */
final class MarketDataEligibilitySnapshotProofGate
{
    public const EVIDENCE_PATTERN = '/^E-MD-B16-A001-\d{3}$/';
    ...
    public static function validate(string $root, bool $bound = false, array $overrides = []): array
    {
        ...
        foreach ($entries as $entry) {
            ...
            if (strpos((string) (isset($family['owner']) ? $family['owner'] : ''), 'MD-B16:') !== 0) {
        ...
        return [
            'gate' => 'MarketDataEligibilitySnapshotProofGate',
            'stage_id' => $spec::STAGE,
...
if (realpath(isset($_SERVER['SCRIPT_FILENAME']) ? $_SERVER['SCRIPT_FILENAME'] : '') === __FILE__) {
    $result = MarketDataEligibilitySnapshotProofGate::validate(
        dirname(__DIR__, 5),
