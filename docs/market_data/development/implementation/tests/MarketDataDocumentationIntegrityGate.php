...
$vr=csvrows($md.'/authority/governance/CURRENT_VERIFICATION_REGISTRY.csv');$vseen=array();$legacyProof=array();foreach($vr as $r){$vseen[$r['document_path']]=1;if($r['legacy_origin']==='YES'&&$r['document_role']!=='STRATEGY'&&$r['current_proof_eligible']==='YES')$legacyProof[]=$r['document_path'];}$vmiss=array_values(array_diff($physical,array_keys($vseen)));add($checks,'CURRENT_VERIFICATION_REBASELINE',!$vmiss&&!$legacyProof,array('registry'=>count($vr),'missing'=>array_slice($vmiss,0,20),'legacy_current_proof'=>$legacyProof));
// JSON CSV parse
