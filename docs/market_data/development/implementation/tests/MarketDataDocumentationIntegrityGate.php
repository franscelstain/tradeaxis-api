...
// Q6 / D-MD-B18-A002-005: checking emits a report, never replaces an issued record.
// Callers may retain stdout in their governed runtime artifact location.
$fail=false;foreach($checks as $c)if($c['status']==='FAIL')$fail=true;$out=array('gate'=>'MarketDataDocumentationIntegrityGate','status'=>$fail?'FAIL':'PASS','checks'=>$checks,'generated_at'=>date(DATE_ATOM));echo json_encode($out,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES).PHP_EOL;exit($fail?1:0);
