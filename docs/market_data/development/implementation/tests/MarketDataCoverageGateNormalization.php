...
/**
 * Governed MD-B15 stage-entry normalization. Usage: php this-file.php [--apply].
 *
 * Carries the four guards earlier stages needed, each added after a real failure:
 *
 *   - full-scope coverage: every active row the matrix assigns to MD-B15 must be examined by this
...
$S = 'MarketDataCoverageGateTraceabilitySpec';
...
foreach ($rows as &$row) {
    ...
    if (isset($S::ENTRY_MANDATORY[$rid]) || isset($S::ENTRY_PROMOTED[$rid])) {
        ...
        $row['notes'] = $S::ATTEMPT
            ...
            .$basis
            .'; stage_entry_review=MD-B15 semantic revalidation';
...
if ($unexamined !== []) {
    throw new RuntimeException('MD-B15-owned rows never examined by this normalization: '
        .implode(', ', $unexamined));
}
$counts['b15_owned_examined'] = count($owned);
...
$counts['b15_denominator'] = $denominator;
