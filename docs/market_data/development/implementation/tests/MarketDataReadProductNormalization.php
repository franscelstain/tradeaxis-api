...
/**
 * Governed MD-B17 stage-entry normalization. Usage: php this-file.php [--apply].
 *
 * Carries the four guards earlier stages needed, each added after a real failure:
 *
 *   - full-scope coverage: every active row the matrix assigns to MD-B17 must be examined by this
...
$S = 'MarketDataReadProductTraceabilitySpec';
...
foreach ($rows as &$row) {
    ...
    if (isset($S::ENTRY_MANDATORY[$rid]) || isset($S::ENTRY_PROMOTED[$rid])) {
        ...
        $row['notes'] = $S::ATTEMPT
            ...
            .$basis
            .'; stage_entry_review=MD-B17 semantic revalidation';
    ...
    if (isset($S::ENTRY_CONDITIONAL_NOT_APPLICABLE[$rid])) {
        $row['coverage_requirement'] = 'REQUIRED';
        $row['applicability'] = 'CONDITIONAL_NOT_APPLICABLE';
        $row['coverage_status'] = 'NOT_APPLICABLE';
        $row['current_evidence_ids'] = '';
        $row['notes'] = $S::ATTEMPT
            .': applicability_normalized=CONDITIONAL_NOT_APPLICABLE; predicate_context='.$row['section']
            .'; normalized_predicate='.$normalize($row)
            .'; proof_owner_confirmed='.$S::STAGE
            .'; applicability_basis=condition_absent: '.$S::ENTRY_CONDITIONAL_NOT_APPLICABLE[$rid]
            .'; condition_guard=ReadProductAliasRetirementBoundaryB17Test::'
            .'test_neither_compatibility_alias_is_retired_without_the_demonstration_the_contract_requires'
            .'; stage_entry_review=MD-B17 semantic revalidation';
...
if ($unexamined !== []) {
    throw new RuntimeException('MD-B17-owned rows never examined by this normalization: '
        .implode(', ', $unexamined));
}
$counts['b17_owned_examined'] = count($owned);
...
$counts['b17_denominator'] = $denominator;
