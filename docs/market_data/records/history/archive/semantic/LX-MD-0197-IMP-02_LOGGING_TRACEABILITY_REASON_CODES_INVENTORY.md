# Legacy Semantic Extract — LX-MD-0197-IMP-02
...
- Source range: `L16-L40`
- Extract body SHA1: `1543BB6673B88616647F588CE3A3974082236203`
...
| source provider/API | source acquisition telemetry and recoverable HELD event | `RUN_SOURCE_TIMEOUT`, `RUN_SOURCE_RATE_LIMIT`, `RUN_SOURCE_AUTH_ERROR`, `RUN_SOURCE_RESPONSE_CHANGED`, `RUN_SOURCE_BAD_REQUEST`, `RUN_SOURCE_INVALID_SYMBOL`, `RUN_SOURCE_PROVIDER_REJECTED_RANGE`, `RUN_SOURCE_MALFORMED_PAYLOAD`, `RUN_SOURCE_PARTIAL_RESPONSE` | provider, timeout, retry max, attempts, final HTTP status, final reason | adapter telemetry + held run event | none after local validation | registry/seed verified by guard | local targeted/full PHPUnit PASS |
