...
- when operator passes `--output_dir` and attempt-level source telemetry exists for the run, `market-data:daily` may also persist `source_attempt_telemetry.json` beside the daily summary artifact and print its artifact path so bounded retry/backoff proof for that run can be archived directly from the daily operator path
...
- active code now caps effective API retry budget at `3` even if runtime config is set higher, so operator-facing `retry_max` proof must reflect the capped effective value rather than a more aggressive raw env override
