...
Current validation status: `READY_FOR_LOCAL_RUNTIME_VALIDATION`; container PHPUnit is `BLOCKED_CONTAINER_RUNTIME_ENV` because `dom`, `mbstring`, `xml`, and `xmlwriter` are unavailable. Local validation must run direct `DbIntegrityFkImplicitIntegrityDecisionStaticGuardTest.php`, `DbIntegrity`, `StaticGuard`, and full `tests/Unit/MarketData` before promotion to DONE/LOCKED.
