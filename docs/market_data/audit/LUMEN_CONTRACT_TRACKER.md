...
### Scope
Contract sweep terhadap consumer read surface agar semua pembacaan market-data yang dikonsumsi operator/evidence/replay/snapshot/backfill tetap pointer-first dan readable-only.

### Changes
- `EodEvidenceRepository::findPublicationForRun()` no longer resolves by `run_id + orderByDesc(publication_id)`; it now requires pointer-resolved current publication with sealed/readable/success/pass/current constraints.
- `EodEvidenceRepository::findReplayMetric()` no longer supports missing `trade_date`; latest-row replay metric fallback is blocked at repository level, not only command/service level.
- Static guard test added to prevent regression in replay evidence latest-row lookup and evidence publication lookup.

### Test Proof
Static syntax proof completed:
- `EodEvidenceRepository.php` → `php -l` PASS
- `ConsumerSurfaceSweepStaticGuardTest.php` → `php -l` PASS
- `MarketDataEvidenceExportService.php` → `php -l` PASS
- `ReplayVerificationService.php` → `php -l` PASS
- `SessionSnapshotService.php` → `php -l` PASS
- `ReplayBackfillService.php` → `php -l` PASS
- `EligibilitySnapshotScopeRepository.php` → `php -l` PASS
- `EodPublicationRepository.php` → `php -l` PASS
- `ExportEvidenceCommand.php` → `php -l` PASS

PHPUnit/artisan proof is pending because uploaded ZIP has no `vendor/`.

### Result
Contract enforcement is improved for the remaining concrete consumer gaps found during static sweep. Evidence/replay cannot silently select newest publication/replay row anymore.

### Contract Impact
Read-side enforcement now includes repository-level protection for replay metric selection and evidence publication resolution, reducing reliance on service/command guards only.

### Remaining Gap
PARTIAL until full local regression evidence confirms no consumer surface regression and no hidden consumer remains outside pointer/readable contract.
...
Consumer Surface Sweep static guards must be runnable in plain PHPUnit unit context without requiring a bootstrapped Lumen application helper. Correction finalize must preserve existing correction lifecycle invariants while enforcing read-side consumer safety.

### Enforcement Update
- `ConsumerSurfaceSweepStaticGuardTest` no longer depends on `base_path()` and can run under the same plain `PHPUnit\Framework\TestCase` style used by the surrounding unit tests.
- Correction finalize keeps artifact-history promotion deterministic and single-shot while restoring the pre-existing expectation that history promotion is attempted before pointer-promotion lock conflict handling.

### Contract Impact
No read-side contract was weakened. Evidence publication lookup remains pointer-resolved, replay metric lookup remains explicit-date only, and correction lock-conflict behavior is restored to the existing test contract.

### Remaining Gap
PARTIAL until local PHPUnit confirms the static guard, evidence/replay subset, and full `tests/Unit/MarketData` suite pass.
...
### Scope
Mengunci contract current pointer readiness pada write-side finalization, pointer promotion, fallback restore, dan current-state scanner.

### Changes
- Pointer-resolved readable gateway sekarang mensyaratkan:
  - pointer publication_id/version match publication
  - pointer run_id match publication run_id
  - run terminal_status `SUCCESS`
  - run publishability_state `READABLE`
  - run coverage_gate_state `PASS`
  - run `is_current_publication = 1`
  - run `publication_id` dan `publication_version` match pointer
- Invalid current-state scan sekarang dapat menemukan ghost pointer karena publication join tidak lagi menghilangkan pointer orphan.
- Post-switch verification menjadi bagian dari promotion/restore repository transaction.
- Static guard test dibuat agar future regression terhadap pointer mirror, ghost pointer, dan post-switch verification gagal cepat.

### Test Proof
Static syntax proof selesai untuk file repository, pipeline service, run repository, integration fixture, dan static guard test. PHPUnit lokal belum bisa diklaim dari ZIP ini karena `vendor/` tidak tersedia.

### Result
Contract readiness meningkat dari repository-level guard saja menjadi pointer-resolved + run mirror + post-switch verification. Tidak ada silent pointer repair: mismatch tetap mengarah ke restore/clear path di pipeline dan reason/event eksplisit.

### Contract Impact
Current pointer readiness sekarang mengharuskan linkage tiga arah:
- `eod_current_publication_pointer.publication_id/version`
- `eod_publications.publication_id/version/run_id`
- `eod_runs.publication_id/version/is_current_publication`

Readable current tidak boleh lolos jika salah satu mirror tidak sinkron.

### Remaining Gap
Butuh local evidence:
- static guard PASS
- finalize filter PASS
- pointer filter PASS
- full `tests/Unit/MarketData` PASS
