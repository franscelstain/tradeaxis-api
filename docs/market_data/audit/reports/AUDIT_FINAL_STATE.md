...
Verdict ini hanya menutup dokumentasi strategi. Ia tidak menyatakan sistem saat ini research-ready, implementation-conformant, operationally validated, atau production-ready.

**Pembaruan W22, 2026-08-06 — claim level implementasi ditetapkan.** Setelah W00–W22 dijalankan: `IMPLEMENTATION_CONFORMANT` **tidak diberikan** (24 P1 material tersisa; seluruh P0 ditutup pada W21), `OPERATIONALLY_VALIDATED` **tidak diberikan** (nol sesi teraktivasi), production relock **tidak diberikan**. Claim level final adalah **`IMPLEMENTATION_READY`** — Layer A dan B kuat, `runtime-proven` tidak dipenuhi karena bukti Layer C yang ada sebagian besar tidak admissible. Rinciannya pada `../MARKET_DATA_IMPLEMENTATION_LEDGER.md`, bagian **W22 record**. Klaim lama `FULL GLOBAL MARKET-DATA PRODUCTION READY` tetap historical dan tidak membuktikan kesesuaian terhadap baseline yang sekarang dikoreksi.
...
| P0-04 | Price basis kontradiktif dan dapat mencampur scale | Docs memilih `ADJ_CLOSE`, runtime memakai `close`, selected default membolehkan per-date fallback | MA, ROC, ATR, dan ranking dapat mengalami discontinuity palsu | Kunci `STRUCTURAL_ADJUSTED` sebagai signal basis atau basis lain yang eksplisit; hilangkan per-row fallback — **`CLOSED` pada remediasi W21, 2026-08-06.** Basis kini eksplisit dan tunggal: `price_basis_default = close` atas RAW, adapter mendeklarasikan `forbidden_canonical_basis = [PROVIDER_ADJ_CLOSE]`, provider `adj_close` tidak masuk baris canonical sama sekali (`'adj_close' => null` pada W12) dan tidak pernah diskalakan faktor platform. Setiap vektor indikator menyatakan `price_product_code` miliknya sendiri, dan read product tidak lagi mengarang `RAW` untuk baris yang tidak pernah mencatatnya. Per-row fallback hilang di ketiga tempatnya. **Residu terekam:** 756.328 baris indikator lama tanpa label produk — lihat `P1-32` |
...
### Hasil per order — indeks berurutan 1→22 (review 2026-08-03)
...
| 22 | Audit dan proof pack | **`TIDAK TERPENUHI`** | **31 temuan terbuka**; 14 dokumen klaim usang | seluruhnya |
...
| P0-04 mixed/incoherent price basis | `CLOSED` | runtime default masih `close`, `IndicatorVectorService` masih memiliki `adj_close` selection path, dan belum ada structural-adjusted factor-bound product builder **Ditutup pada remediasi W21, 2026-08-06.** Basis tunggal dan eksplisit: `close` atas RAW, `forbidden_canonical_basis = [PROVIDER_ADJ_CLOSE]`, provider `adj_close` tidak masuk baris canonical dan tidak pernah diskalakan faktor platform, setiap vektor menyatakan `price_product_code`, dan read product tidak lagi mengarang `RAW`. Residu: 756.328 baris indikator lama tanpa label — `P1-32`. |
...
| P1-27 membership sector: sumber campur dan interval tak pernah ditutup | `OPEN` | Dua cacat pada `ticker_sector_memberships`. Pertama, seluruh baris berlabel `classification_system = 'IDX-IC'` tetapi sumbernya campur: `idx_stock_screener` 888, `ksei` 6, `idx_profile` 3, `investing` 2, `invesnesia` 1, dan `lembarsaham` 1 — empat baris terakhir bukan sumber IDX, sehingga di bawah `Sector_Classification_Contract_LOCKED.md` berkelas `DERIVED_REFERENCE` dan **bukan membership otoritatif**. Kedua, dari **971 membership untuk 971 emiten, nol memiliki `effective_to`**: struktur temporalnya lengkap dan tidak pernah dipakai, sehingga reklasifikasi IDX tidak pernah tercatat dan setiap tanggal historis meresolusi ke sektor hari ini. Dampaknya langsung ke `sector_roc20` pada 739.651 baris dan `rs_20_vs_sector` pada 734.645 baris indikator. Bentuknya sama dengan survivorship pada order 6. Ditemukan saat penutupan `DOC-15` |
...
**Status register per 2026-08-03: seluruh `DOC-01`–`DOC-83` `CLOSED`.** `DOC-15` ditutup terakhir setelah keputusan kepemilikan sector diberikan. Dua temuan implementasi yang muncul selama penyusunan register dipindahkan ke backlog sebagai `P1-13` dan `P1-14`; keduanya bukan kekurangan dokumen — pada `P1-14` justru dokumennya yang benar dan database yang menyimpang.
